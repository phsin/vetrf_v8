...
//Ссылка на задачу: https://redmine.kb99.pro/issues/6778
//существует похожий тест, но на проведение просрочки без ошибки

Как Администратор я хочу
проверить, что интеграция НЕ позволит провести просрочку при соответствующих настройках
...
Сценарий: Просрочка по реализации №00000000001 от 31.01.2023
	...
	*Выбор просроченной партии и проверка сообщений
		...
		//И в таблице 'Список' я выбираю текущую строку - аналог 'ФормаВыбрать'
		...
		Затем я жду, что в сообщениях пользователю будет строка "Выбрана партия с недопустимым сроком годности: * * (менее * дней до окончания срока) в строке *!" по шаблону в течение 90 секунд
