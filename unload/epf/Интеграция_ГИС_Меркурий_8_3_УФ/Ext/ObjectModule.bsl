...
Процедура СообщитьИнфо(стр)  Экспорт
...
Функция СохранитьИмяЗапросаВДокументе(док) Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение 
		Сообщить("Обновите SoapDLL = "+КомпонентаНаСервере.Version);
...
//		ВидДокумента = ВыбратьВидДокументаВСД(type);
		...
		Если ЭтоТранспортныйВСД = 1 Тогда
			Попытка докОбъект.Отправитель_ХозСубъект = НайтиХСпоGUID(vetDocument.certifiedConsignment.consignor.businessEntity.guid); Исключение докОбъект.Отправитель_ХозСубъект = ""; КонецПопытки;
			Попытка докОбъект.Отправитель_Площадка 	= НайтиПлощадку(vetDocument.certifiedConsignment.consignor.enterprise.guid, докОбъект.Отправитель_ХозСубъект); Исключение КонецПопытки;
			Попытка докОбъект.Получатель_ХозСубъект = НайтиХСпоGUID(vetDocument.certifiedConsignment.consignee.businessEntity.guid); Исключение докОбъект.Получатель_ХозСубъект = ""; КонецПопытки;
			Попытка докОбъект.Получатель_Площадка 	= НайтиПлощадку(vetDocument.certifiedConsignment.consignee.enterprise.guid, докОбъект.Получатель_ХозСубъект); Исключение КонецПопытки;
...
Функция ИнициализацияХС() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Отправитель_ХозСубъект) Тогда
		Сообщить83("Не удалось получить ХС по указанному Issuer_id - проверьте корректность введенных данных");
		Возврат Ложь;
	КонецЕсли;
