Функция ЗапросПоТранзакции() 
	...
	|	ВЫРАЗИТЬ(ВСД2.Отправитель_Площадка.Адрес КАК СТРОКА(150)) ,
	...
	|	ВЫРАЗИТЬ(ДокВСД2.Получатель_Площадка.Адрес КАК СТРОКА(150)) КАК Получатель_ПлощадкаАдрес,	
	|	ДокВСД2.ТтнНомер КАК ТтнНомер,
	|	ДокВСД2.ТтнДата КАК ТтнДата,
	|	ВСД2_транзакция.Организация.Наименование,
	|	ДокВСД2.UUID,
	|	ВСД2_транзакция.Организация.Инн,
	|	ДокВСД2.ВсдДата КАК Дата,
	|	ВЫРАЗИТЬ(ДокВСД2.Получатель_ХозСубъект.Контрагент.НаименованиеПолное КАК СТРОКА(150)) КАК ПолучательХозСубъектНаименование,
	|	ДокВСД2.Получатель_ХозСубъект.Контрагент.ИНН КАК ПолучательХозСубъектИНН,
	|	ВЫРАЗИТЬ(ДокВСД2.НаименованиеПродукции КАК СТРОКА(150)) КАК НаименованиеПродукции,
	|	ДокВСД2.Количество,
	|	ВЫРАЗИТЬ(ДокВСД2.ДатаИзготовления КАК СТРОКА(50)) КАК ДатаИзготовления,
	|	ДокВСД2.ДатаИзготовления1,
	|	ДокВСД2.ДатаИзготовления2,
	|	ВЫРАЗИТЬ(ДокВСД2.ДатаСрокГодности КАК СТРОКА(50)) КАК ДатаСрокГодности,
