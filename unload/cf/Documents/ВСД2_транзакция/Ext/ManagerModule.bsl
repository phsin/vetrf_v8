...
Процедура Печать( ТабДокумент, МассивСсылок) Экспорт
	...
	Исключение
		ИспользоватьВнешнююКомпоненту=Истина;
		Макет = Документы.ВСД2_транзакция.ПолучитьМакет("Макет82");
	КонецПопытки;
	...
	Пока Выборка.Следующий() цикл
		...
		//Попытка
		//	тПолучатель = "Фирма-получатель: "+ СокрЛП( ДокСсылка.Контрагент.НаименованиеПолное)+", ИНН: "+ДокСсылка.Контрагент.ИНН;
		//Исключение
		...
		Если ИспользоватьВнешнююКомпоненту Тогда
			Попытка
				ОбластьМакета.Рисунки.Штрихкод.Объект.symbology = 58; // тип = QR
		    	ОбластьМакета.Рисунки.Штрихкод.Объект.barcode = ШтрихКод;
			исключение
	...
	//ОбластьУровниУпаковкиШапка = Макет.ПолучитьОбласть("УровниУпаковкиШапка");
	//ОбластьУровниУпаковки = Макет.ПолучитьОбласть("УровниУпаковки");
	//ОбластьМаркировкаШапка = Макет.ПолучитьОбласть("МаркировкаШапка");
	//ОбластьМаркировка = Макет.ПолучитьОбласть("Маркировка");
	//ОбластьУсловияПеревозкиШапка = Макет.ПолучитьОбласть("УсловияПеревозкиШапка");
	//ОбластьУсловияПеревозки = Макет.ПолучитьОбласть("УсловияПеревозки");
	//ТабДок.Очистить();
	...
	//	ТабДок.Вывести(ОбластьУровниУпаковкиШапка);
	//	ВыборкаУровниУпаковки = Выборка.УровниУпаковки.Выбрать();
	//	Пока ВыборкаУровниУпаковки.Следующий() Цикл
	//		ОбластьУровниУпаковки.Параметры.Заполнить(ВыборкаУровниУпаковки);
	//		ТабДок.Вывести(ОбластьУровниУпаковки, ВыборкаУровниУпаковки.Уровень());
	//	КонецЦикла;

	//	ТабДок.Вывести(ОбластьМаркировкаШапка);
	//	ВыборкаМаркировка = Выборка.Маркировка.Выбрать();
	//	Пока ВыборкаМаркировка.Следующий() Цикл
	//		ОбластьМаркировка.Параметры.Заполнить(ВыборкаМаркировка);
	//		ТабДок.Вывести(ОбластьМаркировка, ВыборкаМаркировка.Уровень());
	//	КонецЦикла;

	//	ТабДок.Вывести(ОбластьУсловияПеревозкиШапка);
	//	ВыборкаУсловияПеревозки = Выборка.УсловияПеревозки.Выбрать();
	//	Пока ВыборкаУсловияПеревозки.Следующий() Цикл
	//		ОбластьУсловияПеревозки.Параметры.Заполнить(ВыборкаУсловияПеревозки);
	//		ТабДок.Вывести(ОбластьУсловияПеревозки, ВыборкаУсловияПеревозки.Уровень());
	//	КонецЦикла;
