...
Функция ПолучитьДанныеДляСозданияВСДТранзакции( ПараметрыОрганизации ) 
	...
	Если кб99_ВСД_Общий.ЭтоДокументОтгрузка(ДокументОснование) Тогда
	...
	ИначеЕсли  кб99_ВСД_Общий.ЭтоДокументПеремещение(ДокументОснование) Тогда
	...
	Иначе
		...
		Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОснование, "Контрагент") Тогда 						
	    ...
	    ИначеЕсли ПараметрыОрганизации["РеквизитГрузополучатель"] = 1 Тогда
			//Адрес доставки
			Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОснование, "АдресДоставки") Тогда 						
				_Получатель_Площадка = кб99_ВСД.НайтиПлощадкуПоКонтрагенту(ДокументОснование.АдресДоставки);
			Иначе
		...
		ИначеЕсли ПараметрыОрганизации["РеквизитГрузополучатель"] = 2 Тогда
	        // Партнер
	        Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОснование, "Партнер") Тогда 						
				_Получатель_Площадка = кб99_ВСД.НайтиПлощадкуПоКонтрагенту(ДокументОснование.Партнер);
			Иначе
		...
		ИначеЕсли ПараметрыОрганизации["РеквизитГрузополучатель"] = 3 Тогда
	        // Грузополучатель
	        Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОснование, "Грузополучатель") Тогда 						
				_Получатель_Площадка = кб99_ВСД.НайтиПлощадкуПоКонтрагенту(ДокументОснование.Грузополучатель);
			Иначе
		...
		Иначе	
			// Договор
	        Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОснование, "Договор") Тогда 						
				_Получатель_Площадка = кб99_ВСД.НайтиПлощадкуПоКонтрагенту(ДокументОснование.Договор);
			Иначе
...
Процедура ОбработкаЗаполнения( ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если кб99_ВСД_Общий.ЭтоДокументОтгрузка(ДанныеЗаполнения) 
		 	ИЛИ	кб99_ВСД_Общий.ЭтоДокументПеремещение(ДокументОснование) Тогда
		...
		ТтнДата = Дата; 
		номерАвто = кб99_ВСД_Общий.ПолучитьНомерАвто( ДокументОснование, ПараметрыОрганизации ); 
	...
	ИначеЕсли ( ТипЗнч(ДанныеЗаполнения) = Тип("Структура") ) Тогда 
		...
		Иначе
			...
			Если ПараметрыДокумента.Свойство("ДокОснование") И ПараметрыДокумента.ДокОснование <> Неопределено Тогда 
				...
				Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОснование, "Организация") Тогда 			
					Организация = ДокументОснование.Организация;
				Иначе
