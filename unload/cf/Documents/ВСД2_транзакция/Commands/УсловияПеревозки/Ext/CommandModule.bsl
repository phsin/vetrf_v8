...
Процедура кнРегионализацияНаСервере(Знач ДокСсылка)
	Обработка = кб99_ВСД.ИнициализацияОбработки();
...
Процедура ОбработкаОтвета(Ответ, Парам) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Если кб99_ВСД.ЕстьреквизитОбъекта("Статус",Парам.ПараметрКоманды) Тогда
			Если СокрЛП(кб99_ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(Парам.ПараметрКоманды,"Статус")) = "COMPLETED" Тогда
		...
		Иначе
			...
			ДокСсылка = кб99_ВСД.НайтиВсдТранзакцию(Парам.ПараметрКоманды);
