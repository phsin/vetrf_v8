...
Процедура ТоварыПартияПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элемент.Родитель.ТекущиеДанные;
	ДанныеСтрокиТЧ = новый структура("Партия, Продукция_Элемент, ЕдиницаИзмерения, ДатаИзготовления, ДатаИзготовления2, ДатаСрокГодности, ДатаСрокГодности2, Цель, КлючСтроки");
...
Процедура ЗаполнитьРеквизитыСтрокиПродукцияСервер(СтрокаТЧ)
	...
	Если ПереопределенныйМодуль <> Неопределено Тогда		
		ПереопределенныйМодуль.ЗаполнитьРеквизитыСтрокиПродукцияСервер( СтрокаТЧ );
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Партия) Тогда
		...
		СтрокаТЧ.РезультатыИсследований = ?(ТипЗнч(СтрокаТЧ.Партия.vetDocument.РезультатыИсследований) = Тип("ПеречислениеСсылка.кб99_РезультатИсследования"),СтрокаТЧ.Партия.vetDocument.РезультатыИсследований,"");
		...
		СтрокаУпак.Количество = 1; //Цел(СтрокаОснования.Упаковки/СтрокаОснования.Количество*НоваяСтрока.Количество); //Пропорционально Распределенному количеству
