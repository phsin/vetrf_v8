...
Процедура ЗаписатьСоответсвиеХозСубъекта(ДокументОснование, Получатель_ХозСубъект)
	...
	Если кб99_ВСД_Общий.ЭтоДокументПеремещение(ДокументОснование)
...
Процедура ЗаполнитьРеквизитыСтрокиПродукцияСервер(СтрокаТЧ)
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Партия) Тогда
		Если кб99_ВСД_Общий.ПроверитьЗначениеПараметра(ПараметрыОрганизации, "РазрешитьПодменуПродукцииВИсходящемВСД", "Разрешить подмену продукции в исходящем ВСД") Тогда 
		...
		СтрокаУпак.Количество = кб99_ВСД_Общий.РассчитатьКоличествоПотребительскихУпаковок(СтрокаУпак, СтрокаТЧ);
...
Процедура ПодборПоДокументуОснованию(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		Если кб99_ВСД_Общий.ЭтоДокументОтгрузка(Объект.ДокументОснование) Тогда
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекСтрока.Партия) Тогда
		...
		Если ТекСтрокаУпаковки <> Неопределено Тогда
			...
			ТекСтрокаУпаковки.Количество = кб99_ВСД_Общий.РассчитатьКоличествоПотребительскихУпаковок(СтрокаУпак, СтрокаТЧ);	
		КонецЕсли;
