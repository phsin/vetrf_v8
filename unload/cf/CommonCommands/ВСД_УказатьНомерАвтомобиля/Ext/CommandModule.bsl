...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	ВСД_Док = ?(ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.РеализацияТоваровУслуг"), кб99_ВСД.НайтиВсдТранзакцию(ПараметрКоманды), ПараметрКоманды);
	Если ЗначениеЗаполнено(ВСД_Док) Тогда
		...
		Иначе
			кб99_ВСД.СообщитьИнфо(СтрШаблон("В документ %1 не требуется указание номеров автомобиля в точках перегрузки", ВСД_Док));
		КонецЕсли;
	Иначе
