...
Процедура ВыполнитьНаСервере( ПараметрКоманды )
	...
	Для Каждого ДокСсылка Из ПараметрКоманды Цикл
		...
		Если кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта( ДокСсылка, "Отправитель_Площадка" ) Тогда 
			//ВСД2_транзакция
			ПараметрыОрганизации.Отправитель_Площадка = ДокСсылка.Отправитель_Площадка;
			
		ИначеЕсли кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта( ДокСсылка, "Производитель_Площадка" ) Тогда 
			//ВСД2_Производство
			ПараметрыОрганизации.Отправитель_Площадка = ДокСсылка.Производитель_Площадка;
			
		ИначеЕсли кб99_ВСД_Общий.ЕстьРеквизитИлиСвойствоОбъекта( ДокСсылка, "Владелец_Площадка" ) Тогда 
