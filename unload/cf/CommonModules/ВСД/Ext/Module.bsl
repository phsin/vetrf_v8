...
Функция ПроверитьСуществованиеКаталога(ИмяКаталога) Экспорт
    ...
    Иначе
		...
		СообщитьИнфо("каталога ["+ ИмяКаталога +"] не найден, создаем...");
...
//         СообщитьИнфо("Не определена Основная Организация пользователя");
//     КонецЕсли;
...
Функция ПогаситьВСДПоставщика(Знач ДокСсылка) Экспорт
	...
	Попытка
		...
		иначе
			СообщитьИнфо("Данный вид документа не гасится" + ДокСсылка);
			Возврат "";
		КонецЕсли;
	Исключение
		СообщитьИнфо("Ошибка в Процедуре ВСД ПогаситьВСДПоставщика");
		СообщитьИнфо(ОписаниеОшибки());
		Возврат "";
	КонецПопытки;
КонецФункции
...
//		СообщитьИнфо(""+ВыбДок+" Статус "+Статус+" не запрашиваю");
//		Рез = Ложь;
//	КонецЕсли;
//	Возврат Рез;
//КонецФункции

Функция Получить_Ответ_Меркурий( Параметры ) Экспорт
	Попытка
		Если типЗнч(Параметры) = Тип("Массив") Тогда
			Для Каждого ДокСсылка из Параметры Цикл
				Параметры = ВСД.ЗагрузитьПараметры( ДокСсылка.Организация );									
				Ответ =  ВСД_Запросы.ПолучитьРезультат_ВСД2( Параметры, ДокСсылка.applicationID, ДокСсылка, ложь);	
			КонецЦикла;
		Иначе
			ДокСсылка = Параметры;
			Параметры = ВСД.ЗагрузитьПараметры( ДокСсылка.Организация );									
...
//	     Если НЕ ФайлНаДиске.Существует() Тогда
//	         СообщитьИнфо("Файл: "+АдресФайла + " Отсутствует !!! - Проверьте параметр КаталогИнтеграции и наличие обработки в каталоге");
//			 Возврат "";
//	     КонецЕсли;	 
//		 
//		Попытка		
...
//		     Возврат Обработка;
//		 Исключение
//			СообщитьИнфо("Ошибка подключения Интеграция_ГИС_Меркурий");
//			СообщитьИнфо(ОписаниеОшибки());
//			Возврат "";
//		КонецПопытки;
//	Иначе		
...
//	         СообщитьИнфо("Каталог: "+КаталогИнтеграции + " не существует !!! 
...
//		 	СписокПараметров = ХранилищеОбщихНастроек.Загрузить(КлючОбъекта,Строка(КлючНастроек),,ВладелецНастроек); // на пользователя
		 	СписокПараметров = ХранилищеОбщихНастроек.Загрузить(КлючОбъекта, _Организация.УникальныйИдентификатор(), , ВладелецНастроек); // _Организация на пользователя
		Иначе
...
Функция СохранитьПараметры(Объект, _ИмяПользователя = Неопределено) Экспорт
	...
	Попытка _НастройкиДляТекущегоПользователя = Объект.флСохранятьНастройкиДляТекущегоПользователя; Исключение _НастройкиДляТекущегоПользователя = Ложь; КонецПопытки;
...
//		ХранилищеОбщихНастроек.Сохранить(КлючОбъекта,  Строка(КлючНастроек), СписокКонстант, , _ИмяПользователя);
		ХранилищеОбщихНастроек.Сохранить(КлючОбъекта,  _Организация.УникальныйИдентификатор(), СписокКонстант, , _ИмяПользователя);  // Организация по пользователю
	Иначе
		ХранилищеОбщихНастроек.Сохранить(КлючОбъекта,  _Организация.УникальныйИдентификатор(), СписокКонстант, , _Организация.УникальныйИдентификатор());
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыВОбработку(Объект, СписокПараметров) Экспорт
	Попытка
		...
		Объект.param_username 		= СписокПараметров.Получить("param_username"); ;  // для базовой авторизации
		Объект.param_password 		= СписокПараметров.Получить("param_password");
		Объект.param_issuer_id		= СписокПараметров.Получить("param_issuer_id"); // идентификатор хозяйствующего субъекта в реестре РСХН.
		Объект.param_service_id 	= СписокПараметров.Получить("param_service_id") ;
		Объект.param_api_key 		= СписокПараметров.Получить("param_api_key") ;

		Объект.param_intiator_login 	= СписокПараметров.Получить("param_intiator_login"); // ! логин ветврача или др. пользователя в Меркурии
		Объект.param_vetdoctor_login 	= СписокПараметров.Получить("param_vetdoctor_login");
		Объект.param_vetdoctor_fio 		= СписокПараметров.Получить("param_vetdoctor_fio");
		Объект.param_vetdoctor_post 	= СписокПараметров.Получить("param_vetdoctor_post");
		...
		Объект.Страна 				= СписокПараметров.Получить("Страна");
		Объект.Регион 				= СписокПараметров.Получить("Регион");
		Объект.Город 				= СписокПараметров.Получить("Город");
		
		Объект.ВСД_Экспертиза 		= СписокПараметров.Получить("ВСД_Экспертиза");	
		Если НЕ(ЗначениеЗаполнено(Объект.ВСД_Экспертиза)) Тогда		
			Объект.ВСД_Экспертиза 	= "Произ.лабор ";
		КонецЕсли;
		
		Объект.ВСД_Местность 		= СписокПараметров.Получить("ВСД_Местность");
		...
		Объект.ВСД_ОсобыеОтметки 	= СписокПараметров.Получить("ВСД_ОсобыеОтметки");
			
		Объект.Смещение			= СписокПараметров.Получить("Смещение");
		
		Объект.РеквизитГрузополучатель 	= СписокПараметров.Получить("РеквизитГрузополучатель");
		Объект.ПропускатьПустыеСвойства 	= СписокПараметров.Получить("ПропускатьПустыеСвойства");
		
		Объект.КаталогИнтеграции 	= СписокПараметров.Получить("КаталогИнтеграции");
		...
		Объект.ПаузаСек 				= СписокПараметров.Получить("ПаузаСек");
		Если НЕ(ЗначениеЗаполнено(Объект.ПаузаСек)) Тогда
			Объект.ПаузаСек=10;
		КонецЕсли;
		Объект.ОтладкаЗапросовXML = СписокПараметров.Получить("ОтладкаЗапросовXML");
		Если НЕ(ЗначениеЗаполнено(Объект.ОтладкаЗапросовXML)) Тогда 
			Объект.ОтладкаЗапросовXML = Истина;
		КонецЕсли ;
		
		Объект.ТестовыйРежим = СписокПараметров.Получить("ТестовыйРежим");
		Если НЕ(ЗначениеЗаполнено(Объект.ТестовыйРежим)) Тогда 
			Объект.ТестовыйРежим = Истина;
		КонецЕсли ;
		
		Объект.флИспользоватьВнешнююОбработку = СписокПараметров.Получить("ИспользоватьВнешнююОбработку");
		Объект.флИспользоватьВнешнююОбработку = ?(ЗначениеЗаполнено(Объект.флИспользоватьВнешнююОбработку),Объект.флИспользоватьВнешнююОбработку,Ложь);

		Объект.ПолноеИмяФайлаОбработки = СписокПараметров.Получить("ПолноеИмяФайлаОбработки");
		
		Если Объект.флИспользоватьВнешнююОбработку Тогда 
			Если НЕ ПроверитьСуществованиеФайла(Объект.ПолноеИмяФайлаОбработки) Тогда			
				Объект.ПолноеИмяФайлаОбработки = Объект.КаталогИнтеграции + "Интеграция_ГИС_Меркурий.epf";
				ПроверитьСуществованиеФайла( Объект.ПолноеИмяФайлаОбработки );
			КонецЕсли;
		КонецЕсли;
		
		Объект.ВыводитьПодробнуюИнформацию = СписокПараметров.Получить("ВыводитьПодробнуюИнформацию");
		Объект.АвтоЗаписьВСДСоответствия = СписокПараметров.Получить("АвтоЗаписьВСДСоответствия");
		
		Объект.НазваниеРеквизитаКоличество = СписокПараметров.Получить("НазваниеРеквизитаКоличество");
		Если НЕ ЗначениеЗаполнено(Объект.НазваниеРеквизитаКоличество) Тогда
			Объект.НазваниеРеквизитаКоличество = "Количество";
		КонецЕсли;	
		
		Объект.ПарамКоэффициентПересчетаКоличества = СписокПараметров.Получить("ПарамКоэффициентПересчетаКоличества");
		Объект.ВСД_РезультатыИсследований = СписокПараметров.Получить("ВСД_РезультатыИсследований");
		Объект.ТермУсловияПеревозки = СписокПараметров.Получить("ТермУсловияПеревозки");
		Объект.ВСДЦель = СписокПараметров.Получить("ВСДЦель");
		Объект.ПарамЗаполнятьТранзакциюПриОтсутствииПартий = СписокПараметров.Получить("ПарамЗаполнятьТранзакциюПриОтсутствииПартий");
		Объект.ПарамНомерУровняУпаковкиДляВСД = СписокПараметров.Получить("ПарамНомерУровняУпаковкиДляВСД");
		Объект.ПарамФормаУпаковкиДляВСД = СписокПараметров.Получить("ПарамФормаУпаковкиДляВСД");
		Объект.НазваниеРеквизитаКоличествоМест = СписокПараметров.Получить("НазваниеРеквизитаКоличествоМест");
		Объект.ОчищатьСправочникВСД_Партии = СписокПараметров.Получить("ОчищатьСправочникВСД_Партии");
		Объект.ПарамКолонкаСортировкиПартииСписания = СписокПараметров.Получить("ПарамКолонкаСортировкиПартииСписания");
		Объект.ПарамЗнакСортировкиУбывание = СписокПараметров.Получить("ПарамЗнакСортировкиУбывание");
		Объект.ПарамФильтроватьРеализациюПоСкладуПлощадкиОтправителя = СписокПараметров.Получить("ПарамФильтроватьРеализациюПоСкладуПлощадкиОтправителя");	

		Объект.ПарамПоставщикОрганизация = СписокПараметров.Получить("ПарамПоставщикОрганизация");	
		Объект.ПарамПоставщикХозСубъект = СписокПараметров.Получить("ПарамПоставщикХозСубъект");	
		Объект.ПарамПоставщикПлощадка = СписокПараметров.Получить("ПарамПоставщикПлощадка");	
		Объект.ПарамРазрешитьВводНаОснованииБолееОдногоВСД = СписокПараметров.Получить("ПарамРазрешитьВводНаОснованииБолееОдногоВСД");
		Объект.ПарамРазрешитьВводНаОснованииБолееОдногоВСД = ?(ЗначениеЗаполнено(Объект.ПарамРазрешитьВводНаОснованииБолееОдногоВСД),Объект.ПарамРазрешитьВводНаОснованииБолееОдногоВСД,Ложь);
		
		Объект.ПолноеИмяФайлаПереопределения = СписокПараметров.Получить("ПолноеИмяФайлаПереопределения");
		Объект.флСохранятьНастройкиДляТекущегоПользователя = СписокПараметров.Получить("флСохранятьНастройкиДляТекущегоПользователя");
		
		Объект.ТермическиеУсловияПеревозки = СписокПараметров.Получить("ТермическиеУсловияПеревозки");
		Объект.ОтправлятьВФоне = СписокПараметров.Получить("ОтправлятьВФоне");
	Исключение
...
//     Набор.Записать(true);
//	 СообщитьИнфо(""+Первый+" --> "+Второй+ " : установлено соответствие");
//КонецПроцедуры
...
//		СообщитьИнфо("Обязательные реквизиты не заполнены !!!");
//		Возврат Ложь;
//	Иначе
//		Возврат Истина;
//	КонецЕсли;
...
#Область Партии
Функция ПолучитьАктуальныеПартии(Знач Параметры, ВыбПродукция_Элемент, ВыбПлощадка, ВыбХозСубъект) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры.Получить("ПарамКолонкаСортировкиПартииСписания")) Тогда 
		ПолеСортировки = СокрЛП(Параметры.Получить("ПарамКолонкаСортировкиПартииСписания"));
	Иначе
	...
	//Если ЗнакСортировки = Неопределено Тогда
		ЗнакСортировки = Параметры.Получить("ПарамЗнакСортировкиУбывание");		
	//КонецЕсли;
...
Функция тзПартииСписанияПоТзПродукция_Элемент(Параметры, тзЭлементыкСписанию, тзАктуальныхПартий = "") Экспорт
...
// в Парам = Структура переданы Отправитель_ХозСубъект и Отправитель_Площадка
    ...
    Для Каждого ТекСтрокаТовары Из тзЭлементыкСписанию Цикл
        ...
        Если СтрокиПартий.Количество() = 0  Тогда
            Если Параметры.Получить("ПарамЗаполнятьТранзакциюПриОтсутствииПартий") Тогда
        ...
        Если (СтрокиПартий.Итог("Количество") < ТекСтрокаТовары.Количество) и Параметры.Получить("ПарамЗаполнятьТранзакциюПриОтсутствииПартий")  Тогда
...
//	Исключение
//		СообщитьИнфо("Компонента не ЗАВЕЛАСЬ");
//		СообщитьИнфо(ОписаниеОшибки());
//		Возврат "";
//	КонецПопытки;
//	
...
//	Исключение
//		Обработка.КомпонентаНаСервере = Неопределено;
//		СообщитьИнфо("Библиотека не зарегистрирована");
//		СообщитьИнфо(ОписаниеОшибки());
//		Возврат "";
//	КонецПопытки;
//КонецФункции
