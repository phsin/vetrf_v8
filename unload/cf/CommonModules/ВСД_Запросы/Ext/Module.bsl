...
Функция ПометитьНаУдалениеСправочник(ИмяСправочника) 
...
Функция СохранитьСтатусЗапроса( xdto, ДокСсылка )
	Статус = "";
...
Функция Дата_из_XML(dateXML, Дата1, Дата2, ДатаСтрока )  
	попытка			
...
Функция Дата_из_XML_ВСписокЗначений(dateXML,ДатаСЗ1,ДатаСЗ2,ДатаСтрока) 
...
Функция ИнициализацияХС_ЗагрузитьВсеСправочники( Параметры, АдресХранилища ) Экспорт
	...
	//
	//Параметры["ВыбGUID"] = Параметры["param_issuer_id"];
	//xdto = Неопределено;
	...
	Попытка
		...
		//Параметры["ВыбХозСубъект"] = Отправитель_ХозСубъект;		
...
Функция ИнициализацияХС_ЗагрузитьПродукцию_Все( Знач Параметры, АдресХранилища ) Экспорт
...
Функция ИнициализацияХС_ЗагрузитьВидыПродукции( Знач Параметры, ВидПродукцииGUID, Владелец, _Смещение=0 ) Экспорт
	//Перем ВидПродукцииGUID, Владелец, _Смещение;
	//ПараметрыПроцедуры.Свойство("ВидПродукцииGUID", ВидПродукцииGUID );
	//ПараметрыПроцедуры.Свойство("Владелец", Владелец );
	//Если ЗначениеЗаполнено(Параметры["Смещение"]) Тогда 
	//	_Смещение = Параметры["Смещение"];
	//Иначе
	//	_Смещение =0;
	//КонецЕсли;
	...
	//СтруктураПараметров = ВСД_Отправка.ЗагрузитьГлПеременныеИзВременногоХранилища( Параметры["АдресПараметров"] );	
	ПараметрыОтправки = ВСД_отправка.ПараметрыОтправкиИнициализация( Параметры );
	ПараметрыОтправки.ЗапросXML = ЗапросXML;
    ПараметрыОтправки.Service = Service;
    ПараметрыОтправки.Action = Action;
...
Функция Продукция_Элемент_Получить_Инфо( Знач Параметры, ВыбПродукцияЭлемент ) Экспорт
	//Продукция = Параметры["ВыбПродукцияЭлемент"];
	...
	Если ПустаяСтрока(ВыбПродукцияЭлемент.GUID) Тогда
		//СообщитьИнфо("В ВСД_Продукции_Элемент пустой GUID");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьРезультат_Список_Продукции( Параметры, xdto, ВыбПродукцияЭлемент="") Экспорт
	...
	//Попытка
	//	//xdto.Body.getProductItemByGuidResponse.productItem
	//	response = xdto.Body.Последовательность().ПолучитьЗначение(0);
	//	productItems = response.productItem;		
	//Исключение	
	//	//xdto.Body.getProductItemListResponse.productItemList.productItem
	//	//xdto.Body.result.modifyProducerStockListResponse.productItemList.productItem
	//	response = xdto.Body.result.Последовательность().ПолучитьЗначение(0);
	//	productItems = response.productItemList.productItem;
	//КонецПопытки;
	
	//Вариации загрузки при разных запросах ЖД 
	//Получение списка по площадке
	Попытка productItems = xdto.Body.getProductItemListResponse.productItemList.productItem; Исключение productItems = ""; КонецПопытки;
		
	Если (ТипЗнч(productItems)<>Тип("СписокXDTO"))и (ТипЗнч(productItems)<>Тип("ОбъектXDTO")) Тогда
		// Изменение  / добавление
		Попытка	productItems = xdto.Body.result.modifyProducerStockListResponse.productItemList.productItem; Исключение productItems = "";	КонецПопытки;
	КонецЕсли;
	
	Если (ТипЗнч(productItems)<>Тип("СписокXDTO"))и (ТипЗнч(productItems)<>Тип("ОбъектXDTO")) Тогда
		// Изменение  / добавление
		Попытка	productItems = xdto.Body.receiveApplicationResultResponse.application.result.modifyProducerStockListResponse.productItemList.productItem; Исключение productItems = "";	КонецПопытки;
	КонецЕсли;
	
	Если (ТипЗнч(productItems)<>Тип("СписокXDTO"))и (ТипЗнч(productItems)<>Тип("ОбъектXDTO")) Тогда
		Попытка	productItems = xdto.Body.getProductItemByGuidResponse.productItem;	Исключение	productItems = ""; КонецПопытки;
	КонецЕсли;	
		...
		Для каждого productItem из productItemМассив Цикл		
			...
			Попытка 
				...
				Для Каждого producing Из producingМассив Цикл
					СпрОбъект.Площадка = Площадка_НайтиПоGUID( Параметры, producing.location.guid ); 
				КонецЦикла
...
Функция Продукция_Элемент_Получить_Список_По_Площадке( Знач Параметры, ВыбПлощадка ) Экспорт
	...
	Service = "platform/services/2.0/ProductService";//"platform/services/ProductService";
...
Функция Продукция_Элемент_Изменить_ЗапросXML( Параметры, ВыбПродукцияЭлемент, ВыбКоманда)
	...
	Запрос ="<SOAP-ENV:Envelope 
		...
		Иначе
			...
			Запрос = Запрос + "
		    |              <dt:name>"+СокрЛП(ВыбПродукцияЭлемент.Наименование)+"</dt:name>";
...
Функция  Продукция_Элемент_Изменить( Знач Параметры, ВыбПродукцияЭлемент , ВыбКоманда ) Экспорт
	//Если ПустаяСтрока(ВыбПлощадка)=1 Тогда 
...
//ПолучитьРезультат_Изменить_Продукцию
Функция  Продукция_Элемент_Изменить_ПолучитьОтвет(Параметры, ВыбПродукцияЭлемент, appID) Экспорт
	...
	xdto = ВСД_Отправка.ПолучитьРезультатСервер( Параметры, appID );
...
////		СообщитьИнфо("Не удалось изменить Продукцию "+продукцияЭлемент);
//	КонецЕсли;
//	УдалитьXML( КомпонентаНаСервере.LogFilename );
...
Функция ЗаполнитьФормыУпаковки(Знач Параметры, АдресХранилища ) Экспорт
...
//НайтиПлощадку
Функция Площадка_НайтиПоGUID(Знач Параметры, guid, ХозСубъект="", ОбновитьДанные=ложь)
	Ответ = Неопределено;
	если НЕ ПустаяСтрока(Guid) Тогда			
		Запрос = Новый Запрос();
		ТекстЗапроса = "ВЫБРАТЬ
...
Функция Площадка_ЗагрузитьПоGUID(Знач Параметры, GUID, ХозСубъект=Неопределено, Ссылка_ВСД_Площадка=Неопределено) Экспорт
	...
	СообщитьИнфо(" Запрос getEnterpriseByGuid [ "+(GUID)+" ]");
	ЗапросXML = Площадка_ЗагрузитьПоGUID_Запрос(GUID);
	...
	Попытка	    
		...
		Если НЕ ЗначениеЗаполнено(ХозСубъект) Тогда
			СообщитьИнфо("Внимание: создана площадка ["+GUID+"] без указания ХозСубъекта ");
		Иначе 
		...
		СообщитьИнфо("Записана ВСД_Площадка ["+Объект_ВСД_Площадка.Наименование+"] GUID = "+ Guid);		
		//ЗафиксироватьТранзакцию();
...
Функция Площадка_ЗагрузитьПоХозСубъекту(Знач Параметры, ВыбХозСубъект, ЗагрузитьПлощадку=ложь ) Экспорт	
	...
	Если ПустаяСтрока(ВыбХозСубъект.GUID) Тогда 
		//Параметры["ВыбХозСубъект"] = ВыбХозСубъект;		
	...
	Попытка
		...
		Для каждого activityLocation из activityLocationList Цикл		
			...
			//ЗагрузитьПлощадкуПоGUID(guid, ВыбХозСубъект, , Параметры);
			Площадка_НайтиПоGUID(Параметры, guid, ВыбХозСубъект, ЗагрузитьПлощадку);
...
Процедура Площадки_ЗагрузитьСписокПоХозСубъекту(Знач Параметры, тзСвертка )  Экспорт // !!!! 
	
	Для Каждого стр Из тзСвертка Цикл
		Если стр.Отметка Тогда 
			Если стр.ХозСубъект = Справочники.ВСД_ХозСубъект.ПустаяСсылка() Тогда 
				стр.ХозСубъект = ВСД.НайтиХозСубъект(стр.Контрагент);
			КонецЕсли;	
			
			Параметры.Вставить("ВыбХозСубъект", стр.ХозСубъект );
			Площадка_ЗагрузитьПоХозСубъекту(Параметры, стр.ХозСубъект, Истина);							
...
Функция ХозСубъект_ПолучитьGuidПоИНН(Знач Параметры, ХозСубъект) Экспорт
	//ХозСубъект = Параметры["ВыбХозСубъект"];
...
// ЗагрузитьХСПоGUID
//Функция ХозСубъект_ЗагрузитьПоGUID(Знач Параметры, АдресХранилища ) Экспорт
Функция ХозСубъект_ЗагрузитьПоGUID(Знач Параметры, GUID) 
...
//Функция  Партии2_Запрос_Отправить(Знач Параметры, ВыбПлощадка, ПартииСмещение=0, УдалятьПартии = ложь, ПартияНачПериода = Неопределено, ПартияКонПериода = Неопределено, ТолькоАктуальныеПартии = ложь ) Экспорт	
Функция  Партии2_Запрос_Отправить(Знач Параметры ) Экспорт	
	...
	//ЗапросXML = ПолучитьПартии2_ЗапросXML(Параметры, ВыбПлощадка, ПартииСмещение, ПартияНачПериода, ПартияКонПериода, ТолькоАктуальныеПартии);	
	...
	//Результат = "";
	//Если ЗначениеЗаполнено(appID) Тогда
		//Параметры.Вставить("appID",appID);
...
Функция  Партии2_Ответ_Получить(Знач Параметры, appID)
	...
	//СообщитьИнфо("Отправляем запрос 2 на получение ответа Ветис" );
	...
	//Если Ответ Тогда
		...
		//СообщитьИнфо("Загрузка справочника ВСД_Партии...");
...
//ЗагрузитьXML_ВсеПартии2Ответ
Процедура Партии_Ответ_ЗагрузитьXML(Параметры, xdto )
	Если НайтиОшибки( xdto ) Тогда
		Возврат;	
	КонецЕсли;
...
Функция СписокПроизводителей_из_XML(Параметры, ProducerList) Экспорт 
	...
	Попытка
		Для каждого producer из ProdList Цикл
			...
			Попытка СтрокаТЗ.Площадка = Площадка_НайтиПоGUID( Параметры, producer.enterprise.guid ); Исключение КонецПопытки;
...
Функция Партия_Заполнить( Параметры, stockEntry)
	...
	Партия.Продукция_Элемент = Продукция_Элемент_Найти( Параметры, batch.productItem, Истина );
...
//	Попытка	Некачественный = batch.lowGradeCargo; Исключение	КонецПопытки;
	...
	Если Партия.Количество=0 Тогда 
		...
		Если ( Партия.ЭтоНовый()=0 ) Тогда			
			СообщитьИнфо("Количество партии guid ["+Партия.guid+"] uuid ["+Партия.uuid+"] = 0. Партия удалена. ", Партия);			
			Партия.Записать();
			Партия.УстановитьПометкуУдаления(истина);			
		Иначе
...
Процедура ОтправитьВСДвГИС(Знач Параметры, АдресХранилища) Экспорт
	...
	Если ЗначениеЗаполнено(Параметры["ДокСсылка"]) Тогда 
		СписокДокументов = Новый Массив;
		СписокДокументов.Добавить( Параметры["ДокСсылка"] );
	ИначеЕсли ЗначениеЗаполнено(Параметры["СписокДокументов"]) Тогда 
		СписокДокументов = Параметры["СписокДокументов"];
	КонецЕсли;
	Ответ = Истина;	
	Для Каждого ДокСсылка Из СписокДокументов Цикл
		
		ПараметрыОбщие = ВСД.ЗагрузитьПараметры( ДокСсылка.Организация );
		
		Если типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_транзакция") Тогда
			Ответ = ВСД2_транзакция_Отправить(ПараметрыОбщие, ДокСсылка) и Ответ;
		...
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Производство") тогда
			Ответ = ВСД2_Производство_Отправить(ПараметрыОбщие, ДокСсылка) и Ответ;
		...
		//иначе
			...
			//Иначе
			//	СообщитьИнфо("Данный вид документа не отправляется" + ДокВСД);
			//КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ProductItem_в_XML(СтрПартии, ПроверятьВидПродукцииПартииИProductItem = 0) Экспорт
	...
	Исключение КонецПопытки;
	//Попытка
	//    Запрос = Запрос+?(ПустаяСтрока(СтрПартии.GTIN)= 0,	"
...
Функция МаршрутСледования_из_XML( Параметры, list ) 
	...
	Попытка
	    ...
	    Для каждого узел из Routelist Цикл
			...
			Попытка СтрокаТЗ.ВыбПлощадка = Площадка_НайтиПоGUID(Параметры, Площадка_GUID ); Исключение КонецПопытки;
...
#Область ВСД2
Функция ПолучитьСписокВСД2_ЗапросXML( Параметры, ГУИД_Площадки, vetDocumentType, vetDocumentStatus, Смещение, ПоступилиС = "", ПоступилиПо ="",ОтХСГУИД = "", ОтПлощадкиГУИД="")
...
Функция  ПолучитьСписокВСД( Знач Параметры, ВыбПлощадка, vetDocumentType	= "INCOMING", vetDocumentStatus = "CONFIRMED", Смещение, ПоступилиС = "", ПоступилиПо ="", ОтХС = "", ОтПлощадки="", ЗаписыватьДокументы = ложь, ВсдРезультат = Неопределено) Экспорт
	...
	ЗапросXML = ПолучитьСписокВСД2_ЗапросXML( Параметры, СокрЛП(ВыбПлощадка.GUID), vetDocumentType, vetDocumentStatus, Смещение,ПоступилиС , ПоступилиПо,ОтХСГУИД, ОтПлощадкиГУИД);	
	...
	//СохранитьИмяЗапросаВДокументе(ДокСсылка);
...
//        |      <vd:consignment id='id1'>
	...
	ЗапросПодвал = ЗапросПодвал+"		  
        ...
        |        </vd:transportNumber>
        |      </vd:transportInfo>
        //|      <vd:transportStorageType>"+ ТермическиеУсловияПеревозки.Получить( ДокСсылка.ТермическоеСостояние-1 ) +"</vd:transportStorageType>
...
Функция ВСД2_транзакция_Отправить(Параметры, ДокСсылка) Экспорт
	...
	Ответ =  ПолучитьРезультат_ВСД2( Параметры, appID, ДокСсылка );	
	Для А=1 По 10 Цикл
		//Если НЕ Ответ Тогда 
		Если (Ответ="IN_PROCESS") Тогда
			Ответ =  ПолучитьРезультат_ВСД2( Параметры, appID, ДокСсылка );	
...
Функция ПолучитьРезультат_ВСД2(Параметры, appID, докСсылка = Неопределено, СделатьПаузу = Истина, ЗаписыватьДокументы=Истина, ВсдРезультат = Неопределено) Экспорт
	Если ПустаяСтрока(appID) Тогда
		СообщитьИнфо("Пустая ссылка applicationID");
		Возврат ложь;
	КонецЕсли;
...
//	Результат = КомпонентаНаСервере.SendRequestResult( applicationID );
	...
	НачатьТранзакцию();
	Попытка
		ЗагрузитьXML_ВСД2( Параметры, xdto, докСсылка, ЗаписыватьДокументы, ВсдРезультат);

		ЗагрузитьXML_Партии2( Параметры, xdto );
...
Функция ЗагрузитьXML_Партии2(Параметры, xdto ) 
...
//	xdto = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
	...
	Для каждого stockEntry из stockEntryList Цикл		
		...
		Партия_Заполнить( Параметры, stockEntry);
...
// ВСД = Струтура  результат загрузки xdto  
...
//	Если ОбъектXDTO.Установлено(Свойство) Тогда	
	...
	Для каждого vetDocument из vetDocumentList Цикл
		...
		//Если НЕ ЗначениеЗаполнено(ВсдСтруктура) Тогда 
			ВсдСтруктура = Новый Структура("ДокументОснование, Организация, ФормаВСД, UUID, applicationId, Статус, Дата, ВСДДата, ТипВСД, ВсдСерия, ВсдНомер, ВидВСД,
			...
			|ДатаИзготовления, ДатаИзготовления1, ДатаИзготовления2, ДатаСрокГодности, ДатаСрокГодности1, ДатаСрокГодности2, ДатаИзготовления1СЗ, ДатаИзготовления2СЗ, ДатаСрокГодности1СЗ, ДатаСрокГодности2СЗ,
		...
		Попытка ВсдСтруктура.Статус = vetDocument.vetDStatus; Исключение КонецПопытки;
		...
		//Попытка	ВсдСтруктура.Организация = ДокОснование.Организация; Исключение КонецПопытки;
...
//		ВидДокумента = ВыбратьВидДокументаВСД(type);
		...
		Если ЭтоТранспортныйВСД = 1 Тогда
			Попытка ВсдСтруктура.Отправитель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.consignor.businessEntity.guid); Исключение ВсдСтруктура.Отправитель_ХозСубъект = ""; КонецПопытки;
			Попытка ВсдСтруктура.Отправитель_Площадка 	= Площадка_НайтиПоGUID(Параметры, vetDocument.certifiedConsignment.consignor.enterprise.guid, ВсдСтруктура.Отправитель_ХозСубъект); Исключение КонецПопытки;
			Попытка ВсдСтруктура.Получатель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.consignee.businessEntity.guid); Исключение ВсдСтруктура.Получатель_ХозСубъект = ""; КонецПопытки;
			Попытка ВсдСтруктура.Получатель_Площадка 	= Площадка_НайтиПоGUID(Параметры, vetDocument.certifiedConsignment.consignee.enterprise.guid, ВсдСтруктура.Получатель_ХозСубъект); Исключение КонецПопытки;
			...
			Попытка
				...
				Если ТипТС = "1" Тогда
					ВсдСтруктура.номеравто = transportInfo.transportNumber.vehicleNumber;//("*[local-name()='transportNumber'] ").selectSingleNode("*[local-name()='vehicleNumber']").text; 
...
//					КонецЕсли;
		...
		Попытка
			ВсдСтруктура.ДатаИзготовления1СЗ = ДатаИзготовления1СЗ;	
			ВсдСтруктура.ДатаИзготовления2СЗ = ДатаИзготовления2СЗ;
...
//			докОбъект.ДатаИзготовления2СЗ = "";
		...
		Попытка
			ВсдСтруктура.ДатаСрокГодности1СЗ = ДатаСрокГодности1СЗ;	
			ВсдСтруктура.ДатаСрокГодности2СЗ = ДатаСрокГодности2СЗ;
			//ДатаИзготовления - уже записали	
		Исключение
...
//			докОбъект.ДатаСрокГодности2СЗ = "";
		...
		Если СоздаватьДокументы Тогда
			Если (ТипЗнч(докОснование) = Тип("ДокументСсылка.ВСД_транзакция")) или
				(ТипЗнч(докОснование) = Тип("ДокументСсылка.ВСД")) Тогда     //ЖД !!!!! ВНИМАНИЕ
				ДокОбъект = НайтиВСДпоUUID( _uuid, "ВСД" );
			Иначе
...
// НЗП
	...
	Запрос=Запрос+"		
		...
		Если ЗначениеЗаполнено(ДокСсылка.НомерПартии) Тогда
			...
			Исключение СообщитьИнфо("Обновите конфигурацию!!!"); КонецПопытки;
		КонецЕсли;
		Запрос=Запрос+"
...
Функция ВСД2_Производство_Отправить(Параметры, ДокСсылка) Экспорт
...
//        	|            <bs:uuid>a3cecd19-98ca-46f9-ecd5-90c41fd95e13</bs:uuid>
			...
			КонецЕсли;
			Запрос = Запрос + "
        	|            <dt:name>" + СокрЛП(ДокСсылка.Продукция_Элемент.Наименование) + "</dt:name>";
...
Функция ВСД2_инвентаризация_ЗапросXML( Параметры, ДокСсылка)
	...
	Для каждого СтрПартии из тзПартии Цикл
		Запрос = Запрос + "	
		|                <vd:stockEntry>";
	    Если ЗначениеЗаполнено(СтрПартии.Партия) Тогда // изменяем конкретную партию
			Запрос = Запрос + "
...
//		|                    <vd:batchID>123</vd:batchID>
        ...
        // Добавляем Партию
//		Если (ПустаяСтрока(СтрПартии.Партия) = 1) и (ПустаяСтрока(СтрПартии.Производитель_Площадка.GUID) = 0) и (ПустаяСтрока(СтрПартии.Производитель_Страна) = 0)Тогда
			Запрос=Запрос+"
			|                    <vd:origin>";
...
Функция ВСД2_Аннулировать(Знач Параметры, ДокСсылка ) Экспорт
	...
	//Если СокрЛП(ДокСсылка.Статус)= "WITHDRAWN" Тогда
	//	докОбъект = ДокСсылка.ПолучитьОбъект();
...
Процедура ВСД2_транзакция_Аннулировать( Знач Параметры, ВСД2_Транзакция_Ссылка ) Экспорт
	...
	Для Каждого строкаВСД из тзВСД Цикл
		Если СокрЛП(строкаВСД.Док.ТипВСД) = "PRODUCTIVE" Тогда
			...
			Продолжить;	
		КонецЕсли;
		Ответ = Ответ и ВСД2_Аннулировать(Параметры, строкаВСД.Док);
	КонецЦикла;
...
Запрос = "
	...
	Если (типЗнч(ДокСсылка) = тип("ДокументСсылка.ВСД2_входящий")) Тогда          //(типЗнч(Док) = тип("ДокументСсылка.ВСД2_исходящий")) или 
			Запрос = Запрос+"
			|            <dt:cargoType>
...
//Функция ПроверитьВозможностьПеремещения(ДокСсылка) Экспорт
Функция ВСД2_Транзакция_УсловияПеревозки_Отправить(Знач Параметры, ДокСсылка) Экспорт

	Организация = ДокСсылка.Организация;//док.ДокументОснование.Организация;
	//Инициализация();
	//
	//Отправитель_Площадка = ДокСсылка.Отправитель_Площадка;
	//
...
// Артикул должен быть уникален в пределах одного производителя 	<dt:code>12</dt:code> 	
	...
	Попытка
		Запрос = Запрос + "
		|              <dt:name>"+ СокрЛП(ДокСсылка.НаименованиеПродукции) +"</dt:name>";
	Исключение
		Запрос = Запрос + "
		|              <dt:name>"+ СокрЛП(ДокСсылка.Продукция_Элемент.Наименование) +"</dt:name>";
	КонецПопытки;
...
// сведения о фасовке
	...
	Запрос = Запрос + "
			...
			|            </vd:unit>";
		Если (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления1)) или (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления2)) или (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления)) Тогда
			Запрос = Запрос + "
        				|                <vd:dateOfProduction>"; 

			Если (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления1)) Тогда
				Запрос = Запрос + "
				|              <vd:firstDate>
				|                <dt:year>"+ДатаГод(ДокСсылка.ДатаИзготовления1)+"</dt:year>
				|                <dt:month>"+ ДатаМесяц(ДокСсылка.ДатаИзготовления1) +"</dt:month>
				|                <dt:day>"+ ДатаЧисло(ДокСсылка.ДатаИзготовления1) +"</dt:day>
				|                <dt:hour>0</dt:hour>
				|              </vd:firstDate>";
			КонецЕсли;
			Если ((ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления2)) и 
					(ДокСсылка.ДатаИзготовления2 > ДокСсылка.ДатаИзготовления1)) Тогда
				Запрос = Запрос + "
				|              <vd:secondDate>
				|                <dt:year>"+ ДатаГод(ДокСсылка.ДатаИзготовления2) +"</dt:year>
				|                <dt:month>"+ ДатаМесяц(ДокСсылка.ДатаИзготовления2) +"</dt:month>
				|                <dt:day>"+ ДатаЧисло(ДокСсылка.ДатаИзготовления2) +"</dt:day>
				|                <dt:hour>0</dt:hour>
				|              </vd:secondDate>";
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления) Тогда
				Запрос = Запрос + "
				|              <vd:informalDate>"+ СокрЛП(ДокСсылка.ДатаИзготовления) +"</vd:informalDate>";
			КонецЕсли;
			Запрос = Запрос + "
			|                </vd:dateOfProduction>"; 
		КонецЕсли;
		
		Если (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности1) ) или (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности2) ) или (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности)) Тогда
			Запрос = Запрос + "
				|                <vd:expiryDate>";
			Если (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности1)) Тогда
				Запрос = Запрос + "
				|              <vd:firstDate>
				|                <dt:year>"+ ДатаГод(ДокСсылка.ДатаСрокГодности1) +"</dt:year>
				|                <dt:month>"+ ДатаМесяц(ДокСсылка.ДатаСрокГодности1) +"</dt:month>
				|                <dt:day>"+ ДатаЧисло(ДокСсылка.ДатаСрокГодности1) +"</dt:day>
				|                <dt:hour>0</dt:hour>
				|              </vd:firstDate>";
			КонецЕсли;
			Если ((ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности2)) и 
					(ДокСсылка.ДатаСрокГодности2 > ДокСсылка.ДатаСрокГодности1)) Тогда
				Запрос = Запрос + "
				|              <vd:secondDate>
				|                <dt:year>"+ ДатаГод(ДокСсылка.ДатаСрокГодности2) +"</dt:year>
				|                <dt:month>"+ ДатаМесяц(ДокСсылка.ДатаСрокГодности2) +"</dt:month>
				|                <dt:day>"+ ДатаЧисло(ДокСсылка.ДатаСрокГодности2) +"</dt:day>
				|                <dt:hour>0</dt:hour>
				|              </vd:secondDate>";
			КонецЕсли; 
			Если ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности) Тогда
				Запрос = Запрос + "
				|              <vd:informalDate>"+ СокрЛП(ДокСсылка.ДатаСрокГодности) +"</vd:informalDate>";
			КонецЕсли;
	...
	Запрос = Запрос + "
		...
		Если (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления1)) или (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления2)) или (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления)) Тогда
			...
			Если (ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления1)) Тогда
				...
				Если ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления1СЗ) Тогда
					сзДаты = ЗначениеИзСтрокиВнутр(ДокСсылка.ДатаИзготовления1СЗ);
			...
			Если ((ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления2)) и 
				...
				Если ЗначениеЗаполнено(ДокСсылка.ДатаИзготовления2СЗ) Тогда
					сзДаты = ЗначениеИзСтрокиВнутр(ДокСсылка.ДатаИзготовления2СЗ);
					для каждого элементсз из сзДаты Цикл
						Запрос = Запрос + "
						|"+элементсз.значение;
					КонецЦикла;
		...
		Если (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности1) ) или (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности2) ) или (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности)) Тогда
			...
			Если (ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности1)) Тогда
				...
				Если ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности1СЗ) Тогда
					сзДаты = ЗначениеИзСтрокиВнутр(ДокСсылка.ДатаСрокГодности1СЗ);
					для каждого элементсз из сзДаты Цикл
						Запрос = Запрос + "
						|"+элементсз.значение;
					КонецЦикла;
			...
			Если ((ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности2)) и 
				...
				Запрос = Запрос + "
				|              <vd:secondDate>";
				Если ЗначениеЗаполнено(ДокСсылка.ДатаСрокГодности2СЗ) Тогда
					сзДаты = ЗначениеИзСтрокиВнутр(ДокСсылка.ДатаСрокГодности2СЗ);
					для каждого элементсз из сзДаты Цикл
						Запрос = Запрос + "
						|"+элементсз.значение;
					КонецЦикла;
...
Функция ВСД2_Входящий_ОтправитьГашение(Параметры, ДокСсылка) Экспорт
	...
	Ответ = ВСД2_Входящий_ГашениеПолучитьОтвет( Параметры, appID, ДокСсылка );	
	Для А=1 По 10 Цикл
		//Если НЕ Ответ Тогда 
		Если (Ответ="IN_PROCESS") Тогда
			Ответ =  ВСД2_Входящий_ГашениеПолучитьОтвет( Параметры, appID, ДокСсылка );	
...
//Функция ПолучитьРезультат_ГашениеВСД2(Параметры, applicationID, докСсылка = Неопределено ) Экспорт
Функция ВСД2_Входящий_ГашениеПолучитьОтвет(Параметры, appID, докСсылка ) Экспорт
