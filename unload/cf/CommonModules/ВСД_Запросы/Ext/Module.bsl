Процедура СообщитьИнфо(ТекстСообщения, Ссылка = Неопределено)  Экспорт
	...
	ВСД.СообщитьПользователю(ТекстСообщения, Ссылка,,,Отказ);
	//Если Отказ Тогда 
...
//Функция Дата_из_СписокЗначений_В_XML( сзДатаВнутрСтрока ) 
//	сзДата = ЗначениеИзСтрокиВнутр( сзДатаВнутрСтрока );
//	
//	Ответ="";
//	Для А=1 По сзДата.Количество() Цикл
//		Ответ = Ответ + сзДата.Получить(А);
//	КонецЦикла;		
...
Функция Преобразовать_СтрокаДата_в_XML(ДатаСтр) Экспорт
	...
	Если ЗначениеЗаполнено( час1 ) Тогда
		...
		Исключение
			сообщитьИнфо(ДатаСтр+" "+ОписаниеОшибки(),);
		КонецПопытки; 
	КонецЕсли;
...
Функция Преобразовать_ДатаXML_в_Строку( dateXML )
	попытка
		...
		//год = Формат( год, "ЧЦ=4; ЧДЦ=0; ЧН=' '; ЧВН=; ЧГ=" );
	Исключение
		год ="    ";
	КонецПопытки;
	попытка
...
//	Попытка	Некачественный = batch.lowGradeCargo; Исключение	КонецПопытки;
	...
	лабы=неопределено;
	Попытка
		лабы=stockEntry.vetEventList.laboratoryResearch;
		попытка 
			размерТаблицы=лабы.количество();
			ПартияЛабы=Партия.ЛабораторныеИсследования;
			ПартияЛабы.Очистить();
			для сч=0 по РазмерТаблицы-1 цикл
				строкалаб=лабы[сч];
				сообщить(Партия.Наименование);
				...
				попытка новстр.РезультатИсследования=Строкалаб.result; Исключение КонецПопытки;
				попытка новстр.ДатаРезультата=строкалаб.actualDateTime;  Исключение КонецПопытки;
				//строкалаб.referencedDocument.type
				попытка новстр.НомерАктаОтбораПроб=строкалаб.referencedDocument.issueNumber; Исключение КонецПопытки;
				попытка новстр.ДатаОтбораПроб=строкалаб.referencedDocument.issueDate; Исключение КонецПопытки;
			конеццикла;
		исключение
			если типзнч(лабы)=тип("ОбъектXDTO") тогда
				ПартияЛабы=Партия.ЛабораторныеИсследования;
				ПартияЛабы.Очистить();
				сообщить(Партия.Наименование);
				новстр=партиялабы.добавить();
				попытка новстр.наименованиеЛаборатории = лабы.operator.name;  Исключение КонецПопытки;
				попытка новстр.НомерЭкспертизы=лабы.expertiseID; Исключение КонецПопытки;
				попытка новстр.заключение  =лабы.conclusion;  Исключение КонецПопытки;
				попытка новстр.НаименованиеПоказателя = лабы.indicator.name; Исключение КонецПопытки;
				попытка новстр.методИсследования=лабы.method.name;  Исключение КонецПопытки;
				попытка новстр.РезультатИсследования=лабы.result;  Исключение КонецПопытки;
				попытка новстр.ДатаРезультата=лабы.actualDateTime; Исключение КонецПопытки;
				попытка новстр.НомерАктаОтбораПроб=лабы.referencedDocument.issueNumber; Исключение КонецПопытки;
				попытка	новстр.ДатаОтбораПроб=лабы.referencedDocument.issueDate; Исключение КонецПопытки;
			конецесли;
		конецпопытки;
	исключение
	конецпопытки;
...
Процедура ОтправитьВСДвГИС(Знач Параметры, АдресХранилища) Экспорт
	...
	//ИначеЕсли ЗначениеЗаполнено( СписокДокументов ) Тогда 
...
//	Если ОбъектXDTO.Установлено(Свойство) Тогда	
	...
	Для каждого vetDocument из vetDocumentList Цикл
		...
		//Если НЕ ЗначениеЗаполнено(ВсдСтруктура) Тогда 
			...
			//|ДатаИзготовления1СЗ, ДатаИзготовления2СЗ, ДатаСрокГодности1СЗ, ДатаСрокГодности2СЗ,
...
//					КонецЕсли;
		...
		//ДатаИзготовления1Строка="";	ДатаИзготовления2Сстрока="";
		...
		КонецПопытки;		
//		Попытка
//			ВсдСтруктура.ДатаИзготовления1Строка = ДатаИзготовления1СЗ;	
//			ВсдСтруктура.ДатаИзготовления2Строка = ДатаИзготовления2СЗ;
		...
		Если СоздаватьДокументы Тогда
			...
			Если ЗначениеЗаполнено( МаршрутСледования ) Тогда 
				...
				Для Каждого ТочкаПерегрузки Из МаршрутСледования Цикл
					...
					//НоваяСтрока.ТипТранспорта = Метаданные.Перечисления["ВСД_ТипыТранспорта"].ЗначенияПеречисления[(ТочкаПерегрузки.ТипТранспорта-1)]
...
Процедура ВСД2_транзакция_Аннулировать( Знач Параметры, ВСД2_Транзакция_Ссылка ) Экспорт
	...
	//Если Ответ Тогда
		...
		докОбъект.Статус = "Аннулирован"; // Статус;
		докОбъект.Записать( РежимЗаписиДокумента.ОтменаПроведения, );
