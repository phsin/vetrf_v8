...
Функция Площадка_ЗагрузитьПоGUID(Знач Параметры, _guid, ХозСубъект=Неопределено, Ссылка_ВСД_Площадка=Неопределено) Экспорт
	...
	Попытка	    
		...
		СообщитьИнфо("Записана ВСД_Площадка ["+Объект_ВСД_Площадка.Наименование+"] GUID = "+ _guid);		
		//ЗафиксироватьТранзакцию();
...
Функция Площадки_ЗагрузитьПоУсловиям(Параметры, СписокУсловий) Экспорт
	...
	Пока ПолученоЗаписей = 1000 Цикл
		...
		xdto = ВСД_Запросы.Площадки_НайтиПоУсловиям( Параметры, СписокУсловий );	
		Если xdto = Неопределено Тогда
			Возврат Площадки;	
		КонецЕсли;
		//Попытка
...
//Площадки = Новый ТаблицаЗначений;
//Площадки.Колонки.Добавить("Отдел",,"Отдел");
//Площадки.Колонки.Добавить("Сотрудник",,"Фамилия сотрудника");
//Площадки.Колонки.Добавить("Оклад",,"Оклад");	

			ПолученоЗаписей = enterpriseList.Количество();
			Для каждого enterprise из enterpriseList Цикл		
											
				//ЭлементПлощадка = Площадки.Добавить();
				...
				//ЖД  Если есть owner то его ГУИД и пишем
				Попытка 
				...
				КонецПопытки;  			
				//ЭлементПлощадка.GUIDХозСубъекта = ГУИДХС;
...
Функция ПолучитьРезультат_ВСД2(Параметры, appID, докСсылка = Неопределено, СделатьПаузу = Истина, ЗаписыватьДокументы=Истина, ВсдРезультат = Неопределено) Экспорт
	...
	Статус = СтатусЗапроса(xdto, докСсылка);
	Если НайтиОшибки(xdto, докСсылка) Тогда
		Возврат Статус;
	КонецЕсли;
...
Процедура ВСД2_транзакция_Аннулировать( Знач Параметры, ВСД2_Транзакция_Ссылка ) Экспорт
	...
	//Если Ответ Тогда
		...
		докОбъект.Статус = Статус;//"Аннулирован";
		докОбъект.Записать( РежимЗаписиДокумента.ОтменаПроведения, );
		докОбъект.УстановитьПометкуУдаления(Истина);
	//КонецЕсли;
КонецПроцедуры
...
Функция ВСД2_Входящий_ОтправитьГашение(Параметры, ДокСсылка) Экспорт
	...
	Если ТипЗНЧ(ДокСсылка) = ТИП("ДокументСсылка.ВСД2_входящий") Тогда
		Ответ = СтатусЗапроса(xdto, докСсылка);
	КонецЕсли;
