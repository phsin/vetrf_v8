...
Процедура СохранитьСтатусЗапроса( xdto, ДокСсылка )
	Если ЗначениеЗаполнено( ДокСсылка ) Тогда
		Попытка
			...
			ДокОбъект.Статус 		= Status;						
			//ДокОбъект.Комментарий 	= ВсеОшибки;
			ДокОбъект.Записать();
			
			СообщитьИнфо(""+ДокСсылка+" статус = "+Status, ДокСсылка);
		Исключение
...
Функция ЗагрузитьВидыПродукции_ЗапросXML( _guid, _Смещение = 0 )
	Запрос = "
	|<soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/'
	|                  xmlns:ws='http://api.vetrf.ru/schema/cdm/argus/production/ws-definitions'
	|                  xmlns:base='http://api.vetrf.ru/schema/cdm/base'
...
Функция Продукция_Элемент_Найти( Знач Параметры, productItem, ОбновитьДанные=ложь ) 
	...
	Если Результат.Следующий() Тогда 
		Ответ = Результат.Ссылка;
	Иначе
		СообщитьИнфо("ВСД_Продукция_Элемент Guid "+productItemGuid+" не найдена. Создан новый элемент");
...
Процедура Площадки_ЗагрузитьСписокПоХозСубъекту(Знач Параметры, тзСвертка )  Экспорт // !!!! 
	...
	Для Каждого стр Из тзСвертка Цикл
		Если стр.Отметка Тогда 
			...
			//	ХозСубъект_ПолучитьGuidПоИНН(Параметры, стр.ХозСубъект);
			//КонецЕсли;
			Параметры.Вставить("ВыбХозСубъект", стр.ХозСубъект );
			Площадка_ЗагрузитьПоХозСубъекту(Параметры, стр.ХозСубъект);							
...
Функция ПолучитьПартии2_ЗапросXML( Параметры )
	...
	|            <bs:listOptions>
	|              <bs:count>1000</bs:count>
	|              <bs:offset>"+ Формат(ПартииСмещение, "ЧДЦ=0; ЧН=0; ЧГ=") +"</bs:offset>
	|            </bs:listOptions>
...
Процедура Партии_Ответ_ЗагрузитьXML(Параметры, xdto )
	...
	СообщитьИнфо("Загрузка партий завершена");
...
Функция  ПолучитьСписокВСД( Знач Параметры, ДокСсылка, ВыбПлощадка, vetDocumentType	= "INCOMING", vetDocumentStatus = "CONFIRMED", Смещение,ПоступилиС = "", ПоступилиПо ="",ОтХС = "", ОтПлощадки="") Экспорт
	...
	ЗапросXML = ПолучитьСписокВСД2_ЗапросXML(СокрЛП(ВыбПлощадка.GUID), vetDocumentType, vetDocumentStatus, Смещение,ПоступилиС , ПоступилиПо,ОтХСГУИД, ОтПлощадкиГУИД);	
...
Функция ПолучитьРезультат_ВСД2(Параметры, appID, докСсылка, СделатьПаузу = Истина ) Экспорт
	Если ПустаяСтрока(appID) Тогда
		СообщитьИнфо("Пустая ссылка applicationID");
		Возврат ложь;
	КонецЕсли;
...
//	Результат = КомпонентаНаСервере.SendRequestResult( applicationID );
	...
	СохранитьСтатусЗапроса(xdto, докСсылка);
	...
	НачатьТранзакцию();
	Попытка
		Ответ = ЗагрузитьXML_ВСД2( Параметры, xdto, докСсылка);
...
Функция ЗагрузитьXML_ВСД2(Знач Параметры, xdto, докОснование = Неопределено) Экспорт
...
//	Если ОбъектXDTO.Установлено(Свойство) Тогда	
	...
	Для каждого vetDocument из vetDocumentList Цикл
		...
		Попытка	докОбъект.Организация = ДокОснование.Организация; Исключение КонецПопытки;
...
//		ВидДокумента = ВыбратьВидДокументаВСД(type);
		...
		Если ЭтоТранспортныйВСД = 1 Тогда
			Попытка докОбъект.Отправитель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.consignor.businessEntity.guid); Исключение докОбъект.Отправитель_ХозСубъект = ""; КонецПопытки;
			Попытка докОбъект.Отправитель_Площадка 	= Площадка_НайтиПоGUID(Параметры, vetDocument.certifiedConsignment.consignor.enterprise.guid, докОбъект.Отправитель_ХозСубъект); Исключение КонецПопытки;
			Попытка докОбъект.Получатель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.consignee.businessEntity.guid); Исключение докОбъект.Получатель_ХозСубъект = ""; КонецПопытки;
			Попытка докОбъект.Получатель_Площадка 	= Площадка_НайтиПоGUID(Параметры, vetDocument.certifiedConsignment.consignee.enterprise.guid, докОбъект.Получатель_ХозСубъект); Исключение КонецПопытки;
			Попытка докОбъект.ВладелецХС = ХозСубъект_НайтиПоGUID( Параметры, batch.owner.guid);	Исключение КонецПопытки;
			Попытка
				Если НЕ(ЗначениеЗаполнено(докОбъект.ВладелецХС)) Тогда
					докОбъект.ВладелецХС = докОбъект.Отправитель_ХозСубъект;	
				КонецЕсли;
			Исключение КонецПопытки;
	        Попытка докОбъект.Перевозчик_ХозСубъект 	= ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.broker.guid); Исключение КонецПопытки;
	...
	//			Иначеесли ТЗВСД.ТипТС = "7" Тогда
			...
			Попытка докОбъект.ТермическиеУсловияПеревозки = Перечисления.ВСД_ТермическиеУсловияПеревозки[ vetDocument.certifiedConsignment.transportStorageType ]; 	Исключение 	КонецПопытки;
...
//					КонецЕсли;
		...
		докОбъект.Продукция_Элемент = Продукция_Элемент_Найти( Параметры, batch.productItem );
...
//			докОбъект.ДатаСрокГодности2СЗ = "";
		...
		Если НЕ ЭтоТЗ Тогда
			Если ДокОбъект.Статус = "WITHDRAWN" Тогда 
