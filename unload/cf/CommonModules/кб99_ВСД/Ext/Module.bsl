				...
				Для А=НомерСтроки По тзПартий.Количество()-1 Цикл
					...
					Если тзПартий[А].Партия.ИсключенаИзПодбора Тогда
						Если ПараметрыФункции["ВыводитьПодробнуюИнформацию"] Тогда
							СообщитьИнфо(СтрШаблон("Партия [%1], с номером записи [%2] была исключена из подбора", тзПартий[А].Партия.Наименование, тзПартий[А].Партия.НомерЗаписи), тзПартий[А].Партия);
						КонецЕсли;
						Продолжить;
					КонецЕсли;
...
Функция ПолучитьПутьКПриложениюУдаленнойПоддержки() Экспорт
	...
	СсылкиНаПрограммыУдаленнойПоддержки.Вставить("_1cConnect", "https://1c-connect.com/download-desktop/");
