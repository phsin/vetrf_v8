...
Процедура ОтправитьВСДвГИС(Знач ДокСсылка) Экспорт
	...
	Попытка
		Если типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_транзакция") Тогда
			...
			кб99_ВСД_Запросы.ВСД2_транзакция_Отправить( ПараметрыФункции );
		...
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Инвентаризация") Тогда
			...
			Параметры.Вставить("СписокПартий", Новый Массив);
			кб99_ВСД_Запросы.ВСД2_Инвентаризация_Отправить( ПараметрыФункции );
			
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_ОбъединениеПартий") Тогда
			
			Параметры["Отправитель_Площадка"] = ДокСсылка.Владелец_Площадка;
			Параметры.Вставить("СписокПартий", Новый Массив);
			кб99_ВСД_Запросы.ВСД2_ОбъединениеПартий_Отправить( ПараметрыФункции );
		...
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Производство") Тогда
			...
			Параметры.Вставить("СписокПартий", Новый Массив);
			кб99_ВСД_Запросы.ВСД2_Производство_Отправить( ПараметрыФункции );
		...
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_ЛабораторныеИсследования") Тогда
			...
			кб99_ВСД_Запросы.ВСД2_ЛабораторныеИсследования_Отправить( ПараметрыФункции );
			
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_ОбновитьТранспорт") Тогда
			
			кб99_ВСД_Запросы.ВСД2_ОбновитьТранспорт_Отправить( ПараметрыФункции );
...
Процедура ОтправитьВСД_Параллельно(Знач Параметры, АдресХранилища=Неопределено) Экспорт
	...
	Для Каждого ДокСсылка Из СписокДокументов Цикл	
		Если типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_транзакция") Тогда
			...
			Статус = кб99_ВСД_Запросы.ВСД2_транзакция_Отправить(ПараметрыОбщие, ДокСсылка, Неопределено, Ложь);
			
		ИначеЕсли типЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Производство") тогда
			
			Статус = кб99_ВСД_Запросы.ВСД2_Производство_Отправить(ПараметрыОбщие, ДокСсылка, Ложь );
	...
	Для Каждого ДокСсылка Из СписокДокументовПолучитьОтвет Цикл
		applicationID = кб99_ВСД_Общий.НайтиApplicationID( ДокСсылка );
		Статус =  кб99_ВСД_Запросы.ПолучитьРезультат_ВСД2( Параметры,, applicationID, ДокСсылка, Ложь );	
		Для А=1 По 10 Цикл
			//Если (Статус="IN_PROCESS") Тогда
			Если Статус = Перечисления.кб99_СтатусЗапроса.IN_PROCESS Тогда 
				Статус =  кб99_ВСД_Запросы.ПолучитьРезультат_ВСД2( Параметры,, applicationID, ДокСсылка, Истина );	
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;	
