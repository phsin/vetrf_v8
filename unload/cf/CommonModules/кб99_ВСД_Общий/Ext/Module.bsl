...
Функция ПараметрыСервер( ФОбъект ) Экспорт
	...
	Параметры.Вставить("НомерАвто", ФОбъект.НомерАвто);
	Параметры.Вставить("ПЭ_Сливки", ФОбъект.ПЭ_Сливки);
	Параметры.Вставить("ПЭ_Обрат", ФОбъект.ПЭ_Обрат);
	Параметры.Вставить("ЖирностьМолокаПроцент", ФОбъект.ЖирностьМолокаПроцент);
	Параметры.Вставить("ЖирностьСливокПроцент", ФОбъект.ЖирностьСливокПроцент);	
	Параметры.Вставить("ПроизводствоРазбиватьПострочно", ФОбъект.ПроизводствоРазбиватьПострочно);	
...
Функция ВыгрузитьТч( Док, ПараметрыОрганизации="" ) Экспорт
	...
	Иначе
		тзОснование = Док.Товары.Выгрузить();	
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыСтрокиВСД( ПараметрыОрганизации, НоваяСтрока, СтрокаОснования, ДокСсылка ) Экспорт
	...
	Если кб99_ВСД.ЕстьТабличныеЧастиОбъекта("УровниУпаковки", докСсылка) Тогда 
...
Функция СформироватьЗапросПеремещения( ПараметрыФункции ) Экспорт
	...
	|	И ПеремещениеТовары.Ссылка.СкладОтправитель В ИЕРАРХИИ(&ПлощадкаОтправителяСклад)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	...
	|	&Отправитель_Хозсубъект КАК Отправитель_Хозсубъект,
