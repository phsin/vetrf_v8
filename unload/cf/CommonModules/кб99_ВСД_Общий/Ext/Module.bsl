...
Функция ПараметрыСервер( ФОбъект ) Экспорт
	...
	Параметры.Вставить("ДобавлятьОсобыеОтметкиИзВходящегоВСД", ФОбъект.ЗаполнятьТермическиеУсловияИзВходящегоВСД);
...
Функция ЗаполнитьТабЧастьВСД( ПараметрыОрганизации, ДанныеЗаполнения = Неопределено, докСсылка, тзАктуальныхПартий = Неопределено, ВыбТермУсловиеПеревозки = Неопределено) Экспорт
	...
	Если ПараметрыФункции["ПропускатьПустыеСвойства"] Тогда
		ТекстОтбор = "
			...
			|	(ВЫБРАТЬ
				...
				|	ДокСтроки1.@КоличествоМест@ КАК КоличествоМест,    
...
Функция СформироватьЗапросПеремещения( ПараметрыФункции ) Экспорт
	...
	Если ПараметрыФункции["ПропускатьПустыеСвойства"] Тогда
		...
		|	ЗапросТовары.ПродукцияЭлемент КАК ПродукцияЭлемент,				
		|	(ДокСтроки1.@Количество@ * @КоэффициентПересчета@ ) КАК @Количество@,
		|	ДокСтроки1.СуммаВРознице КАК Сумма
		|ИЗ
		|	Документ.ПеремещениеТоваров.Товары КАК ДокСтроки1
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			Спр1.ПродукцияЭлемент КАК ПродукцияЭлемент,
		|			Спр1.Владелец КАК Номенклатура
		|		ИЗ
