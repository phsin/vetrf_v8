...
Функция ПараметрыСервер( ФОбъект ) Экспорт
	...
	Параметры.Вставить("КонтролироватьДокументПроисхожденияПродукции", ФОбъект.КонтролироватьДокументПроисхожденияПродукции);
	Параметры.Вставить("КонтролироватьПравильностьЗаполненияНомераАвтомобиля", ФОбъект.КонтролироватьПравильностьЗаполненияНомераАвтомобиля);
	Параметры.Вставить("КонтролироватьОстаткиПартийПоРегистру", ФОбъект.КонтролироватьОстаткиПартийПоРегистру);
	Параметры.Вставить("ДатаНачалаКонтроляОстатковПартийПоРегистру", ФОбъект.ДатаНачалаКонтроляОстатковПартийПоРегистру);
	Параметры.Вставить("СвойКонтрагент", ФОбъект.СвойКонтрагент);
...
// Заполняет в документ новые партии, добавленные этим документом(производство, инвентаризация)
//Процедура ЗаполнитьНовыеПартииВТЧДокумента(СписокПартий, ДокументОбъект) Экспорт
//	
...
//	
//	ДокументОбъект.ПартииРезультат.Загрузить(тзПартии);
//	
//КонецПроцедуры
...
Функция ПолучитьТзРеализаций( ПараметрыФункции ) Экспорт
	...
	|    @Отбор_по_Складам@
	|    @Маршрут_Условие@
	|	И РеализацияТоваровУслуг.Контрагент <> &НашКонтрагент
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если ЗначениеЗаполнено(ПараметрыФункции["СвойКонтрагент"]) Тогда
		Запрос.УстановитьПараметр("НашКонтрагент", ПараметрыФункции["СвойКонтрагент"]);
	Иначе	
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И РеализацияТоваровУслуг.Контрагент <> &НашКонтрагент", "");
	КонецЕсли;
...
Процедура ГашениеПроверитьСтрокуТаблицы( СтрокаВСДВходящие, ПараметрыОрганизации ) Экспорт
	...
	// В меркурии контролируется дата срока годности 1
...
Функция ПроверитьЗначениеПараметра(ПараметрыОрганизации, НазваниеПараметра, ОписаниеПараметра="" ) Экспорт
	...
	Иначе		
		...
		кб99_ВСД.СообщитьПользователю( СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Не указано значение параметра %1. Откройте параметры и нажмите кнопку [Сохранить]", ОписаниеПараметра ));
		Возврат Ложь;
...
// Обработка проведения документов с табличной частью "ПартииРезультат"
//Процедура ОбработкаПроведенияПоРегиструПартийСКонтролем(Отказ, Документ) Экспорт
//		
//	Если ФормироватьДвиженияПартий(Документ.Организация) Тогда
//		
//		Документ.Движения.кб99_ОстаткиПартийВСД.Записывать = Истина;
//		//Документ.Движения.кб99_ОстаткиПартийВетис.Записывать = Истина;
...
//			|	СписокПартий.Количество КАК Количество,
//			|	ВЫБОР
//			|		КОГДА НЕ ЕСТЬNULL(кб99_ЗапросыСрезПоследних.СтатусЗапроса, """") = ""Завершен""
//			|			ТОГДА ИСТИНА
//			|		ИНАЧЕ ЛОЖЬ
//			|	КОНЕЦ КАК ЕстьОшибкаДокументНеОбработан
//			|ПОМЕСТИТЬ втПартии
//			|ИЗ
//			|	Документ."+ИмяДокумента+".ПартииРезультат КАК СписокПартий
//			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.кб99_Запросы.СрезПоследних КАК кб99_ЗапросыСрезПоследних
//			|		ПО СписокПартий.Ссылка = кб99_ЗапросыСрезПоследних.Объект
//			|ГДЕ
//			|	СписокПартий.Ссылка = &Ссылка
//			|
//			|ИНДЕКСИРОВАТЬ ПО
//			|	Партия
//			|;
...
//			|	ЕСТЬNULL(кб99_ОстаткиПартийВСДОстатки.КоличествоОстаток, 0) КАК КоличествоОстаток,
//			|	втПартии.ЕстьОшибкаДокументНеОбработан КАК ЕстьОшибкаДокументНеОбработан
//			|ИЗ
//			|	втПартии КАК втПартии
//			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.кб99_ОстаткиПартийВСД.Остатки(
//			|				&МоментВремени,
//			|				Партия В
//			|					(ВЫБРАТЬ
//			|						втПартии.Партия КАК Партия
//			|					ИЗ
//			|						втПартии КАК втПартии)) КАК кб99_ОстаткиПартийВСДОстатки
//			|		ПО втПартии.Партия = кб99_ОстаткиПартийВСДОстатки.Партия";
//		
...
//		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
//			
//			Если ВыборкаДетальныеЗаписи.ЕстьОшибкаДокументНеОбработан И Не ИмяДокумента = "ПартииВСД" Тогда
//				Отказ = Истина;
//				кб99_ВСД.СообщитьИнфо("Документ не обработан. Проведение запрещено");
//				Возврат;	
//			КонецЕсли;
//			
//			//Движение			 = Документ.Движения.кб99_ОстаткиПартийВетис.Добавить();
...
//			КонецЕсли;
//			
//			Количество = ?(ВыборкаДетальныеЗаписи.Количество - ВыборкаДетальныеЗаписи.КоличествоОстаток < 0, 
//					     -(ВыборкаДетальныеЗаписи.Количество - ВыборкаДетальныеЗаписи.КоличествоОстаток),
//						   ВыборкаДетальныеЗаписи.Количество - ВыборкаДетальныеЗаписи.КоличествоОстаток);
//			
//			Движение			 = Документ.Движения.кб99_ОстаткиПартийВСД.Добавить();
...
// Движения на основании записей регистра сведений
//Процедура ОбработкаПроведенияПоРегиструПартийСКонтролем(Отказ, Документ) Экспорт
...
//		               |	кб99_ОстаткиПартийВетисСрезПоследних.Количество КАК КоличествоРезультат,
//		               |	ВЫБОР
//		               |		КОГДА НЕ ЕСТЬNULL(кб99_ЗапросыСрезПоследних.СтатусЗапроса, """") = ""Завершен""
//		               |			ТОГДА ИСТИНА
//		               |		ИНАЧЕ ЛОЖЬ
//		               |	КОНЕЦ КАК ЕстьОшибкаДокументНеОбработан
//		               |ПОМЕСТИТЬ втПартииПоДокументу
//		               |ИЗ
//		               |	РегистрСведений.кб99_ОстаткиПартийВетис.СрезПоследних(, ДокументДвижения = &ДокументДвижения) КАК кб99_ОстаткиПартийВетисСрезПоследних
//		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.кб99_Запросы.СрезПоследних КАК кб99_ЗапросыСрезПоследних
//		               |		ПО кб99_ОстаткиПартийВетисСрезПоследних.ДокументДвижения = кб99_ЗапросыСрезПоследних.Объект
//		               |;
//		               |
//		               |////////////////////////////////////////////////////////////////////////////////
//		               |ВЫБРАТЬ
//		               |	втПартииПоДокументу.Партия КАК Партия,
//		               |	втПартииПоДокументу.КоличествоРезультат КАК КоличествоРезультат,
//		               |	ЕстьNULL(кб99_ОстаткиПартийВетисСрезПоследних.Количество,0) КАК КоличествоОстаток,
//		               |	втПартииПоДокументу.ЕстьОшибкаДокументНеОбработан КАК ЕстьОшибкаДокументНеОбработан
//		               |ИЗ
//		               |	втПартииПоДокументу КАК втПартииПоДокументу
//		               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.кб99_ОстаткиПартийВетис.СрезПоследних(&ДатаПолученияОстатков, ) КАК кб99_ОстаткиПартийВетисСрезПоследних
//		               |		ПО втПартииПоДокументу.Партия = кб99_ОстаткиПартийВетисСрезПоследних.Партия";
//		Запрос.УстановитьПараметр("ДокументДвижения", Документ.Ссылка);
//		Запрос.УстановитьПараметр("ДатаПолученияОстатков", Новый Граница(Документ.МоментВремени(), ВидГраницы.Исключая));
//		
//		РезультатЗапроса = Запрос.Выполнить();
...
//			КонецЕсли;
//			
//			Если ВыборкаДетальныеЗаписи.КоличествоРезультат > ВыборкаДетальныеЗаписи.КоличествоОстаток Тогда
//				ВидДвижения = ВидДвиженияНакопления.Приход;
//			ИначеЕсли ВыборкаДетальныеЗаписи.КоличествоРезультат < ВыборкаДетальныеЗаписи.КоличествоОстаток Тогда
//				ВидДвижения = ВидДвиженияНакопления.Расход;
//			ИначеЕсли ВыборкаДетальныеЗаписи.КоличествоРезультат = ВыборкаДетальныеЗаписи.КоличествоОстаток Тогда
//				Продолжить;
//			КонецЕсли;
//			
//			Количество = ?(ВыборкаДетальныеЗаписи.КоличествоРезультат - ВыборкаДетальныеЗаписи.КоличествоОстаток < 0, 
//							-(ВыборкаДетальныеЗаписи.КоличествоРезультат - ВыборкаДетальныеЗаписи.КоличествоОстаток),
//							ВыборкаДетальныеЗаписи.КоличествоРезультат - ВыборкаДетальныеЗаписи.КоличествоОстаток);
//			
//			Движение			 = Документ.Движения.кб99_ОстаткиПартийВСД.Добавить();
//			Движение.ВидДвижения = ВидДвижения;
