...
Функция ПогаситьВсдОтИмениПолучателя( ДокСсылка ) Экспорт
	...
	//Если НЕ ЗначениеЗаполнено( ДокСсылка.ОрганизацияПолучатель ) Тогда 
	//	кб99_ВСД.СообщитьИнфо("Не заполнен реквизит [Оранизация получатель], гашение невозможно");
	//	возврат Ложь;
	//КонецЕсли;
...
Функция ПогаситьВсдОтИмениПолучателяСписком( ДокСсылка, СписокВСД ) Экспорт
	...
	Для Каждого строкаВсдВходящие Из мВсдВходящие Цикл 
		...
		Если строкаВсдВходящие.ТипВСД <> "TRANSPORT" Тогда 
			Продолжить;
		ИначеЕсли строкаВсдВходящие.СтатусВСД <> Перечисления.кб99_СтатусВСД.CONFIRMED Тогда 
			Продолжить;
		КонецЕсли;			
