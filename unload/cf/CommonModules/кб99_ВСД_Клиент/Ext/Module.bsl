...
Функция ПогаситьВСДПоставщика( ДокСсылка, ТаблицаВсдДляГашения) Экспорт
	...
	Ответ =  кб99_ВСД_Запросы.ПолучитьРезультат_ВСД2( ПараметрыОрганизации, ДокСсылка.applicationID, ДокСсылка, Ложь, Истина, ТаблицаВсдДляГашения );	
	...
	Попытка 
		...
		КонецЦикла;
	Исключение 
		кб99_ВСД.СообщитьИнфо("Не удалось получить партии для гашения");
		Возврат Ложь;
	КонецПопытки;	
...
Процедура СоздатьИлиОткрытьВСД2( ДокОснование ) Экспорт
	...
	Если НЕ ПараметрыОрганизации["ПарамРазрешитьВводНаОснованииБолееОдногоВСД"] Тогда
		ДокВСД = кб99_ВСД.НайтиВСД( ДокОснование.Ссылка );
