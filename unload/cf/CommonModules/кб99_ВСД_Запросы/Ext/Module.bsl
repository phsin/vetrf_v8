...
Функция НайтиВсдОбъектПоUUID( _uuid, ВидСоздаваемогоДокумента = "ВСД" ) 
...
Функция ВСД2_входящий_Гашение_ЗапросXML(Параметры, ДокСсылка)
   ...
   Если ЗначениеЗаполнено(ДокСсылка.Отправитель_ХозСубъект) ИЛИ ЗначениеЗаполнено(ДокСсылка.Отправитель_Площадка) Тогда   
	   Запрос = Запрос +"
	   	|<vd:consignor>   
		|<dt:businessEntity>
		|<bs:guid>"+СокрЛП(ДокСсылка.Отправитель_ХозСубъект.GUID)+"</bs:guid>
