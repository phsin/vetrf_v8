...
Процедура ОтправитьВСД_Параллельно(Знач Параметры, АдресХранилища=Неопределено) Экспорт
	...
	Для Каждого ДокСсылка Из СписокДокументов Цикл	
		...
		Если ЗначениеЗаполнено(ДокСсылка.Получатель_Площадка.Регион) И
			...
			ВСД2_Транзакция_УсловияПеревозки_Отправить( ПараметрыОбщие, ДокСсылка, Ложь, Истина );
...
Функция ВСД2_Транзакция_УсловияПеревозки_Отправить(Знач Параметры, ДокСсылка, ПолучитьОтветВетис=Истина, ЗаписыватьВДокумент=Ложь) Экспорт
