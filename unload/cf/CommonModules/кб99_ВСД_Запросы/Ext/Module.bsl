...
Функция Продукция_Элемент_Найти( Знач Параметры, productItem, ОбновитьДанные=Истина, ВидПроисхожденияНеПищевойПродукции = Неопределено ) Экспорт
...
Функция ЗагрузитьXML_ВСД2(Знач Параметры, xdto, докОснование = Неопределено, ВсдРезультатМассив = Неопределено ) Экспорт
	...
	Для каждого vetDocument ИЗ vetDocumentList Цикл
		...
		ВсдСтруктура.Продукция_Элемент = Продукция_Элемент_Найти( Параметры, batch.productItem,,ВсдСтруктура.ВидПроисхожденияНеПищевойПродукции );
		Попытка	
