...
Функция ПроверитьЗаполнениеДокументаТранзакция( ДокСсылка, ПараметрыОрганизации )
	...
	Если ПараметрыОрганизации["КонтролироватьПравильностьЗаполненияНомераАвтомобиля"] Тогда
		ПроверитьНомерАвтомобиля(Ответ, ДокСсылка);	
	КонецЕсли;
...
Функция ПроверитьЗаполнениеДокументаПроизводство( ДокСсылка, ПараметрыОрганизации )
	...
	Если ПараметрыОрганизации["ПарамКонтроллироватьСрокГодностиПриОтправке"] Тогда 
		Для Каждого Стр Из тзПартии Цикл
			...
			//	Если ДатаСрокГодности2 - ПараметрыОрганизации["СрокГодностиДней"]*86400 <= ТекущаяДата() Тогда
			//		
			//		кб99_ВСД.СообщитьИнфо("Выбрана просроченная партия по срокам годности2: " +ДатаСрокГодности2+ " (менее "+ ПараметрыОрганизации["СрокГодностиДней"] +" дней до окончания срока) в строке "+Стр.НомерСтроки+"!", ДокСсылка);
			//		Ответ = Ложь;
			//		
			//	КонецЕсли;
			//Иначе
...
//Процедура СохранитьСтатусТранзакции( ПараметрыФункции )
//	
//	ДокСсылка = ПараметрыФункции["ДокОснование"];
...
//			
//			Если СтатусВСД = Перечисления.кб99_СтатусВСД.CONFIRMED ИЛИ
//				 СтатусВСД = Перечисления.кб99_СтатусВСД.UTILIZED Тогда 
//				ДокОбъект.Записать( РежимЗаписиДокумента.Проведение );
//			Иначе
//				ДокОбъект.Записать( РежимЗаписиДокумента.Запись );
//			КонецЕсли;
//		Иначе
//			кб99_ВСД.СообщитьОбОшибке("Не указан статус ВСД "+докСсылка, ДокСсылка );
//		КонецЕсли;
//				
//	Исключение 
//		
	...
	Попытка
		...
		ИначеЕсли (ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Производство")) Тогда 
			...
			Если ПараметрыФункции.Параметры.Свойство("ЗавершитьПроизводство") Тогда
				...
				//Если ПараметрыФункции.Свойство("СписокПартий") Тогда
				//	СписокПартий = ПараметрыФункции.СписокПартий;
				//	кб99_ВСД_Общий.ЗаполнитьНовыеПартииВТЧДокумента( СписокПартий, ДокСсылка);
				//КонецЕсли;
		...
		Иначе
			...
			Если ПараметрыФункции.Свойство("СписокПартий") Тогда
				...
				СписокПартий = ПараметрыФункции.СписокПартий;
				кб99_ВСД_Общий.ЗаполнитьНовыеПартииВТЧДокумента( СписокПартий, ДокОбъект);
...
Функция ПолучитьРезультат_ВСД2( ПараметрыФункции ) Экспорт
	...
	ДокСсылка 		= ПараметрыФункции["ДокОснование"];
	//тзВсдРезультат 	= ПараметрыФункции["тзВсдРезультат"];
	...
	Если Статус = Перечисления.кб99_СтатусЗапроса.COMPLETED Тогда 	
		...
		Попытка
			...
			//Попытка
				...
				ЗагрузитьXML_ВСД2( ПараметрыФункции );
				ЗагрузитьXML_Партии2( ПараметрыФункции );
				ЗафиксироватьТранзакцию();
	...
	Если ПараметрыФункции.Свойство("Смещение") Тогда 
		//ПараметрыЗапроса = ПараметрыФункции.ПараметрыЗапроса;
...
Функция ЗагрузитьXML_Партии2( ПараметрыФункции ) 
...
Функция ЗагрузитьXML_ВСД2( ПараметрыФункции ) Экспорт
