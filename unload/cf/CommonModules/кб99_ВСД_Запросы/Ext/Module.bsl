		...
		КонецЕсли; 
			...
			Для каждого enterprise ИЗ enterpriseList Цикл		
				...
				Попытка 
					...
					ЭлементПлощадка.ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, ЭлементПлощадка.GUIDХозСубъекта, Истина, Истина );
				Исключение 
...
Функция ХозСубъекты_ЗагрузитьСписокПоИнн(Знач Параметры ) Экспорт
	...
	Для каждого businessEntity ИЗ businessEntityList Цикл		
		...
		ЭлементХозсубъект.ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, ЭлементХозсубъект.GUID, Ложь, Ложь );
...
Функция Партия_Заполнить( Параметры, stockEntry, докСсылка = Неопределено )
	...
	Попытка	Партия.Получатель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, batch.owner.guid, Истина, Истина ); Исключение КонецПопытки;
...
Функция ЗагрузитьXML_ВСД2(Знач Параметры, xdto, докОснование = Неопределено, ВсдРезультатМассив = Неопределено ) Экспорт
	...
	Для каждого vetDocument ИЗ vetDocumentList Цикл
		...
		Если ЭтоТранспортныйВСД = 1 Тогда
			Попытка 
				ВсдСтруктура.Отправитель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.consignor.businessEntity.guid, Истина, Истина); 
			Исключение 
			...
			Исключение КонецПопытки;
			Попытка 
				ВсдСтруктура.Получатель_ХозСубъект = ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.consignee.businessEntity.guid, Истина, Истина); 
			Исключение 
			...
			Исключение КонецПопытки;
			Попытка ВсдСтруктура.ВладелецХС = ХозСубъект_НайтиПоGUID( Параметры, batch.owner.guid, Истина, Истина);	Исключение КонецПопытки;
			Попытка
				Если НЕ(ЗначениеЗаполнено(ВсдСтруктура.ВладелецХС)) Тогда
					ВсдСтруктура.ВладелецХС = ВсдСтруктура.Отправитель_ХозСубъект;	
				КонецЕсли;
			Исключение КонецПопытки;
			Попытка ВсдСтруктура.Перевозчик_ХозСубъект 	= ХозСубъект_НайтиПоGUID( Параметры, vetDocument.certifiedConsignment.broker.guid, Истина, Истина); Исключение КонецПопытки;
