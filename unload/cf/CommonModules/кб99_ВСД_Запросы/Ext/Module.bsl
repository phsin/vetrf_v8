...
Функция Партии_Ответ_ЗагрузитьXML( Параметры, xdto )
	...
	Если ПришлоПартий > 0 Тогда // Иначе возникает ошибка если получено 0 партий
		...
		Если кб99_ВСД_Общий.ФормироватьДвиженияПартий(Параметры.Организация) И Не Параметры.Свойство("ЭтоВходящийВСД") Тогда
...
Функция ЗагрузитьXML_Партии2( ПараметрыФункции ) 
	...
	Если кб99_ВСД_Общий.ФормироватьДвиженияПартий(ДокСсылка.Организация) Тогда
...
Процедура ПроверитьПроизводствоПередОтправкой(Результат, ПараметрыОрганизации, ДокСсылка, тзПартииСписания)
	...
	Если ЗначениеЗаполнено(Комментарий) Тогда
		...
		кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе %1 содержаться ошибки: %2. Документ не отправлен.", ДокСсылка, Комментарий));
...
Функция ВСД2_Входящий_ГашениеПолучитьОтвет(Параметры, appID, докСсылка, ВСД_Партия = Неопределено, СделатьПаузу = Истина ) Экспорт
	...
	Если кб99_ВСД_Общий.ФормироватьДвиженияПартий(ДокСсылка.Организация) И ЗначениеЗаполнено(ВСД_Партия) Тогда
