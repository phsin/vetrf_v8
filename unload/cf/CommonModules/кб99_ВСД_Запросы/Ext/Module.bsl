...
Функция ПолучитьНастройкиПодключенияКСерверуЛицензирования()
	
	АдресСервера = "http://1c.kb99.pro/lic/ws/validation.1cws?wsdl";
	ИмяПользователя = "Service";
	Пароль = "p9gPzaJA0E";
	
	Возврат Новый Структура("АдресСервера, ИмяПользователя, Пароль", 
							 АдресСервера, ИмяПользователя, Пароль);
	
КонецФункции

Функция ПолучитьДанныеОтехподдержке(Параметры) Экспорт
	
	ПараметрыПодключения = ПолучитьНастройкиПодключенияКСерверуЛицензирования();
	
	Определение = Новый WSОпределения(ПараметрыПодключения.АдресСервера, 
	ПараметрыПодключения.ИмяПользователя, 
	ПараметрыПодключения.Пароль);
	
	Прокси = Новый WSПрокси(Определение, "validation", "validation", "validationSoap");
	Прокси.Пользователь = ПараметрыПодключения.ИмяПользователя;
	Прокси.Пароль = ПараметрыПодключения.Пароль;
	
	ПараметрыЗапроса = Сериализовать(Параметры);
	Ответ = Десериализовать(Прокси.CheckLic(ПараметрыЗапроса));
...
Функция ПолучитьРезультат_Список_Продукции( Параметры, xdto, ВыбПродукцияЭлемент="", Площадка) Экспорт
	...
	Попытка
		...
		Если Число(xdto.Body.getProductItemListResponse.productItemList.count) = 1000 Тогда
