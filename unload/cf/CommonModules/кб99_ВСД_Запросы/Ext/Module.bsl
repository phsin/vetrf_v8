...
Процедура ИнициализацияХС_ЗагрузитьУлицы_ВФоне( Знач Параметры, АдресХранилища ) Экспорт
	...
	_Город = ?(ЗначениеЗаполнено(Параметры["НаселенныйПункт"]),Параметры["НаселенныйПункт"], Параметры["Город"]);
...
Функция Площадка_ЗагрузитьПоGUID(Знач Параметры, _guid, ХозСубъект=Неопределено, Ссылка_ВСД_Площадка=Неопределено) Экспорт
	...
	Попытка	    
		...
		НаселенныйПункт = ?(ЗначениеЗаполнено(Площадка_Объект.НаселенныйПункт), Площадка_Объект.НаселенныйПункт, Площадка_Объект.Город);
...
Функция ХозСубъект_ЗагрузитьПоGUID(Знач Параметры, GUID, Ссылка_ВСД_ХозСубъект=Неопределено) Экспорт
	...
	Попытка	
		...
		НаселенныйПунктДляПоискаУлицы = ?(ЗначениеЗаполнено(ХозСубъект_Объект.НаселенныйПункт), ХозСубъект_Объект.НаселенныйПункт, ХозСубъект_Объект.Город);
		Попытка ХозСубъект_Объект.Улица	= НайтиУлицуПоGUID( businessEntity.juridicalAddress.street.GUID, businessEntity.juridicalAddress.street.name, НаселенныйПунктДляПоискаУлицы); Исключение КонецПопытки;
