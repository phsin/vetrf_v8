...
Функция ПроверитьЗаполнениеДокумента( ДокСсылка, ПараметрыОрганизации )
	...
	Если ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Транзакция") Тогда
		тчПартии = ДокОбъект.Товары;
	ИначеЕсли  ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Производство") Тогда 
		тчПартии = ДокОбъект.ПартииСписания;
	КонецЕсли;
		
	Если ПараметрыОрганизации["ПарамКонтроллироватьСрокГодностиПриОтправке"] И тчПартии <> Неопределено Тогда 
