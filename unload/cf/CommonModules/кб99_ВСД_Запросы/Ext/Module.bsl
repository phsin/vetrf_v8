...
Функция Партии_Ответ_ЗагрузитьXML(Параметры, xdto )
	...
	Если ПришлоПартий > 0 Тогда // Иначе возникает ошибка если получено 0 партий
		...
		Для каждого stockEntry ИЗ stockEntryList Цикл		
			...
			Если Параметры.Свойство("СписокПартий") И Партия <> Неопределено  Тогда
				Параметры.СписокПартий.Добавить(Партия.Ссылка);	
			КонецЕсли;
		КонецЦикла;
...
Функция ВСД2_ЗагрузитьПоUUID( Знач Параметры, докОснование ) Экспорт
	...
	Для Каждого строкаВСД ИЗ тзВСД Цикл
		...
		Ответ =  ПолучитьРезультат_ВСД2( Параметры,, appID, докОснование );	
		Для А=1 По 10 Цикл
			Если (Ответ="IN_PROCESS") Тогда
				Ответ =  ПолучитьРезультат_ВСД2( Параметры,, appID, докОснование );	
			КонецЕсли;
...
Функция ПолучитьИнформациюОпроисхожденииВСД(СтрокаТЧ, ПараметрыОрганизации) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ДанныеОДокументе) Тогда
			...
			СтруктураДок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ДанныеОДокументе, " ", Ложь);
...
Функция ВСД2_транзакция_ЗапросXML( Параметры, ДокСсылка)
	...
	Для А=0 По тз.Количество()-1 Цикл
		...
		ОсобыеОтметки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 %2", ?(ЗначениеЗаполнено(СтрокаДокумента.ОсобыеОтметки), СтрокаДокумента.ОсобыеОтметки+";", ""),
	...
	Попытка
		...
		ИначеЕсли (ТипЗнч(ДокСсылка) = Тип("ДокументСсылка.ВСД2_Производство")) И Параметры.Свойство("ЗавершитьПроизводство") Тогда
			Если Параметры.Свойство("СписокПартий") Тогда
				кб99_ВСД_Общий.ЗаполнитьНовыеПартииВТЧДокумента(Параметры.СписокПартий, ДокОбъект);
			КонецЕсли;
	...
	Если Статус = "COMPLETED" Тогда 
		...
		Попытка
			...
			Исключение
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при записи ВСД в документе %1'"), ДокСсылка),
					УровеньЖурналаРегистрации.Ошибка,,,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗагрузитьXML_Партии2(Параметры, xdto, докСсылка = Неопределено ) 
	...
	Для каждого stockEntry ИЗ stockEntryList Цикл		
		...
		Если Партия <> Неопределено И Параметры.Свойство("СписокПартий") Тогда
			СтруктураСтроки = Новый Структура("Партия, Количество", Партия.Ссылка, Партия.Количество);
			Параметры.СписокПартий.Добавить(СтруктураСтроки);	
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьНомерАвтомобиля(РезультатПроверки, Документ)
	...
	Если Отказ Тогда
		...
		кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе %1 %2", Документ.Ссылка, стрОшибка));
...
Процедура ПроверитьПроизводствоПередОтправкой(Результат, ПараметрыОрганизации, ДокОбъект, тзПартииСписания)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Продукция ""%1"" присутствует в партиях списания и вырабатываемой продукции.", ВыборкаДетальныеЗаписи.Продукция_Элемент);
			кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе %1 %2", ДокОбъект.Ссылка, Комментарий));
			ДокОбъект.Комментарий = ДокОбъект.Комментарий + Комментарий;
		КонецЦикла;
	КонецЕсли;
    ...
    Если ПараметрыОрганизации["ПроверятьБалансРасходаСырья"] И Не РезультатЗапроса.Пустой() Тогда
		Если тзПартииСписания.Количество() Тогда	
			...
			Если Разница > ПараметрыОрганизации["ПроцентРасхожденияСырьеПродукция"] Тогда
				Результат = Ложь;
				Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Нарушен допустимый процент расхождения [Кол-во вырабатываемое / Кол-во списания]. Разрешено: %1%%, расхождение: %2%%",
		...
		Иначе
			Результат = Ложь;
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Нарушен допустимый процент расхождения [Кол-во вырабатываемое / Кол-во списания]. Разрешено: %1%%, расхождение: 100%%",
																								   ПараметрыОрганизации["ПроцентРасхожденияСырьеПродукция"]);
			кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе %1 %2", ДокОбъект.Ссылка, Комментарий));
			ДокОбъект.Комментарий = ДокОбъект.Комментарий + Комментарий;
		КонецЕсли;
...
Функция ВСД2_Входящий_ГашениеПолучитьОтвет(Параметры, appID, докСсылка, ВСД_Партия = Неопределено, СделатьПаузу = Истина ) Экспорт
	...
	Если ЗначениеЗаполнено(ВСД_Партия) Тогда
