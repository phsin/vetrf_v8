...
Функция Площадки_ЗагрузитьПоУсловиям(Параметры, СписокУсловий) Экспорт
	...
	Пока ПолученоЗаписей = 1000 Цикл
		...
		xdto = кб99_ВСД_Запросы.Площадки_НайтиПоУсловиям( Параметры, СписокУсловий );	
		Если xdto = Неопределено Тогда
			Возврат Площадки;	
		КонецЕсли;
...
Функция Площадка_СвязатьСХозСубъектом_Запрос( ПараметрыОрганизации, ВыбПлощадка ) 
	...
	|        <apl:issueDate>" + кб99_ВСД_Запросы.ДатаXML(Текущаядата(), "T00:00:00") + "</apl:issueDate>
	|        <apl:data>
...
Процедура Площадка_СвязатьСХозСубъектом( ПараметрыОрганизации, ВыбПлощадка) Экспорт
	...
	кб99_ВСД_Запросы.Пауза( ПараметрыОрганизации["ПаузаСек"] );	
	...
	Статус = кб99_ВСД_Запросы.СтатусЗапроса(xdto);
	Если кб99_ВСД_Запросы.НайтиОшибки( xdto ) Тогда
		Возврат ;
	КонецЕсли;
