...
//ТЕСТ вызов переопределения
	...
	Попытка 
		ДокВСД.номерАвто = ПолучитьНомерАвто(ДокВСД.ДокументОснование); 
	Исключение 
...
//	КонецЕсли;
	...
	Рез = 0;
	Попытка
		КПересчета = ПараметрыОрганизации.Получить("ПарамКоэффициентПересчетаКоличества"); //СокрЛП(СписокКонстант.Получить("ПарамКоэффициентПересчетаКоличества"));
		Коэфф = 0;
		Попытка
		...
		Рез = СтрокаТЧДок[ ПараметрыОрганизации.Получить("НазваниеРеквизитаКоличество") ]*Коэфф;
...
//	КонецЕсли;
	...
	для каждого стрДок из док.Товары Цикл
		...
		Если (ВСДПродукцияЭлемент = Справочники.ВСД_Продукция_Элемент.ПустаяСсылка()) и НЕ( ПараметрыОрганизации.Получить("ПропускатьПустыеСвойства") ) Тогда
			ВСД.СообщитьИнфо("["+стрДок.Номенклатура+"] -> не указан ВСД_Продукция_Элемент");
		ИначеЕсли (ВСДПродукцияЭлемент=Справочники.ВСД_Продукция_Элемент.ПустаяСсылка()) и ( ПараметрыОрганизации.Получить("ПропускатьПустыеСвойства") ) Тогда
			Продолжить;
		КонецЕсли;
		...
		Попытка
		Если ЗначениеЗаполнено(стрДок[ ПараметрыОрганизации.Получить("НазваниеРеквизитаКоличествоМест") ]) Тогда
			СтрТЧ.Упаковки = стрДок[ ПараметрыОрганизации.Получить("НазваниеРеквизитаКоличествоМест") ];
		КонецЕсли;
		Исключение КонецПопытки;
	КонецЦикла;
	Возврат тзРез;
...
Функция ПолучитьДанныеДляСозданияВСДПеремещения(ПараметрыОраганизации, тзПартии, ДокОсн ) Экспорт
    Рез = Новый Структура;
	Рез.Вставить("Организация", ПараметрыОраганизации.Получить("ПарамПоставщикОрганизация"));
	Рез.Вставить("Отправитель_Хозсубъект", ПараметрыОраганизации.Получить("ПарамПоставщикХозСубъект"));
	Рез.Вставить("Отправитель_Площадка", ПараметрыОраганизации.Получить("ПарамПоставщикПлощадка"));
	Рез.Вставить("Получатель_Хозсубъект", ПараметрыОраганизации.Получить("Отправитель_Хозсубъект"));
	Рез.Вставить("Получатель_Площадка", ПараметрыОраганизации.Получить("Отправитель_Площадка"));
	Рез.Вставить("ДокОснование", ПараметрыОраганизации.Получить("ДокОсн"));
...
//	НоваяСтрока.ЕдиницаИзмерения 		= НоваяСтрока.Партия.ЕдиницаИзмерения;
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(НоваяСтрока.Цель) Тогда 
			НоваяСтрока.Цель = ПараметрыОрганизации.Получить("ВСДЦель");
		КонецЕсли;
	Исключение КонецПопытки;
	
	Если докСсылка.Метаданные().ТабличныеЧасти.Найти("УровниУпаковки") <> Неопределено тогда
		Если НЕ(ЗначениеЗаполнено( ПараметрыОрганизации.Получить("ПарамНомерУровняУпаковкиДляВСД") )) или НЕ(ЗначениеЗаполнено( ПараметрыОрганизации.Получить("ПарамФормаУпаковкиДляВСД"))) Тогда
		...
		Попытка
			...
			Если ЗначениеЗаполнено(СтрокаОснования.Упаковки) Тогда
				...
				иначе
					...
					СтрокаУпак.НомерУровня = ПараметрыОрганизации.Получить("ПарамНомерУровняУпаковкиДляВСД");//СписокПараметров.Получить("ПарамНомерУровняУпаковкиДляВСД");
					СтрокаУпак.ФормаУпаковки = ПараметрыОрганизации.Получить("ПарамФормаУпаковкиДляВСД");//СписокПараметров.Получить("ПарамФормаУпаковкиДляВСД");;
					//Маркировки
...
//	КонецЕсли;
	...
	ПартииКСписанию = ВСД.тзПартииСписанияПоТзПродукция_Элемент( ПараметрыОрганизации, тзЭлементыОснования, тзАктуальныхПартий );
	
	Если (ПартииКСписанию.Количество() = 0) и НЕ(ПараметрыОрганизации.Получить("ПарамЗаполнятьТранзакциюПриОтсутствииПартий")) Тогда
...
Функция ПолучитьДанныеДляСозданияВСДТранзакции( ПараметрыОрганизации, ДокументОснование ) Экспорт
// вызов переопределения
	...
	////Если ДокументОснование <> Неопределено Тогда //Заполним все по основанию
		...
		//_Отправитель_ХозСубъект = ПараметрыОрганизации.Получить("Отправитель_ХозСубъект");			
		//_Перевозчик_ХозСубъект = ПараметрыОрганизации.Получить("Перевозчик_ХозСубъект");
		...
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			...
			Если ПараметрыОрганизации.Получить("ПарамФильтроватьРеализациюПоСкладуПлощадкиОтправителя") Тогда   // Определим нашу площадку по Складу
				_Отправитель_Площадка = ВСД.НайтиПлощадкуПоСкладу(ДокументОснование.Склад, ПараметрыОрганизации.Получить("Отправитель_ХозСубъект"));
				//Отправитель_Площадка = _Отправитель_Площадка;
			Иначе
				_Отправитель_Площадка = ПараметрыОрганизации.Получить("Отправитель_Площадка");
			КонецЕсли;
			_Получатель_ХозСубъект = ВСД.НайтиХозСубъект(ДокументОснование.Контрагент);				
			Если ПараметрыОрганизации.Получить("РеквизитГрузополучатель") = 0 Тогда 
				//контрагент
		...
		Иначе //Перемещение
			_Отправитель_Площадка = ВСД.НайтиПлощадкуПоСкладу(ДокументОснование.СкладОтправитель, ПараметрыОрганизации.Получить("Отправитель_ХозСубъект"));
			//Отправитель_Площадка = _Отправитель_Площадка;
			_Получатель_ХозСубъект = ПараметрыОрганизации.Получить("Отправитель_ХозСубъект");
		...
		Рез.Вставить("Отправитель_Хозсубъект", ПараметрыОрганизации.Получить("Отправитель_ХозСубъект"));  // Взяли из реквизитов обработки - активные данные
...
//		Рез.Вставить("Организация", ПараметрыОрганизации.Получить("Организация") );
//		Рез.Вставить("Отправитель_Хозсубъект", ПараметрыОрганизации.Получить("Отправитель_Хозсубъект") );  // Взяли из реквизитов обработки - активные данные
//		Рез.Вставить("Отправитель_Площадка", ПараметрыОрганизации.Получить("Отправитель_Площадка") );
//		Рез.Вставить("Получатель_Хозсубъект", ПараметрыОрганизации.Получить("Получатель_Хозсубъект") ); // в параметре получили
//		Рез.Вставить("Получатель_Площадка",  ПараметрыОрганизации.Получить("Получатель_Площадка") );
