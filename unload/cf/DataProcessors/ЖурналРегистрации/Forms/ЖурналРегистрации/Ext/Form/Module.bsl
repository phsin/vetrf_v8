...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьВидимостьРазделения(ЭтаФорма, Истина);
...
Процедура ЖурналВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Новый ОписаниеОповещения("УстановитьИнтервалДатДляПросмотраЗавершение", ЭтаФорма));
...
Процедура ЖурналПриАктивизацииПоля(Элемент)
	...
	ДоступенОтборПоЗначениюВТекущейКолонке = Элемент.ТекущийЭлемент <> Элементы.Дата
		И Элемент.ТекущийЭлемент <> Элементы.Дата2;
	
	Элементы.УстановитьОтборПоЗначению.Доступность = ДоступенОтборПоЗначениюВТекущейКолонке;
	
	Элементы.УстановитьОтборПоЗначениюВТекущейКолонке2.Доступность = ДоступенОтборПоЗначениюВТекущейКолонке;
	
	Элементы.УстановитьОтборПоЗначениюВТекущейКолонкеКонтекст.Доступность = ДоступенОтборПоЗначениюВТекущейКолонке;
	
	Элементы.УстановитьОтборПоЗначениюВТекущейКолонкеКонтекст2.Доступность = ДоступенОтборПоЗначениюВТекущейКолонке;
...
Процедура УстановитьВидимостьРазделения(Форма, ВидимостьРазделения)
	...
	Элементы.ПредставлениеРазделенияДанныхСеанса.Видимость = ВидимостьРазделения И Не Форма.ТолькоСтандартныеРазделители;
	
	Элементы.ПредставлениеРазделенияДанныхСеанса2.Видимость = ВидимостьРазделения И Не Форма.ТолькоСтандартныеРазделители;
...
Функция ОтборПоПользователюИзПараметра(ПараметрПользователь)
	...
	Для Каждого Имя Из Имена Цикл
		ПользовательИБ = ПараметрыСеанса.ТекущийПользователь.Наименование;
...
Функция ПрочитатьЖурнал()
	...
	Возврат Новый Структура("АдресРезультата", АдресХранилища);
