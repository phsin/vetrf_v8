...
Функция СоздатьДокумент_ВСД2_Транзакция(СписокПараметров, тзАктуальныхПартий ) Экспорт
	...
	//ДанныеЗаполнения.Вставить("ПараметрыОрганизации", ПараметрыОрганизации);
	ДанныеЗаполнения.Вставить("тзАктуальныхПартий", тзАктуальныхПартий);	
	ДанныеЗаполнения.Вставить("ПараметрыДокумента", СписокПараметров);	
	
	ДокВСД = Документы.ВСД2_транзакция.СоздатьДокумент();
	ДокВСД.Заполнить( ДанныеЗаполнения );
	...
	ДокВСД.Записать();
	ВСД.СообщитьИнфо(" Создан документ "+докВСД, докВСД.Ссылка);
...
Процедура СоздатьВСД2() Экспорт
	//Создает ВСД по списку из тч Отгрузки
 	// С учетом актуальных Партий (за минусом неотправленных ВСД из списка ГО
	...
	Для Каждого СтрОтгрузки Из Отгрузки Цикл
		...
		Если ЗначениеЗаполнено( СтрОтгрузки.ВСД ) Тогда
			//Если СтрОтгрузки.ВСД.Проведен Тогда
				СообщитьОшибку("Для "+СтрОтгрузки.Док+" уже создан "+СтрОтгрузки.ВСД+", Статус:"+СтрОтгрузки.ВСД.Статус);
				Продолжить;
			//КонецЕсли;
			//Если ПустаяСтрока(СтрОтгрузки.ВСД.Статус) Тогда 
		...
		//ПараметрыДокумента.Вставить("Отправитель_Хозсубъект", СтрОтгрузки.ХозСубъект);
		//ПараметрыДокумента.Вставить("Отправитель_Площадка", СтрОтгрузки.Площадка);
		...
		//ПараметрыДокумента = ВСД_Общий.ПолучитьДанныеДляСозданияВСДТранзакции( СписокКонстант, СтрОтгрузки.Док );
		ДокВСД = СоздатьДокумент_ВСД2_Транзакция( ПараметрыДокумента, тзАктуальныхПартий );
...
//	
//	ДокВСД = СоздатьДокумент_ВСД2_Транзакция( СписокПараметров, Неопределено);
