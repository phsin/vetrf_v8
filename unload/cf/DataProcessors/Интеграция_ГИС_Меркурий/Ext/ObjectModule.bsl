    ...
    Если ПарамФильтроватьРеализациюПоСкладуПлощадкиОтправителя тогда
        Попытка 
            ...
            Если НЕ(ЗначениеЗаполнено(ПлощадкаОтправителя.Склад)) Тогда
                СообщитьИнфо("Не определен Склад по Площадке "+ПлощадкаОтправителя+" привяжите склад к площадке либо отключите параметр [Фильтровать Реализацию По Складу Площадки Отправителя]!");    
            КонецЕсли;
        Исключение 
            Запрос.УстановитьПараметр("ПлощадкаОтправителяСклад",    ""); 
            СообщитьИнфо("Не определен Склад по Площадке для фильтрации документов!"); 
        КонецПопытки;    
    КонецЕсли;
...
Функция СформироватьЗапросПеремещения(Организации = Неопределено, ПлощадкаОтправителя, ДатаНачала, ДатаОкончания) Экспорт
	...
	Запрос.УстановитьПараметр("СписокОрганизаций",			Организации);
//	Запрос.УстановитьПараметр("ПлощадкаОтправителяСклад",				ПлощадкаОтправителя.Склад);  
 	Попытка Запрос.УстановитьПараметр("ПлощадкаОтправителяСклад",	ПлощадкаОтправителя.Склад); Исключение Запрос.УстановитьПараметр("ПлощадкаОтправителяСклад",	""); СообщитьИнфо("Не определили Склад!"); КонецПопытки;
...
//	КонецЕсли;
	...
	ПартииКСписанию = ВСД_Производство.тзПартииСписанияПоТзПродукция_Элемент( СписокКонстант, тзЭлементыОснования, тзАктуальныхПартий );
...
Функция ПроверитьВозможностьПеремещенияВСДПоСписку() Экспорт
	...
	Для каждого строкаТЗ из тзОтгрузки Цикл
		...
		Попытка
			...
			ВСД_Запросы.ВС2_Транзакция_УсловияПеревозки_Отправить();
		Исключение
		    СообщитьИнфо(ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
...
Процедура ВСД_Производство_ВводНаОсновании(ДокВСД, СписокПараметров = Неопределено, ПараметрыОрганизации = Неопределено) Экспорт
	...
	тзСырье = ВСД_Производство.ЗаполнитьТчСписаниеИзСпецификацииНоменклатуры( ДокВСД.Продукция.Выгрузить() );
...
Процедура Создать_ВСД2_Производство(ДатаВыпуска = "") Экспорт
	...
	ТзНеобходимогоСырья = ВСД_Производство.ЗаполнитьТчСписаниеИзСпецификацииНоменклатуры( ТЗНеобходимойПродукции );	
...
//		КонецЕсли;
		...
		//ТабСырья = ВСД_Производство.ЗаполнитьСырьеИзВСД_Продукция_Элемент( ДокВСД.Продукция.Выгрузить() );
		ТабСырья = ВСД_Производство.ЗаполнитьТчСписаниеИзСпецификацииНоменклатуры( ДокВСД.Продукция.Выгрузить() );
		...
		СообщитьИнфо(" создан документ "+стрПартий.ВСД_Производство);		
	КонецЦикла;
	РаскраситьТЧПартий();
КонецПроцедуры
