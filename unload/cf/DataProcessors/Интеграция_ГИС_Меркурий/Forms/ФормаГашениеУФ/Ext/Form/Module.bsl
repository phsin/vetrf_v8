...
//			Продолжить;	
//		КонецЕсли;
		докОбъект = ВСД_Запросы.ВСД2_Входящий_НайтиПоUUID( стрВход.uuid );//Документы.ВСД2_входящий.СоздатьДокумент();//	
		...
		Если ЗначениеЗаполнено(стрВход.СвязанныеДокументы) Тогда    
			...
			Для Каждого строкаДок Из СпСвязанныеДокументы Цикл
				...
				стрТЧ.ДатаДокумента = ВСД_Запросы.СтрокаВДату(строкаДок.ДатаДокумента);
		...
		ВСД.СообщитьИнфо("Записан "+докОбъект+" "+докОбъект.Номер+" от "+докОбъект.Дата+" ["+докОбъект.uuid+"]");
	...
	Для Каждого СтрокаТЗ Из Объект.ВСДВходящие Цикл 
		...
		Если Объект.флСоздаватьВсдВходящий Тогда
			...
			Если СтрокаТЗ.ДокВСД.Проведен Тогда
			    Продолжить;
			КонецЕсли;
			СтрокаТЗ.Статус = ВСД_Запросы.ВСД2_Входящий_ОтправитьГашение( ПараметрыОрганизации, СтрокаТЗ.ДокВСД );
		Иначе
			Если СокрЛП(СтрокаТЗ.Статус) = "COMPLETED" Тогда
				ВСД.СообщитьИнфо("Партия уже оформлена. uuid "+СтрокаТЗ.uuid);
		    	Продолжить;
			КонецЕсли;
			СтрокаТЗ.Статус = ВСД_Запросы.ВСД2_Входящий_ОтправитьГашение( ПараметрыОрганизации, СтрокаТЗ ); // не создаем документ ВСД2_Входящий в 1с
		КонецЕсли;
...
Процедура Отправитель_ПлощадкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Элемент.имя = "ВыбрПлощадка" Тогда
		ГУИДХСдляОтбора = ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(ВыбрХозСубъект,"GUID");
	Иначе
...
Процедура кнПолучитьВСДНаСервере()
	...
	Результат = ВСД_Запросы.ПолучитьСписокВСД( ПараметрыОрганизации, Объект.Отправитель_Площадка, "INCOMING", "CONFIRMED", 0, ВыбначалоПериода, ВыбОкончаниеПериода, ВыбрХозСубъект, ВыбрПлощадка, Ложь, ВсдМассив );
	Пока Результат = "REJECTED" и (Запросов > 0) Цикл
		Запросов = Запросов - 1;
		ВСД.СообщитьИнфо("Попыток запроса ВСД осталось "+Запросов);
		Результат = ВСД_Запросы.ПолучитьСписокВСД( ПараметрыОрганизации, Объект.Отправитель_Площадка, "INCOMING", "CONFIRMED", 0, ВыбначалоПериода, ВыбОкончаниеПериода, ВыбрХозСубъект, ВыбрПлощадка, Ложь, ВсдМассив );
	КонецЦикла;
	//ЗначениеВРеквизитФормы(Фобъект,"Объект");
...
Функция ПолучитьНоменклатуруПоПродукцияЭлемент(ПродукцияЭлемент, Только1элемент = Ложь)
	...
	Возврат ВСД.ПолучитьНоменклатуруПоПродукцияЭлемент( ПродукцияЭлемент, Только1элемент);
...
Функция НайтиНоменклатуруПоРеквизиту(ВыбРеквизит,ЗначениеРеквизита)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВСД.СообщитьИнфо("Что-то пошло не так");
...
Процедура ОчиститьСоответствие(ВыбНоменклатура,ВСДЭлемент)
	...
	ВСД.ОчиститьСоответствие_ВСД_Продукция_Элемент( ВыбНоменклатура, ВСДЭлемент );
КонецПроцедуры
...
Процедура УстановитьФильтрНаСервере()
	...
	пока НомерСтроки>=0 Цикл
		...
		Если ЗначениеЗаполнено(ВыбНачалоПериода) и ТекСтрока.ВСДДата < ВыбНачалоПериода Тогда 
			ВСД.СообщитьИнфо("Отфильтрована партия "+ТекСтрока.ВСДДата+" ["+ТекСтрока.НаименованиеПродукции+"] = "+ТекСтрока.Количество );
		...
		Если ЗначениеЗаполнено(ВыбрХозСубъект) и ТекСтрока.Отправитель_Хозсубъект <> ВыбрХозСубъект Тогда 
			ВСД.СообщитьИнфо("Отфильтрована партия "+ТекСтрока.Отправитель_Хозсубъект+" ["+ТекСтрока.НаименованиеПродукции+"] = "+ТекСтрока.Количество );
			Объект.ВСДВходящие.Удалить(Номерстроки);
		КонецЕсли;
		Если ЗначениеЗаполнено(ВыбрПлощадка) и ТекСтрока.Отправитель_Площадка <> ВыбрПлощадка Тогда 
			ВСД.СообщитьИнфо("Отфильтрована партия "+ТекСтрока.Отправитель_Площадка+" ["+ТекСтрока.НаименованиеПродукции+"] = "+ТекСтрока.Количество );
			Объект.ВСДВходящие.Удалить(Номерстроки);
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отправитель_ХозСубъект") Тогда 
		Если НЕ ЗначениеЗаполнено( Параметры.Отправитель_ХозСубъект ) Тогда 
			ВСД.СообщитьИнфо("Не заполнен параметр [Отправитель_ХозСубъект] для загрузки параметров");
			Возврат;			
		КонецЕсли;
		Объект.Организация = ВСД.ПолучитьОрганизациюПоХС( Параметры.Отправитель_ХозСубъект );
		
		Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда 
			ВСД.СообщитьИнфо("Не найдена организация для загрузки параметров по "+Параметры.Отправитель_ХозСубъект);
			Возврат;
		КонецЕсли;
	...
	Если Параметры.Свойство("Отправитель_Площадка") Тогда 
		Если НЕ ЗначениеЗаполнено( Параметры.Отправитель_Площадка ) Тогда 
			ВСД.СообщитьИнфо("Не заполнен параметр [Отправитель_Площадка]");
			Возврат;			
		КонецЕсли;
