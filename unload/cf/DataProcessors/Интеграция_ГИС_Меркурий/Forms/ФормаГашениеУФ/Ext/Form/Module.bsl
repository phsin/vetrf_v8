	...
	//объект.тОписаниеНесоответствия = тОписаниеНесоответствия;
		...
		Если НЕ Объект.ФлНеЗагружать Тогда
			...
			Если СтрокаТЗ.ДокВСД.Проведен Тогда
			    Продолжить;
			КонецЕсли;
			Рез = ВСД_Запросы.ВСД2_Входящий_ОтправитьГашение( ПараметрыОрганизации, СтрокаТЗ.ДокВСД );
		Иначе
			Если СокрЛП(СтрокаТЗ.Статус) = "COMPLETED" Тогда
				ВСД.СообщитьИнфо("Партия уже оформлена. uuid "+СтрокаТЗ.uuid);
		    	Продолжить;
			КонецЕсли;
			Рез = ВСД_Запросы.ВСД2_Входящий_ОтправитьГашение( ПараметрыОрганизации, СтрокаТЗ ); // не создаем документ ВСД2_Входящий в 1с
		КонецЕсли;
