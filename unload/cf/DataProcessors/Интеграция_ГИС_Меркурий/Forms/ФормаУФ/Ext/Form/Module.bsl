...
Процедура ЗаполнитьПредставлениеДокументовВТЧ(СтрОтгрузки)
	...
	Если ЗначениеЗаполнено(СтрОтгрузки.ВСД) Тогда
		СтрОтгрузки.ВСДПредставление = СтрШаблон("%1 от %2", СтрОтгрузки.ВСД.Номер, Формат(СтрОтгрузки.ВСД.Дата, "ДЛФ=DD"));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтрОтгрузки.Док) Тогда
		СтрОтгрузки.ДокПредставление = СтрШаблон("%1 от %2", СтрОтгрузки.Док.Номер, Формат(СтрОтгрузки.Док.Дата, "ДЛФ=DD"));
	КонецЕсли;
