...
&НаКлиенте
Процедура ЗагрузитьПлощадкиСКонтролемЗагруженныхХС(Команда)
	...
	Если Не ЗначениеЗаполнено(ВыбСтрока.ХозСубъект) Тогда 
		ХсПоИНН = ПроверитьСуществованиеХсПоИНН(ВыбСтрока.Контрагент);
		Если ХсПоИНН.Загружен Тогда   // уже имеется загруженный хозсубъект
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеВыбораЗагруженногоХозСубъекта", ЭтаФорма);
			СтруктураОтбора = Новый Структура;
			СтруктураОтбора.Вставить("Контрагент", ВыбСтрока.Контрагент);
			СтруктураОтбора.Вставить("ИНН", ХсПоИНН.ИНН);
			ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора, МножественныйВыбор, СтруктураОтбора", Истина, Истина, Ложь, СтруктураОтбора);	
			ОткрытьФорму("Справочник.ВСД_ХозСубъект.Форма.ФормаВыбораПоИНН", ПараметрыПодбора, ЭтаФорма,,,,ОповещениеОЗакрытии);
		ИначеЕсли ХсПоИНН.Свойство("Ошибка") Тогда
			кб99_ВСД.СообщитьИнфо(ХсПоИНН.Ошибка, ВыбСтрока.Контрагент);
		Иначе   // требуется загрузить данные по хозсубекту
			ЗагрузитьПлощадкиСпискомПоХозСубъекту_Сервер();
			ОбновитьСписокРеализацийНаСервере()
		КонецЕсли;
	Иначе    // Хозсубъект загружен, требуется обновить список площадок.
...
Функция ПроверитьСуществованиеХсПоИНН(Контрагент) Экспорт
	
	Результат = Новый Структура("Загружен", Ложь);
	...
	Если ЗначениеЗаполнено(Контрагент.ИНН) Тогда 
		...
		|	И ВСД_ХозСубъект.GUID <> """"";
		Запрос.УстановитьПараметр("ИНН", Контрагент.ИНН);
		Выборка = Запрос.Выполнить().Выбрать(); 
		
		Если Выборка.Следующий() Тогда
			Результат.Загружен = Истина;
			Результат.Вставить("ИНН", Контрагент.ИНН);
		КонецЕсли;
	Иначе
		стрОшибка = "Не заполнен реквизит ИНН для контрагента: "+Контрагент.Наименование;
		Результат.Вставить("Ошибка", стрОшибка);	
	КонецЕсли;
