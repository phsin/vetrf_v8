...
Процедура кнОтметитьСОшибкойСервер()
	...
	Для Каждого стр Из Объект.Отгрузки Цикл
		...
		ПоследнийЗапрос = кб99_ВСД_Общий.НайтиПоследнийЗапрос( стр.ВСД );
...
Функция ОтправитьВСД_НаСервере( ОтправитьБыстро )
	...
	Если Объект.ОтправлятьВФоне Тогда 
		...
		Если ОтправитьБыстро Тогда 
			Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне( УникальныйИдентификатор, 
				"кб99_ВСД.ОтправитьВСД_Параллельно", 
				ПараметрыФункции,
				НаименованиеЗадания);
		Иначе
			Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне( УникальныйИдентификатор, 
...
Функция ЗаполнитьТабличныеЧасти( Перемещения = Ложь, Заказы = Ложь ) Экспорт
	...
	Если кб99_ВСД_Общий.ПроверитьЗначениеПараметра(СписокКонстант, "СвойКонтрагент", "Свой контрагент") Тогда
