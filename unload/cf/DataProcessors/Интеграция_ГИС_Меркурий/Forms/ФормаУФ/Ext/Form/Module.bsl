...
&НаСервере
Функция ПолучитьСообщения(УдалятьПолученные = Ложь, ИдентификаторЗадания = Неопределено) Экспорт
...
Процедура ОбработчикОжидания() Экспорт 
	...
	//	Если ДанныеОВыполнении.Инд <> ДанныеОВыполнении.КоличествоОбходов 
	//		Тогда ПодключитьОбработчикОжидания("ОбработчикОжидания",5,Истина); 
	//	КонецЕсли; 
	//КонецЕсли; 
...
Процедура кнХозСубъект_ПолучитьGuidПоИНН(Команда)
	...
	Результат = кнХозСубъект_ПолучитьGuidПоИНН_НаСервере(ВыбранныйХС);
...
Процедура кнЗагрузитьПлощадкиПоХозСубъекту(Команда)
	...
	Результат = кнЗагрузитьПлощадкиПоХозСубъектуНаСервере( ВыбранныйХС );	
...
&НаСервере
Процедура ЗагрузитьПлощадкиСпискомПоХозСубъекту_Сервер( )
	//ФОбъект = РеквизитФормыВЗначение("Объект");
	тзСвертка = Объект.Отгрузки.Выгрузить();
	тзСвертка.Свернуть("Отметка, Контрагент, ХозСубъект",);
	ПараметрыФункции = ВСД_Общий.ПараметрыСервер( Объект );	
	ВСД_Запросы.Площадки_ЗагрузитьСписокПоХозСубъекту( ПараметрыФункции, тзСвертка );
	...
	//ФОбъект.ЗаполнитьТабличныеЧасти(флПеремещения); // обновим список
...
Процедура ЗагрузитьПлощадкиПоНаименованию(Команда)
	Если ЭтаФорма.Элементы.Отгрузки.ТекущиеДанные <> Неопределено Тогда
		СтруктураПараметров = Новый Структура("ВыбХС,ВыбРегион",ЭтаФорма.Элементы.Отгрузки.ТекущиеДанные.ХозСубъект,Объект.Регион);
	Иначе
		СтруктураПараметров = Новый Структура("ВыбХС,ВыбРегион","","");
	КонецЕсли;
...
Функция ПолучитьПартии2_НаСервере( НачДата, КонДата )
	ПараметрыФункции = ВСД_Общий.ПараметрыСервер( Объект );	
	ПараметрыФункции["ВыбПлощадка"] = Объект.Отправитель_Площадка;
	ПараметрыФункции["ПартииСмещение"] =0;
	ПараметрыФункции["УдалятьПартии"] = ложь;
	ПараметрыФункции["ТолькоАктуальныеПартии"] = Ложь;
	ПараметрыФункции["ПартияНачПериода"] = НачДата;
	ПараметрыФункции["ПартияКонПериода"] = КонДата;
...
Функция ПроверитьВозможностьПеремещенияВСДПоСписку() Экспорт
	...
	Параметры = ВСД.ЗагрузитьПараметры( Объект.Организация );
...
&НаСервере
Функция кнОтправитьВСДНаСервере(сзДокументыКОтправке)
	
	ПараметрыФункции = ВСД_Общий.ПараметрыСервер( Объект );
	ПараметрыФункции["СписокДокументов"] = сзДокументыКОтправке;
...
Процедура ОтгрузкиПлощадкаПриИзменении(Элемент)
	...
	//ВСД.СообщитьИнфо("Разработчику - сделать отвязку прежней площадки ! Смотри процедуру ниже");
...
Функция СформироватьЗапрос(Организации = Неопределено, ПлощадкаОтправителя = Неопределено, ДатаНачала, ДатаОкончания) Экспорт
	...
	//	Возврат Обработка.СформироватьЗапрос( Объект, Организации , ПлощадкаОтправителя , ДатаНачала, ДатаОкончания);
	//КонецЕсли;	
...
Функция СформироватьЗапросПеремещения(Организации = Неопределено, ПлощадкаОтправителя, ДатаНачала, ДатаОкончания) Экспорт
	...
	//Если обработка <> Неопределено Тогда
	//	Возврат Обработка.СформироватьЗапросПеремещения( Объект, Организации , ПлощадкаОтправителя , ДатаНачала, ДатаОкончания);
	//КонецЕсли;	
...
Функция ЗаполнитьТабличныеЧасти(Перемещения=False) Экспорт
	...
	//КонецЕсли;
	//ЖД Тест Перемещения
	//СообщитьИнфо("Заполнение списка документов....");
	...
	Иначе
		РезультатЗапроса = СформироватьЗапрос( Объект.Организация, Объект.Отправитель_Площадка, Объект.ДатаНачалаОтбора, Объект.ДатаОкончанияОтбора);
	КонецЕсли;
