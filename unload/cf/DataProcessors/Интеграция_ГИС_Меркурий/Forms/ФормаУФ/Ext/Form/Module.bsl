...
Процедура ЗаполнитьТаблицуПартийНаСервере( ЗнакСортировки=Неопределено ) Экспорт
	...
	КонецЦикла;
	ВремПартии.Свернуть("ВСД_Продукция_Элемент, СерияНоменклатуры, Номенклатура, Документ","КоличествоСписания");    // !!!
	...
	Для Каждого стрНужныеПартии Из ВремПартии Цикл
		...
		КонецЕсли;
	КонецЦикла;
	РаспределенныеПартии.Сортировать("Документ, Номенклатура, Продукция_Элемент");
...
Функция СоздатьВСДПроизводствоНаСервере( ВыбДата )
	...
	тзПартии.Свернуть("НомерСтроки, Продукция_Элемент, ВСД_Производство, Партия","КоличествоСписания, Количество");
