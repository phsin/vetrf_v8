	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Для Каждого стр Из Объект.Партии Цикл
			Если ЗначениеЗаполнено(стр.ВСД_Производство) Тогда
				ДокОснование = Новый Структура("Док", стр.ВСД_Производство);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ЗначениеЗаполнено(ДокОснование) И (СокрЛП(кб99_ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(ДокОснование.Док,"Статус")) = "COMPLETED") Тогда
			формЛаб = ОткрытьФорму("Обработка.ВСД_ЛабИсследования.Форма.Форма",ДокОснование);
		Иначе
