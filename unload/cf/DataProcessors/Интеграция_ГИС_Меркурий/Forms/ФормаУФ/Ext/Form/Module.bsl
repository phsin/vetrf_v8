...
Функция ОтправитьВСД_НаСервере( ОтправлятьРанееОтправленные )
	...
	Иначе;
		...
		ВСД_Запросы.ОтправитьВСД_Параллельно( ПараметрыФункции, Неопределено );
...
Функция ЗаполнитьТабличныеЧасти( Перемещения = Ложь ) Экспорт
	...
	Если Перемещения Тогда
		РезультатЗапроса = ВСД_Общий.СформироватьЗапросПеремещения( ПараметрыФункции );
	Иначе
		РезультатЗапроса = ВСД_Общий.ПолучитьТзРеализаций( ПараметрыФункции );
	КонецЕсли;
