...
Процедура СохранитьПараметрыНаСервере()
    ФОбъект = РеквизитФормыВЗначение("Объект");
	кб99_ВСД.СохранитьПараметры(ФОбъект);
...
Функция ЗагрузитьПродукциюНаСервере()
	...
	Если Объект.ОтправлятьВФоне Тогда 
		...
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Ответ = кб99_ВСД_Запросы.ИнициализацияХС_ЗагрузитьПродукцию_Все( ПараметрыФункции, "");
		Результат = Новый Структура;
		Результат.Вставить("ЗаданиеВыполнено", Истина);
	КонецЕсли; 
...
Функция кнЗагрузитьФормыУпаковкиНаСервере()
	...
	Если Объект.ОтправлятьВФоне Тогда 
		...
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Ответ = кб99_ВСД_Запросы.ЗаполнитьФормыУпаковки( ПараметрыФункции, "" );
		Результат = Новый Структура;
		Результат.Вставить("ЗаданиеВыполнено", Истина);
	КонецЕсли; 
...
&НаСервере
Процедура ЗаполнитьТаблицуПартий( ЗнакСортировки=Неопределено ) Экспорт
	...
	Для Каждого стрНужныеПартии Из ВремПартии Цикл
		...
		СтрокиПартий = кб99_ВСД.ПодобратьПартииПоПродукцияЭлемент( ПараметрыОрганизации, тзАктуальныхПартий, стрНужныеПартии.ВСД_Продукция_Элемент, стрНужныеПартии.КоличествоСписания );
		...
		КонецЕсли;
	КонецЦикла;
	Объект.Партии.Загрузить(РаспределеннаяПартии);
	РаскраситьТЧПартий();
КонецПроцедуры
...
&НаКлиенте
Процедура СписокЗаполненияПартийОбработатьВыбор(ЗначениеВыбора, ДопПараметры) Экспорт
	...
	ЗаполнитьТаблицуПартий( ВыбЭлемент );
	ПоказатьОповещениеПользователя("Выполнено");
...
&НаКлиенте
Процедура кнЗаполнитьТаблицуПартий(Команда, ПоУмолчанию = Ложь)
	
	Если ПоУмолчанию Тогда 
		ЗаполнитьТаблицуПартий( Неопределено );
	Иначе
...
Процедура кнПолучитьОтветВСД2НаСервере()
	...
	Для Каждого строкаОтгрузки Из Объект.Отгрузки Цикл
		Если строкаОтгрузки.Отметка Тогда 
			...
			кб99_ВСД_Запросы.ПолучитьРезультат_ВСД2( ПараметрыФункции, ВыбВСД.applicationID, ВыбВСД, Ложь );
		КонецЕсли;
	КонецЦикла;
...
Процедура кнПроизводствоОтправить(Команда)
	...
	Для Каждого стр Из Объект.Партии Цикл				
		Если НЕ(ЗначениеЗаполнено(стр.ВСД_Производство)) Тогда
			Продолжить;
		КонецЕсли;
		Если кб99_ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(стр.ВСД_Производство,"Проведен") Тогда
			Продолжить	
		КонецЕсли;
	...
	Для Каждого стр Из Объект.Партии Цикл				
		...
		Если (СокрЛП(кб99_ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(стр.ВСД_Производство,"Статус")) = "COMPLETED") Тогда
	...
	Если ОтправкаОК Тогда
		...
		кнЗаполнитьТаблицуПартий(Команда, Истина);
		ПоказатьОповещениеПользователя("Выполнено");
	КонецЕсли;
КонецПроцедуры
...
Процедура кнПолучитьОтветВСДПроизводство(Команда)
	...
	Для Каждого строкаТЗ Из Объект.Партии Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыбВСД) Тогда
			Продолжить;	
		КонецЕсли;
		_Статус = СокрЛП(кб99_ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(ВыбВСД,"Статус"));
		_applicationID = кб99_ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(ВыбВСД,"applicationID");
...
Функция ВнутреннееПеремещение() Экспорт
	...
	//Ответ = кб99_ВСД_Запросы.ВСД2_транзакция_Отправить( ПараметрыОрганизации, ДокВСД, ТаблицаВсдДляГашения );
	//
...
&НаСервере
Функция ОформитьЛабИсследованияНаСервере(АдресВХ) 

	тзПартии = Новый ТаблицаЗначений;
	тзПартии.Колонки.Добавить("Продукция");
	тзПартии.Колонки.Добавить("ДатаИзготовления1");
	тзПартии.Колонки.Добавить("ВсдДата");
	тзПартии.Колонки.Добавить("Количество");
	тзПартии.Колонки.Добавить("Партия");
	тзПартии.Колонки.Добавить("НомерЗаписи");
	тзПартии.Колонки.Добавить("СтатусПартии");
	тзПартии.Колонки.Добавить("ЛабИсследования");
	тзПартии.Колонки.Добавить("СтатусЗапроса");
	
	Для Каждого Стр Из Объект.Партии Цикл
		Если Стр.Количество = Стр.КоличествоСписания И СокрЛП(Стр.Партия.Статус) = "103" ИЛИ СокрЛП(Стр.Партия.Статус) = "102" Тогда
			НоваяСтрока = тзПартии.Добавить();
			НоваяСтрока.Продукция = Стр.ВСД_Продукция_Элемент;
			НоваяСтрока.ДатаИзготовления1 = Стр.Партия.ДатаИзготовления1;
			НоваяСтрока.ВсдДата = Стр.Партия.ВсдДата;
			НоваяСтрока.Количество = Стр.Партия.Количество;
			НоваяСтрока.Партия = Стр.Партия;
			НоваяСтрока.НомерЗаписи = Стр.Партия.НомерЗаписи;
			НоваяСтрока.СтатусПартии = Стр.Партия.Статус;
			ДокЛИ = Документы.ВСД2_ЛабораторныеИсследования.НайтиПоРеквизиту("Партия", стр.Партия);
			Если НЕ ДокЛИ.Пустая() И НЕ ДокЛИ.ПометкаУдаления Тогда 
				НоваяСтрока.ЛабИсследования = ДокЛИ;
			    НоваяСтрока.СтатусЗапроса = ДокЛИ.Статус;
			КонецЕсли;
	    КонецЕсли;
	КонецЦикла;
	
	АдресВХ = ПоместитьВоВременноеХранилище(тзПартии, Новый УникальныйИдентификатор());
	АдресТз = Новый Структура("АдресВХ", АдресВХ);
	Возврат АдресТЗ;
...
Процедура кнОформитьЛабИсследованияОтвет(Ответ,Парам) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		АдресВХ = "";
 		АдресТЗ = ОформитьЛабИсследованияНаСервере(АдресВХ);
		ЛабИсследования = Обработки.ВСД_ЛабИсследования.Создать();
		формЛаб = ОткрытьФорму("Обработка.ВСД_ЛабИсследования.Форма.Форма",АдресТЗ);
	КонецЕсли;		
КонецПроцедуры
