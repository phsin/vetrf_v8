...
Функция СоздатьВСДПроизводствоНаСервере( ВыбДата, ПараметрыОрганизации )
	...
	ДокВСД.ЗавершитьОперацию = Не ПараметрыОрганизации["НеЗавершатьПроизводство"];

	Если ПараметрыОрганизации.Свойство("НомерНезавершенногоПроизводства") Тогда
		Если ПараметрыОрганизации.Свойство("ЗавершитьПроизводство") Тогда
			ДокВСД.ЗавершитьОперацию = Истина;
		Иначе
			ДокВСД.operationId = ПараметрыОрганизации.НомерНезавершенногоПроизводства;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ДокВСД.ЗавершитьОперацию Тогда
		ДокВСД.operationId = "НЗП-"+кб99_ВСД_Запросы.ДатаXML(ВыбДата);	
	КонецЕсли;
...
Процедура кнПроизводство(Команда)
	...
	Если ПараметрыОрганизации["НеЗавершатьПроизводство"] Тогда
		...
		ИначеЕсли НезавершенноеПроизводство.НомерНезавершенногоПроизводства <> Неопределено Тогда
			ПараметрыОрганизации.Вставить( "НомерНезавершенногоПроизводства", НезавершенноеПроизводство.НомерНезавершенногоПроизводства );
			ТекстВопроса = "Добавить недостающую продукцию в ранее оформленное незавершенное производство ?";
			ПроизводствоВопрос(ТекстВопроса, ПараметрыОрганизации);
		Иначе
			ТекстОповещения = "При работе в режиме ""Незавершенное производство"" в информационной базе должен быть только один документ ""ВСД2 Производство"" со статусом ""Незавершенное производство""!";
			ПоказатьПредупреждение(,ТекстОповещения,60,"Ошибка");
		КонецЕсли;
...
Функция НайтиНезавершенноеПроизводство()
	...
	Если Выборка.Количество() = 1 Тогда
		...
		Выборка.Следующий();
		НомерНезавершеннойОперации = Выборка.operationId;
		
	ИначеЕсли Выборка.Количество() > 1 Тогда
		
		ТекстСообщения = "В информационной базе имеется несколько документов со статусом [Незавершенное производство]";
		кб99_Всд.СообщитьИнфо(ТекстСообщения);
		НомерНезавершеннойОперации = Неопределено;
	...
	Результат.Вставить( "НомерНезавершенногоПроизводства", НомерНезавершеннойОперации );
