...
Процедура ЗаполнитьТаблицуПартийНаСервере( ЗнакСортировки=Неопределено ) Экспорт
	...
	Для Каждого СтрОтгрузки Из Объект.Отгрузки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрОтгрузки.Грузополучатель) Тогда
			...
			Если Объект["РеквизитГрузополучатель"] = 0 Тогда
				ПолеГрузополучатель = "Контрагент";        
			ИначеЕсли Объект["РеквизитГрузополучатель"] = 1 Тогда
				ПолеГрузополучатель = "АдресДоставки";
			ИначеЕсли Объект["РеквизитГрузополучатель"] = 2 Тогда
				ПолеГрузополучатель = "Партнер";
			ИначеЕсли Объект["РеквизитГрузополучатель"] = 3 Тогда
				ПолеГрузополучатель = "Грузополучатель";
			Иначе	
				ПолеГрузополучатель = "Договор";
			КонецЕсли;
		...
		Для Каждого стрТЗ Из ТЗ Цикл
			...
			СтрПартий.ИдентификаторСтроки = стрТЗ.ИдентификаторСтроки;
		КонецЦикла;
	...
	Для Каждого стрНужныеПартии Из ВремПартии Цикл
		...
		Если СтрокиПартий.Количество() = 0  Тогда
			...
			Исключение
			КонецПопытки;
			НоваяСтрока.ИдентификаторСтроки = стрНужныеПартии.ИдентификаторСтроки;
...
Функция ОтправитьВСД_НаСервере( ОтправитьБыстро )
	...
	Иначе;
		...
		Иначе
			...
			//Проверить условия перевозки перед отправкой
			...
			Для Каждого ДокСсылка Из ДокументыДляПроверки Цикл
				...
				Если ЗначениеЗаполнено( applicationID ) Тогда				
					Ответ =  кб99_ВСД_Запросы.ВСД2_Транзакция_УсловияПеревозки_ПолучитьОтвет( ПараметрыФункции, applicationID, ДокСсылка, Ложь);
				КонецЕсли;
			КонецЦикла;
            //Конец "Проверить условия перевозки перед отправкой"
...
Функция ПечатьРеестра() Экспорт
	...
	Для Каждого стр Из Объект.Отгрузки Цикл				
		...
		Если Объект.РеквизитГрузополучатель = 0 Тогда     
			...
			МодульУправлениеКонтактнойИнформацией = кб99_ВСД_Общий.ОбщийМодуль("УправлениеКонтактнойИнформацией");
			
			Если МодульУправлениеКонтактнойИнформацией <> Неопределено Тогда
				СведенияОГрузополучателе  = МодульУправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(стр.Грузополучатель,  ТекущаяДата());
				СтрокаАдрес = "";
...
&НаСервере
Функция ПолучитьСкладПеремещенияНаСервере( ДокСсылка )
	
	ПереопределяемыеПоляЗапроса = кб99_ВСД_Общий.ПолучитьПредставлениеПолейДокументаВЗапрос();
	Возврат ДокСсылка[ПереопределяемыеПоляЗапроса.ПолеСкладПолучатель];
...
Процедура ОтгрузкиПлощадкаПриИзменении(Элемент)
	...
	ОтгрузкиПлощадкаПриИзмененииНаСервере( Элемент.Родитель.ТекущиеДанные.Площадка, НовыйГрузополучатель );
...
&НаКлиенте
Функция СоздатьВСДПроизводствоКлиент( ВыбДата, ПараметрыОрганизации )
...
Процедура ПослеВводаДатыПроизводство(ВыбДата, ПараметрыОрганизации) Экспорт
	...
	Если ВыбДата = Неопределено Тогда
		...
		Возврат;
	Иначе		
		ДокВСД = СоздатьВСДПроизводствоКлиент(ВыбДата, ПараметрыОрганизации);		
	КонецЕсли;
...
// Старый вариант
&НаКлиенте
Процедура кнПроизводство_(Команда)
...
// старый вариант
&НаСервере
Функция НайтиНезавершенноеПроизводство_()
...
Функция ПолучитьАктуальнуюВерсиюКэш()
	...
	Возврат кб99_ВСД_Отправка.ПолучитьАктуальнуюВерсиюСервер(Объект.Организация);
	...
	Для Каждого стрДок Из Объект.Отгрузки Цикл
		Если Стрдок.Отметка Тогда
			...
			Для Каждого Файл Из ПрисоединенныеФайлыМассив Цикл
				...
				кб99_ВСД_Общий.СообщенияПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Прикреплен файл %1 к документу %2", ВремФайл, стрДок.Док));
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
