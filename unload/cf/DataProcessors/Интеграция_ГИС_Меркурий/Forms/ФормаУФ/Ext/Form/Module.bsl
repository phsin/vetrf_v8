...
Функция кнХозСубъект_ПолучитьGuidПоИНН_НаСервере(ВыбХозСубъект) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыФункции["ОтправлятьВФоне"]) и ПараметрыФункции["ОтправлятьВФоне"] Тогда 
...
Процедура ЗагрузитьПлощадкиСпискомПоХозСубъекту_Сервер( )
	...
	тзСвертка = ФОбъект.Отгрузки.Выгрузить();
	тзСвертка.Свернуть("Отметка, Контрагент, ХозСубъект",);
	ПараметрыФункции = ВСД_Общий.ПараметрыСервер( ФОбъект );	
	ВСД_Запросы.Площадки_ЗагрузитьСписокПоХозСубъекту( ПараметрыФункции, тзСвертка );
	//ФОбъект.РаскраситьТЧОтгрузки();
	
	//отменяем пометки в ТЧ
	Для каждого ДанныеСтроки из ФОбъект.Отгрузки Цикл
