...
Процедура ПолучитьАктуальнуюВерсиюИзХранилища()
	...
	Если ТипЗнч(ОтветСервераКэш) = Тип("Структура") Тогда
		АктуальнаяВерсия =  ОтветСервераКэш.АктуальнаяВерсия.Версия;
		СсылкаНаОписаниеАктуальнойВерсии = ОтветСервераКэш.АктуальнаяВерсия.Описание;
		
		ИнформацияОДоговореТехподдержки = ?(ОтветСервераКэш.ДоговорДействует, СтрШаблон("Договор техподдержки действует до %1.", Формат(ОтветСервераКэш.ДатаОкончанияПоддержки, "ДФ=dd.MM.yyyy")),
																						"Нет активного договора техподдержки.");
		ИнформацияОНеобходимостиОбновления = ?(ВерсияОбработки<АктуальнаяВерсия, "Доступно обновление, актуальная версия:", ""); 
		
		Элементы.НадписьИнформацияОТехподдержке.Заголовок = СтрШаблон("%1 %2", ИнформацияОДоговореТехподдержки, ИнформацияОНеобходимостиОбновления); 
...
Процедура ПолучитьИнформациюОбОбновленияхНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Организация) И ЗначениеЗаполнено(ЛогинВетис) Тогда
		...
		Если ТипЗнч(ОтветСервера) = Тип("Структура") И Не ОтветСервера.Ошибка Тогда
			ЕстьИнформацияПоАктуальнойВерсии = ЗначениеЗаполнено(ОтветСервера.АктуальнаяВерсия.Версия);
			АктуальнаяВерсия = ?(ЕстьИнформацияПоАктуальнойВерсии, ОтветСервера.АктуальнаяВерсия.Версия, "0");
			Если ЕстьИнформацияПоАктуальнойВерсии Тогда
				кб99_ВСД.СохранитьАктуальнуюВерсию(ОтветСервера);	
			КонецЕсли;
			СсылкаНаОписаниеАктуальнойВерсии = ?(ЕстьИнформацияПоАктуальнойВерсии, ОтветСервера.АктуальнаяВерсия.Описание, "");
			
			ИнформацияОДоговореТехподдержки = ?(ОтветСервера.ДоговорДействует, СтрШаблон("Договор техподдержки действует до %1.", Формат(ОтветСервера.ДатаОкончанияПоддержки, "ДФ=dd.MM.yyyy")),
			"Нет активного договора техподдержки.");
			ИнформацияОНеобходимостиОбновления = ?(ЕстьИнформацияПоАктуальнойВерсии, ?(ВерсияОбработки<АктуальнаяВерсия, "Доступно обновление, актуальная версия:", ""), ""); 
...
//&НаСервере
//Процедура ПолучитьДанныеОбАктуальнойВерсии()
//	
...
//			ПараметрыЗапроса.Вставить("ТекущаяВерсия",			 ВерсияОбработки);
//			
//			ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(АдресХранилища);
//			ПараметрыВыполнения.АдресРезультата = АдресХранилища; 
//			
//			ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне("кб99_ВСД_Отправка.ПолучитьДанныеОтехподдержке", ПараметрыЗапроса, ПараметрыВыполнения);
//					
//		Иначе
//			Элементы.НадписьИнформацияОТехподдержке.Заголовок = "";
//			Элементы.АктуальнаяВерсия.Видимость = Ложь;
//		КонецЕсли;
//	Исключение
//		Элементы.НадписьИнформацияОТехподдержке.Заголовок = "Сервер не доступен. Нет данных об актуальной версии.";
//	КонецПопытки;
//	
//КонецПроцедуры
...
//	
//	ОтветСервера = ПолучитьИзВременногоХранилища(АдресХранилища);
//	
