...
Процедура кнОтметитьСОшибкойСервер()
	Для Каждого стр Из Объект.Отгрузки Цикл
		...
		СтатусДок = кб99_ВСД_Общий.НайтиПоследнийЗапрос( стр.ВСД ).СтатусЗапроса;
...
//	ЗаполнитьТаблицуПартийНаСервере( ВыбЭлемент );
//	ПоказатьОповещениеПользователя("Выполнено");
...
Процедура СоздатьВсдНаСервере()
	// Создает ВСД по списку из тч Отгрузки
 	// С учетом актуальных Партий, за минусом неотправленных ВСД из списка документов
	...
	тзАктуальныхПартий = кб99_ВСД.ПолучитьАктуальныеПартии( СписокКонстант, Объект.Партии.Выгрузить( ).ВыгрузитьКолонку("ВСД_Продукция_Элемент"), Объект.Отправитель_Площадка, Объект.Отправитель_ХозСубъект );
...
Процедура кнСоздатьВСДОтвет(Ответ,Парам) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ЭтаФорма.ТекущийЭлемент = Элементы.ОтправитьВсдПоследовательно;
	КонецЕсли;		
КонецПроцедуры
