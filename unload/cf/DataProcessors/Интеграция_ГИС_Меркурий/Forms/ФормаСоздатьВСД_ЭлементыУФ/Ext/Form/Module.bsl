...
Функция СоздатьВСДЭлементы() Экспорт
	...
	Если (Не ЗначениеЗаполнено(Объект.Отправитель_Площадка)) или (Не ЗначениеЗаполнено(Объект.Организация)) Тогда
		ВСД.СообщитьИнфо("Укажите Организацию и Площадку");
		Возврат "";
	КонецЕсли;
	...
	Для каждого СтрЭлемент из Объект.ВСДЭлементы Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция) или
			...
			НЕ ЗначениеЗаполнено(СтрЭлемент.ЕдиницаИзмерения)	Тогда
				ВСД.СообщитьИнфо("Обязательные реквизиты для "+СтрЭлемент.Номенклатура+" в строке "+СтрЭлемент.номерСтроки+" не заполнены !!!");
				Продолжить;
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция_Элемент) Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция_Элемент) Тогда
				ВСД.СообщитьИнфо("Что-то пошло не так. Не создался Продукция Элемент для "+СтрЭлемент.Номенклатура);
				Продолжить;
			КонецЕсли;
		...
		Если НЕ (СтрЭлемент.Продукция_Элемент.Площадка = Объект.Отправитель_Площадка) Тогда
			ВСД.СообщитьИнфо("Не совпадает Площадка с установленной у "+СтрЭлемент.Продукция_Элемент+" -> ПРОПУСКАЮ");
			Продолжить;
		КонецЕсли;
		
		Попытка 
			ВСД_Запросы.Продукция_Элемент_Изменить( ПараметрыОрганизации, СтрЭлемент.Продукция_Элемент,"CREATE");
			СтрЭлемент.GUID = СтрЭлемент.Продукция_Элемент.GUID;
		Исключение
...
Функция ИзменитьВСДЭлементы(ИмяКоманды = "UPDATE") Экспорт
	...
	Для каждого СтрЭлемент из Объект.ВСДЭлементы Цикл
		...
		Если НЕ(ЗначениеЗаполнено(СтрЭлемент.GUID)) Тогда
			ВСД.СообщитьИнфо("В строке "+стрЭлемент.НомерСтроки+" нет элемента для изменения или удаления в Меркурий, пропускаю.");
			Продолжить;
		КонецЕсли;
		...
		Если Не(ИмяКоманды = "DELETE") Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция) или
				...
				НЕ ЗначениеЗаполнено(СтрЭлемент.Номенклатура) или 
				НЕ ЗначениеЗаполнено(СтрЭлемент.ЕдиницаИзмерения)	Тогда
					ВСД.СообщитьИнфо("Обязательные реквизиты не заполнены !!!");
					Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Попытка 
			ВСД_Запросы.Продукция_Элемент_Изменить( ПараметрыОрганизации, СтрЭлемент.Продукция_Элемент, ИмяКоманды );
			СтрЭлемент.GUID = СтрЭлемент.Продукция_Элемент.GUID;
		Исключение
			ВСД.СообщитьИнфо("Что-то пошло не так. Не удалось создать в Меркурий "+СтрЭлемент.Продукция_Элемент);
			ВСД.СообщитьИнфо(ОписаниеОшибки());
...
Процедура кнЗаполнить(Команда)
	...
	Если (НЕ ЗначениеЗаполнено(ВыбНоменклатура)) или
		...
		ВСД.СообщитьИнфо("Не все параметры для заполнения указаны");
		Возврат;
	КонецЕсли;
	ЗаполнитьНоменклатуруВТЗ();
...
Функция ПолучитьСведенияОПродЭлементе( ВыбНоменклатура ) 	
	//Фобъект = РеквизитФормыВЗначение("Объект");
	НайденЭлемент = ВСД.Получить_ВСД_Продукция_Элемент( ВыбНоменклатура );
	//ЗначениеВРеквизитФормы(Фобъект,"Объект");
