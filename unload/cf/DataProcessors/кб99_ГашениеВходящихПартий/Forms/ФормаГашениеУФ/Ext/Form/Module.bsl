...
&НаСервере
Функция Гашение_НайтиСрокГодностиПоСерии( СтрокаВСДВходящие )
	Ответ = 0;
	
	тзСерии = СтрокаВСДВходящие.ДокументПрихода.Серии.Выгрузить();
	Для каждого строкаСерия Из тзСерии Цикл
		Если строкаСерия.Номенклатура =  СтрокаВСДВходящие.Номенклатура Тогда 
			Если ЗначениеЗаполнено( СтрокаВСДВходящие.ДатаСрокГодности2 ) Тогда 
				Если строкаСерия.Серия.ГоденДо >= СтрокаВСДВходящие.ДатаСрокГодности1 И 
					 строкаСерия.Серия.ГоденДо <= СтрокаВСДВходящие.ДатаСрокГодности2 Тогда 
					 
					 Ответ = Ответ + строкаСерия.Количество;
					 СтрокаВСДВходящие.ДатаСрокГодности1С = строкаСерия.Серия.ГоденДо;
					 
				 КонецЕсли;
			 Иначе 
				 Если строкаСерия.Серия.ГоденДо = СтрокаВСДВходящие.ДатаСрокГодности1 Тогда 
		...
		Если ЗначениеЗаполнено(СтрокаВСДВходящие.ДокументПрихода) Тогда 
			...
			Если ЗначениеЗаполнено( СтрокаТовары ) Тогда 
				...
				Если КоличествоСерия > 0 Тогда 				
					СтрокаВСДВходящие.КоличествоПринято1С = КоличествоСерия;
				Иначе					
