...
Процедура ПриЗакрытииФормыВыбора(РезультатВыбора, ДопПараметры) Экспорт
	...
	Если КоличествоВыбранныхСтрок = 0 Тогда
		кб99_ВСД.СообщитьПользователю("Не выбраны партии для установки документов прихода.");
	Иначе
		кб99_ВСД.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Для %1 партии подобраны документы.",
																		КоличествоВыбранныхСтрок));
	КонецЕсли;
