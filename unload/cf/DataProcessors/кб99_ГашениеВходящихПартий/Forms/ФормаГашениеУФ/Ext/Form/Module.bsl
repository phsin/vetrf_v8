...
//				стрТЧ.НомерУровня = Перечисления.кб99_УровниУпаковки.Получить(СтрУровня.НомерУровня-1);
...
//						стрТЧМ.Класс = Перечисления.кб99_Маркировка[стрМаркировки.Представление];//стрМаркировки.Представление;
...
//		Если ЗначениеЗаполнено(стрВход.ТочкиМаршрута) Тогда    
//			Маршруты = ЗначениеИзСтрокиВнутр(стрВход.ТочкиМаршрута);
//			Для Каждого точкаМаршрута Из Маршруты Цикл
//				стрТЧ = ДокОбъект.ТочкиМаршрута.Добавить();
...
Процедура кнПолучитьВСДНаСервере()
	...
	Для Каждого строкаВСД Из ВсдМассив Цикл
		...
		СтрокаВСДВходящие.ДокументПрихода = "";
		СтрокаВСДВходящие.КоличествоПринято1С = 111;
		СтрокаВСДВходящие.ДатаСрокГодности1С = "11.11.11";
...
Процедура ЗаполнитьВсдПоСправочнику()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	кб99_ВСД2.ДатаИзготовления1Строка,
	               |	кб99_ВСД2.ДатаИзготовления2Строка,
	               |	кб99_ВСД2.ДатаСрокГодности1Строка,
	               |	кб99_ВСД2.ДатаСрокГодности2Строка,
	               |	кб99_ВСД2.Страна
	               |ИЗ
	               |	Справочник.кб99_ВСД2 КАК кб99_ВСД2
