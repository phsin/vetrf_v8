...
Процедура ОтправкаЗапросовНаГашение( ПараметрыОрганизации = Неопределено )
	...
	Для Каждого СтрокаТЗ Из Объект.ВСДВходящие Цикл
		...
		Если ПараметрыОрганизации.ПривязкаВходящихПартийКДокументамПоступленияОбязательна Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаТЗ.ДокументПрихода) 
						И Не СтрокаТЗ.Отправитель_Хозсубъект.НеКонтролироватьНаличиеДокументаПоступление Тогда
...
Процедура ЗаполнитьВсдПоСправочнику()
	...
	|	кб99_ВСД2.ДокументПрихода КАК ДокументПрихода,
...
Процедура ГашениеЗаполнитьСтрокуТаблицы(СтрокаВСДВходящие, ПараметрыОрганизации)
	...
	Попытка
		ИскатьДокументПрихода = ПараметрыОрганизации["ПарамВсдВходящиеИскатьДокументПрихода"] 
			И НЕ СтрокаВСДВходящие.Отправитель_Хозсубъект.НеКонтролироватьНаличиеДокументаПоступление;
	Исключение
	...
	Если ИскатьДокументПрихода Тогда
		Если Не ЗначениеЗаполнено(СтрокаВСДВходящие.ДокументПрихода) Тогда
			...
			Если ПараметрыОрганизации.ПривязкаВходящихПартийКДокументамПоступленияОбязательна Тогда;
				ЗаписатьДокументПриходаВСправочникВСД2(СтрокаВСДВходящие.ДокВСД, СтрокаВСДВходящие.ДокументПрихода);
			КонецЕсли;
		КонецЕсли;	
...
Процедура ПриЗакрытииФормыВыбора(РезультатВыбора, ДопПараметры) Экспорт
	...
	Для Каждого Строка Из Объект.ВСДВходящие Цикл
		...
		СтрВСДВходящие = Новый Структура("ДокументПрихода,
			|Номенклатура,
			|КоличествоПринято1С,
			|Ошибки,
			|ДатаСрокГодности1С,
			...
			|ДатаИзготовления1,
			|ДатаИзготовления2,
			|СменаВладельцаБезПеревозки,
			|НомерАвто,
			|Количество,
			|ВСД_Партия,
			|Продукция_Элемент,
			|НаличиеЛабИсследований");
		ЗаполнитьЗначенияСвойств(СтрВСДВходящие, Строка);
		
		Гашение_ЗаполнитьСтрокуПоПоступлениеТоваров(СтрВСДВходящие);
		кб99_ВСД_Общий.ГашениеПроверитьСтрокуТаблицы(СтрВСДВходящие, ПараметрыОрганизации);
		
		ЗаполнитьЗначенияСвойств(Строка, СтрВСДВходящие);
		...
		Если ПараметрыОрганизации.ВыводитьПодробнуюИнформацию Тогда
			кб99_ВСД_Клиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Документ [%1] соспоставлен с ВСД [%2]",
				РезультатВыбора, Строка.ДокВСД), Строка.ДокВСД);
		КонецЕсли;
