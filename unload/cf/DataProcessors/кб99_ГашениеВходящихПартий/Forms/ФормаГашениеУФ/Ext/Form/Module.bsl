...
Процедура кнПолучитьВСДНаСервере()
	...
	Для Каждого строкаВСД Из ВсдМассив Цикл
		...
		ЗаполнитьСтрокуПоПоступлениеТоваров( СтрокаВСДВходящие );
...
&НаСервере
Функция НайтиДокументПоступлениеТоваров( ДатаВходящегоДокумента, НомерВходящегоДокумента)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
...
&НаСервере
Процедура ЗаполнитьСтрокуПоПоступлениеТоваров( СтрокаВСДВходящие )
	...
	Иначе
		кб99_ВСД.СообщитьИнфо("Не найдена номенклатура ["+СтрокаВСДВходящие.Номенклатура+"] в документе ["+СтрокаВСДВходящие.ДокументПрихода+"]" );
	КонецЕсли;
...
Процедура ГашениеЗаполнитьСтрокуТаблицы( СтрокаВСДВходящие )
	...
	СтрокаВСДВходящие.ДокументПрихода = НайтиДокументПоступлениеТоваров( СтрокаВСДВходящие.ДокВСД.ТтнДата, СтрокаВСДВходящие.ДокВСД.ТтнНомер );
