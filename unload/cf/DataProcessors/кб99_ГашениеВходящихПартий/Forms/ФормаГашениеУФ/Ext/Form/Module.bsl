...
Процедура ОтправкаДокументовВсдОбновитьТранспорт()
	...
	Для Каждого СтрокаТЗ Из Объект.ВСДВходящие Цикл 
		...
		ПараметрыФункции.Вставить("ДокОснование", СтрокаТЗ.ДокОбновитьТранспорт);
...
Функция ОформитьЗакупкуУПоставщикаНаСервере()
	...
	Для Каждого ТТН Из тзПоТТН Цикл
		...
		кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Записан документ %1", ДокументПрихода.Ссылка), ДокументПрихода.Ссылка);
	КонецЦикла;
