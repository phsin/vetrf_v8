...
&НаКлиенте
Процедура кнГашениеСВХ(Команда)
	
	массивВСД = Новый Массив;
	Для Каждого стр из Объект.ВСДВходящие Цикл
		Если НЕ ЗначениеЗаполнено(стр.Отправитель_Площадка) Тогда
        	СтрПартий = Новый Структура("ДокВСД, Количество, ЕдиницаИзмерения");
...
Процедура ПослеРаспределенияПродукцииСВХ(РезультатЗакрытия, допПараметры = неопределено) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		Объект.ВСДВходящие.Очистить();
		Для Каждого ВСД из РезультатЗакрытия Цикл
			ЗаполнитьВСДВходящиеРаспределеннойПродукцией(ВСД.ДокВСД);
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьВСДВходящиеРаспределеннойПродукцией(ВСД)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	кб99_ВСД2.ОсобыеОтметки КАК ОсобыеОтметки,
	               ...
	               |	кб99_ВСД2.НаименованиеПродукции КАК НаименованиеПродукции,
