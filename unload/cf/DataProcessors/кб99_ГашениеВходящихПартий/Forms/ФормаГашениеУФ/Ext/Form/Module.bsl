...
Процедура ЗаполнитьВсдПоСправочнику()
	...
	Для Каждого строкаВыборка Из Выборка Цикл
		...
		ГашениеЗаполнитьСтрокуТаблицы( СтрокаВСДВходящие, ПараметрыОрганизации["ПарамВсдВходящиеИскатьДокументПрихода"] );
...
&НаСервере
Процедура ГашениеПроверитьСтрокуТаблицы( СтрокаВСДВходящие, ИскатьДокументПрихода )
	
	Если ИскатьДокументПрихода Тогда 
		Если не ЗначениеЗаполнено( СтрокаВСДВходящие.ДокументПрихода ) Тогда
			СтрокаВСДВходящие.Ошибки = "Документа ПриходнаяНакладная не найден";
