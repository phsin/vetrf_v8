...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	// Замечены вылеты 1с на обычных формах при закрытии обработки
...
&НаКлиенте
Процедура ОбновитьТраспорт(Команда)
	
	тзВсд = Новый Массив;
	
	Для Каждого стрВходящие Из объект.ВСДВходящие Цикл
		Если стрВходящие.Отметка Тогда 
			СтрВСД = Новый Структура("ДокВСД, ТтнНомер");
			ЗаполнитьЗначенияСвойств( СтрВсд, стрВходящие );
			тзВсд.Добавить( СтрВсд );
		КонецЕсли;
	КонецЦикла;
	
	Если тзВсд.Количество()>0 Тогда 
		
		ПараметрыДокумента = Новый Структура("Организация, Получатель_Площадка", Объект.Организация, Объект.Отправитель_Площадка  );
		ЭлементыОтбора = Новый Структура("ПараметрыДокумента, ДокВсд", ПараметрыДокумента, тзВсд);
		ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЭлементыОтбора);

		ОткрытьФорму("Документ.ВСД2_ОбновитьТранспорт.ФормаОбъекта", ПараметрыФормы );
				
	Иначе
		Сообщить("ВСД не выбраны");
	КонецЕсли;
