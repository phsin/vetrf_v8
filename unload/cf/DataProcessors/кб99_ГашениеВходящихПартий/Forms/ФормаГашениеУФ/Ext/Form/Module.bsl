...
Процедура ПриЗакрытииФормыВыбора(РезультатВыбора, ДопПараметры) Экспорт
	...
	Если КоличествоВыбранныхСтрок = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбраны партии для установки документов прихода.'"));
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Для %1 партии подобраны документы.'"), КоличествоВыбранныхСтрок));
	КонецЕсли;
