...
Процедура ЗагрузитьВСД(Команда)
	...
	ИначеЕсли Не ЗначениеЗаполнено(ТипВСД) Тогда
		
		кб99_ВСД.СообщитьПользователю("Выберите тип ВСД",, "ТипВСД",, Отказ);
	...
	ПараметрыЗапроса.Вставить("vetDocumentType", 		ТипВСД);	
	ПараметрыЗапроса.Вставить("vetDocumentStatus", 		"");	
	ПараметрыЗапроса.Вставить("Смещение", 				0);	
	ПараметрыЗапроса.Вставить("ПоступилиС",  			ПериодЗагрузкиВСД.ДатаНачала);	
	ПараметрыЗапроса.Вставить("ПоступилиПо",			ПериодЗагрузкиВСД.ДатаОкончания);	
	...
	кб99_ВСД_Запросы.ПолучитьСписокВСД(кб99_ВСД.ЗагрузитьПараметры(Объект.Организация), ПараметрыЗапроса);
...
Процедура кнПолучитьОтветВетисНаСервере()
	...
	applicationID = кб99_ВСД_Общий.НайтиApplicationID( ВыбранныйДокумент );
	
	Ответ =  кб99_ВСД_Запросы.ПолучитьРезультат_ВСД2( ПараметрыОргнаизации_,, applicationID, ВыбранныйДокумент, Ложь);
