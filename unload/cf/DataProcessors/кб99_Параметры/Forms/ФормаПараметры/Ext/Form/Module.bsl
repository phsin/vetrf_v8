...
Функция кнЗагрузитьУлицыНаСервере()
	...
	Если Объект.ОтправлятьВФоне Тогда 
		...
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Ответ = кб99_ВСД_Запросы.ИнициализацияХС_ЗагрузитьУлицы( ПараметрыФункции, ?(ЗначениеЗаполнено(Объект.НаселенныйПункт), Объект.НаселенныйПункт, Объект.Город ));
		Результат = Новый Структура;
		Результат.Вставить("ЗаданиеВыполнено", Истина);
	КонецЕсли; 
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗначениеОтбора = Новый Структура("Владелец", ?(ЗначениеЗаполнено(Объект.НаселенныйПункт), Объект.НаселенныйПункт, Объект.Город));
	ПараметрыОткрытия = Новый Структура("Отбор, ЗакрыватьПриВыборе", ЗначениеОтбора, Истина);
