...
&НаСервере
Процедура кнЗаполнитьПоСпецификацииСервер()
	ЭлементыСпецификации = ВСД.СформироватьТЧСырьяДляСписанияВПроизводствоПоСписку(Объект.Продукция.Выгрузить());
	Объект.ПартииСписания.Загрузить(ЭлементыСпецификации);
КонецПроцедуры

&НаКлиенте
Процедура кнЗаполнитьПоСпецификации(Команда)
	кнЗаполнитьПоСпецификацииСервер()
КонецПроцедуры

&НаСервере
Процедура кнАвтоподборПартийПоСпецификацииНаСервере()
	...
	Сырье = ВСД.СформироватьТЧСырьяДляСписанияВПроизводствоПоСписку(Объект.Продукция.Выгрузить());
	ПартииСписания = ВСД.ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементовОбщМодуль(Парам,Сырье);
	Объект.ПартииСписания.Загрузить(ПартииСписания);
КонецПроцедуры

&НаКлиенте
Процедура кнАвтоподборПартийПоСпецификации(Команда)
	кнАвтоподборПартийПоСпецификацииНаСервере();
КонецПроцедуры
