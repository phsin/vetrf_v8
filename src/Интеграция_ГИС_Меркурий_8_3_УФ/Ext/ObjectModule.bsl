...
//    Попытка Запрос.УстановитьПараметр("ПлощадкаОтправителяСклад",    ПлощадкаОтправителя.Склад); Исключение КонецПопытки;
    ...
    Если РеквизитГрузополучатель = 0 Тогда
        ...
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "@Грузополучатель_Связи@", " (РеализацияТоваровУслуг.Контрагент = ВСД_Площадка.Контрагент) И (ВСД_Площадка.ПометкаУдаления = ЛОЖЬ) ");
    ...
    Иначе // Грузополучатель
        ...
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "@Грузополучатель_Связи@", " (ПОДСТРОКА(РеализацияТоваровУслуг.АдресДоставки, 1, 100) = ВСД_Площадка.Контрагент) ") ;
