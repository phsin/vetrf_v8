	...
	Пока ПолученоЗаписей = 1000 Цикл
		...
		Попытка
			...
			Для каждого enterprise из enterpriseList Цикл		
				...
				//ВСД.Установить_Соответствие( ХС, Объект_ВСД_Площадка.Ссылка );
			КонецЦикла;
		Исключение
...
Функция УдалитьПолучитьАктуальныеПартии(ВыбПродукция_Элемент, ВыбПлощадка,ВыбХозСубъект, ПолеСортировки = "", ЗнакСортировки = неопределено) Экспорт
...
Функция УдалитьПодобратьПартииПоПродукцияЭлемент(ТзПартий,ПродукцияЭлемент,КолвоСписания) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ПродукцияЭлемент) Тогда 
		Сообщить("Не указан ПродукцияЭлемент "+ПродукцияЭлемент);
		возврат Рез;
	КонецЕсли;
...
Функция УдалитьПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(Парам,тзЭлементыкСписанию, тзАктуальныхПартий = "") Экспорт
...
// вызов переопределения
	Обработка = ФункцияПереопределена("ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов");
	Если обработка <> Неопределено Тогда
		Возврат Обработка.ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(ЭтотОбъект, Парам,тзЭлементыкСписанию, тзАктуальныхПартий);
	КонецЕсли;	
	...
	Если НЕ ЗначениеЗаполнено(тзАктуальныхПартий) Тогда
		Попытка
			тзАктуальныхПартий = ВСД.ПолучитьАктуальныеПартии(тзЭлементыкСписанию.ВыгрузитьКолонку("Продукция_Элемент"), Парам.Отправитель_Площадка, Парам.Отправитель_ХозСубъект, СписокКонстант);
		Исключение
			Сообщить("Не переданы параметры для получения Актуальных партий - ДанныеЗаполнения или тзЭлементыОснования");
