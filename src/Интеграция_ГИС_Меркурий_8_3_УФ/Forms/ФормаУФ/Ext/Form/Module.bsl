...
&НаКлиенте
Процедура кнЗагрузитьПлощадкиПоНаименованиюОтвет(Результат, Парам)
	Если Результат = КодВозвратаДиалога.Да Тогда
		ПоказатьОповещениеПользователя("Выполняется загрузка Площадок",,"Ожидайте...",БиблиотекаКартинок.Информация32);
		кнЗагрузитьПлощадкиПоНаименованиюНаСервере(Парам.ВыбХС,Парам.ВыбРегион,Парам.ВыбНазвание);
		ПоказатьОповещениеПользователя("Выполнено");
	КонецЕсли;
КонецПроцедуры
...
Процедура кнЗагрузитьПлощадкиПоНаименованию(Команда)
	...
	Если ВвестиЗначение(ПоНаименованию,"Название контрагента","Строка")=0 Тогда
		Возврат;
	КонецЕсли;
	ТекстВопроса = "Загружаем все площадки региона "+Объект.Регион+" 
	| содержащие "+ПоНаименованию+". 
	|При отсутствии явного указания ХС в ответе Меркурий
	|Площадки будут привязаны к "+ВыбранныйХС+" ?";
	
	Парам = Новый Структура("ВыбХС,ВыбРегион,ВыбНазвание",ВыбранныйХС,Объект.Регион,ПоНаименованию);
    Оповещение = Новый ОписаниеОповещения("кнЗагрузитьПлощадкиПоНаименованиюОтвет", ЭтотОбъект,  Парам); 
...
Процедура кнОтправитьВСД(Команда)
	...
	Для каждого СтрОтгрузка Из Объект.Отгрузки Цикл
		...
		Если НЕ(ЗначениеЗаполнено(_ВСД)) или НЕ(СтрОтгрузка.Отметка) Тогда 		
			Продолжить;    
		КонецЕсли;
		//ЖД Переделать - много вызовов сервера !!!!
		_Проведен = ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(_ВСД,"Проведен");
		_Статус = СокрЛП(ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(_ВСД,"Статус"));
		_ПометкаУдаления = ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(_ВСД,"ПометкаУдаления");
		_applicationID = ВСД.ПолучитьЗначениеРевизитаОбъекта_НаСервере(_ВСД,"applicationID");
