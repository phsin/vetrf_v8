...
Функция СоздатьВСДЭлементы() Экспорт
	...
	Если (Не ЗначениеЗаполнено(Объект.Отправитель_Площадка)) или (Не ЗначениеЗаполнено(Объект.Организация)) Тогда
		Сообщить83("Укажите Организацию и Площадку");
		Возврат "";
	КонецЕсли;
	...
	Для каждого СтрЭлемент из ФОбъект.ВСДЭлементы Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция) или
			...
			НЕ ЗначениеЗаполнено(СтрЭлемент.ЕдиницаИзмерения)	Тогда
				Сообщить83("Обязательные реквизиты для "+СтрЭлемент.Номенклатура+" в строке "+СтрЭлемент.номерСтроки+" не заполнены !!!");
				Продолжить;
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция_Элемент) Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция_Элемент) Тогда
				Сообщить83("Что-то пошло не так. Не создался Продукция Элемент для "+СтрЭлемент.Номенклатура);
				Продолжить;
			КонецЕсли;
		...
		Если НЕ (СтрЭлемент.Продукция_Элемент.Площадка = Объект.Отправитель_Площадка) Тогда
			Сообщить83("Не совпадает Площадка с установленной у "+СтрЭлемент.Продукция_Элемент+" -> ПРОПУСКАЮ");
			Продолжить;
		КонецЕсли;
		...
		Попытка 
			...
			СтрЭлемент.GUID = СтрЭлемент.Продукция_Элемент.GUID;
		Исключение
			Сообщить83("Что-то пошло не так. Не удалось создать в Меркурий "+СтрЭлемент.Продукция_Элемент);
...
Функция ИзменитьВСДЭлементы(ИмяКоманды = "UPDATE") Экспорт
	...
	Для каждого СтрЭлемент из Объект.ВСДЭлементы Цикл
		...
		Если НЕ(ЗначениеЗаполнено(СтрЭлемент.GUID)) Тогда
			Сообщить83("В строке "+стрЭлемент.НомерСтроки+" нет элемента для изменения или удаления в Меркурий, пропускаю.");
			Продолжить;
		КонецЕсли;
		...
		Если Не(ИмяКоманды = "DELETE") Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция) или
				...
				НЕ ЗначениеЗаполнено(СтрЭлемент.Номенклатура) или 
				НЕ ЗначениеЗаполнено(СтрЭлемент.ЕдиницаИзмерения)	Тогда
					Сообщить83("Обязательные реквизиты не заполнены !!!");
					Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Процедура кнЗаполнить(Команда)
	...
	Если (НЕ ЗначениеЗаполнено(ВыбНоменклатура)) или
		...
		Сообщить83("Не все праметры для заполнения указаны");
		Возврат;
	КонецЕсли;
	ЗаполнитьНоменклатуруВТЗ();
...
Процедура кнСоздать(Команда)
    ...
    Оповещение = Новый ОписаниеОповещения("кнСоздатьОтвет",ЭтотОбъект);	
...
Процедура кнИзменить(Команда)
	...
	|по площадке "+Объект.Отправитель_Площадка;
    Оповещение = Новый ОписаниеОповещения("кнИзменитьОтвет",ЭтотОбъект);	
    ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,  0, КодВозвратаДиалога.Да, ""   );    
КонецПроцедуры
...
Процедура кнУдалить(Команда)
	...
	|по площадке "+Объект.Отправитель_Площадка;
    Оповещение = Новый ОписаниеОповещения("кнУдалитьОтвет",ЭтотОбъект);	
    ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,  0, КодВозвратаДиалога.Да, ""   );    
КонецПроцедуры
