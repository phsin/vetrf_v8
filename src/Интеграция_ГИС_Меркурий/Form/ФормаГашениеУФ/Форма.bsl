...
Процедура кнПолучитьВСДНаСервере()
	Фобъект = РеквизитФормыВЗначение("Объект");
	Запросов = 20;
	Пока (ФОбъект.ПолучитьСписокВСД( Объект.Отправитель_Площадка, "INCOMING","CONFIRMED", 0,ВыбначалоПериода,ВыбОкончаниеПериода)="REJECTED") и (Запросов > 0) Цикл
		Запросов = Запросов - 1;
...
Процедура кнЗагрузитьИзФайлаНаСервере()
	Фобъект = РеквизитФормыВЗначение("Объект");
	ФОбъект.ЗагрузитьXML_ВСД2(Объект.СтрИмяФайла,Объект.ВСДВходящие,Истина);
	ЗначениеВРеквизитФормы(Фобъект,"Объект");
КонецПроцедуры
