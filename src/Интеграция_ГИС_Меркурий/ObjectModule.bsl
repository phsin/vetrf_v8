...
Процедура Создать_ВСД2_Производство() Экспорт
	...
	Для каждого стрПартий из Партии Цикл
		...
		ДокВСД.НомерПартии = Формат(ДатаНачалаОтбора, "ДЛФ=Д");				
...
//        |      <vd:consignment id='id1'>
		...
		|      <vd:consignment>";
	 	...
	 	consignment = consignment + ProductItem_в_XML(СтрокаДокумента); 
		consignment = consignment+ "
		|        <vd:volume>"+ Формат(СтрокаДокумента.Количество, "ЧРД=.; ЧГ=0") +"</vd:volume>
        |        <vd:unit>
        |          <bs:guid>"+ СокрЛП(СтрокаДокумента.ЕдиницаИзмерения.GUID) +"</bs:guid>
...
Функция СформироватьЗапрос(Организации = Неопределено, ПлощадкаОтправителя = Неопределено, ДатаНачала, ДатаОкончания) Экспорт
	...
	Исключение
		Рез = Неопределено;
		Сообщить("Указан неверный параметр НазваниеРеквизитаКоличество",СтатусСообщения.Важное);
		Сообщить(Описаниеошибки());
	КонецПопытки;
...
Функция СвернутьТч(Док)
	...
	для каждого стрДок из док.Товары Цикл
		...
		Попытка
			...
			Если СписокКонстант.Получить("НазваниеРеквизитаКоличество") = "Количество" Тогда
				СтрТЧ.Количество = стрДок.Количество*стрДок.ЕдиницаИзмерения.Вес; //Продумать этот момент	
			Иначе
				СтрТЧ.Количество = стрДок[СписокКонстант.Получить("НазваниеРеквизитаКоличество")];	
			КонецЕсли;
		Исключение
			Сообщить("Указан неверный параметр НазваниеРеквизитаКоличество",СтатусСообщения.Важное);
			СтрТЧ.Количество = 0;
...
//***
//		Если (ТипЗнч(СтрОтгрузки.Док) = Тип("ДокументСсылка.РеализацияТоваровУслуг")) или
//			(ТипЗнч(СтрОтгрузки.Док) = Тип("ДокументСсылка.ПеремещениеТоваров")) Тогда
			// Заполнение шапки
			ДокВСД.ДокументОснование = СтрОтгрузки.Док;
			ДокВСД.Организация = ДокВСД.ДокументОснование.Организация;
			ДокВСД.Дата = ДокВСД.ДокументОснование.Дата;			
			...
			КонецЦикла;
			ТермическоеСостояние = ?(ТермическоеСостояние=0,1,ДокВСД.ТермическоеСостояние);
//		КонецЕсли;
