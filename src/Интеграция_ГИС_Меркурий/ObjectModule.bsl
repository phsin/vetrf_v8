// ЖД -> Функционал Распределение по Партиям ФИФО, версия для тестирования от 01/07/2018
...
Функция РассчитатьКоличествоДляВСД(СтрокаТЧДок, Продукция_Элемент = "") Экспорт
	Рез = 0;
	Попытка
		КПересчета = СокрЛП(СписокКонстант.Получить("ПарамКоэффициентПересчетаКоличества"));
		Коэфф = 0;
		Попытка
		...
		Рез = СтрокаТЧДок[СписокКонстант.Получить("НазваниеРеквизитаКоличество")]*Коэфф;	
	Исключение
...
Функция СтрокаВДатаВремя(ДатаВремяСтрока)
	...
	_смещение = Число(Сред(ДатаВремяСтрока,21,2));
	Попытка результат = Дата(год, мес,день, (час+_Смещение), мин, сек);	
		...
		Попытка
			...
			Для каждого enterprise из enterpriseList Цикл		
				Попытка
					guid = enterprise.guid;
				Исключение
					Сообщить("Не удалось прочитать GUID площадки из файла "+Компонента.LogFilename,СтатусСообщения.Внимание);
					Продолжить;
				КонецПопытки;
				Спр = Новый("СправочникМенеджер.ВСД_Площадка");
				Ссылка_ВСД_Площадка = Спр.НайтиПоРеквизиту("GUID",Guid);
...
Процедура Создать_ВСД2_Производство() Экспорт
	...
	Если НЕ ВвестиДату(ДатаВыпуска, Подсказка) Тогда
		Сообщить("Отменено");
		Возврат;
	КонецЕсли;
	...
	Для каждого стрПартий из Партии Цикл
		...
		ДокВСД.РезультатыИсследований = СписокКонстант.Получить("ВСД_РезультатыИсследований");//Перечисления.ВСД_РезультатИсследования.VSEFULL;
...
//			КонецЕсли;
		...
		ЗагрузитьXML_ВсеПартии2Ответ(Предприятие, Компонента.LogFilename, ПартииСмещение,ПартияНачПериода, ПартияКонПериода, ПартияНеНулевые );
	Иначе
		//ЖД покажем ошибки
		НайтиОшибки(Компонента.LogFilename);	
	КонецЕсли;		
...
Функция УДАЛИТЬНайтиПродукцияЭлементПоGUID(Guid)
...
//		prodType = productItem.productType;		
		...
		Если active="false" Тогда
			СпрОбъект.УстановитьПометкуУдаления(истина);
			Сообщить("Помечен на удаление ["+name+"] uuid =["+uuid+"], т.к. статус = НЕАКТИВЕН");
		Иначе	
...
//Процедура ЗаполнитьТЧВСД(ДанныеЗаполнения, докТовары, докУровниУпаковки, докМаркировка , докТермическоеСостояние, СписокПараметров) Экспорт
Процедура ЗаполнитьТЧВСД(ДанныеЗаполнения, докСсылка, тзАктуальныхПартий = "", ПолеСортировки = "", ЗнакСортировки = Неопределено) Экспорт
// заполняет ТЧ документа ВСД по Входящим данным , вызывается из ЗаполнитьНаосновании ВСД2_Транзакция
	...
	Если НЕ (ТипЗнч(тзАктуальныхПартий) = Тип("ТаблицаЗначений")) и НЕ ПарамЗаполнятьТранзакциюПриОтсутствииПартий Тогда
		Сообщить("Нет актуальных партий для создания ВСД по "+ДанныеЗаполнения.Ссылка);
		Возврат;
	КонецЕсли;
...
//Ошибка	|	@КоэффициентПересчета";
	...
	|	Дата,
	|	Номер";
//	Запрос.Текст = СтрЗаменить(Запрос.Текст,"@Количество",СписокКонстант.Получить("НазваниеРеквизитаКоличество"));
	КПересчета = СокрЛП(СписокКонстант.Получить("ПарамКоэффициентПересчетаКоличества"));
	Коэфф = 0;
	Попытка
		Коэфф = Число(КПересчета);
...
// ужесть ф-ция ВСД.ВыбратьПартию()
Функция НайтиПервуюПартию(Продукция_Элемент, Отпр_Площадка, Отпр_ХозСубъект) Экспорт
	Рез="";
	Запрос = Новый Запрос;
   	Запрос.Текст = 
	    "ВЫБРАТЬ
...
Процедура ЗаполнитьТаблицуПартийБЕЗРАСПРЕДЕЛЕНИЯ() Экспорт
// Удалить после тестирования	
	...
	Для каждого стрПартии из Партии Цикл
		Попытка
			стрПартии.Партия = НайтиПервуюПартию(стрПартии.ВСД_Продукция_Элемент,Отправитель_Площадка, Отправитель_ХозСубъект);
...
Процедура СоздатьВСД2() Экспорт
	...
	Для Каждого СтрОтгрузки Из Отгрузки Цикл
		...
		ДокВСД.ОсобыеОтметки 	= ВСД_ОсобыеОтметки;
		ДокВСД.cargoInspected  	= истина;			
		ДокВСД.РезультатыИсследований = СписокКонстант.Получить("ВСД_РезультатыИсследований");
...
Процедура СоздатьВСД2БЕЗРАСПРЕДЕЛЕНИЯ() Экспорт
	// Удалить по окончании тест Распределения
	Если Партии.Количество()=0 Тогда
		Сообщить("Не заполнена таблица партий.");
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрОтгрузки Из Отгрузки Цикл
		...
		ДокВСД.ТермическоеСостояние = 4;
		ПропускатьПустыеСвойства = СписокКонстант.Получить("ПропускатьПустыеСвойства");
		ДокоснТовары = СтрОтгрузки.Док.Товары.Выгрузить();
		Для Каждого ТекСтрокаТовары Из ДокоснТовары Цикл
			Продукция_Элемент = ВСД.Получить_ВСД_Продукция_Элемент( ТекСтрокаТовары.Номенклатура );
			Если (Продукция_Элемент = Справочники.ВСД_Продукция_Элемент.ПустаяСсылка()) и НЕ(ПропускатьПустыеСвойства) Тогда
				Сообщить("["+ТекСтрокаТовары.Номенклатура+"] -> не указан ВСД_Продукция_Элемент",СтатусСообщения.Внимание);
			ИначеЕсли (Продукция_Элемент = Справочники.ВСД_Продукция_Элемент.ПустаяСсылка()) и (ПропускатьПустыеСвойства) Тогда
				Продолжить;
			КонецЕсли;
			
			ВремПартия = ВСД.ВыбратьПартию(Продукция_Элемент, Отправитель_Площадка, Отправитель_ХозСубъект );
			
			Если НЕ(СписокКонстант.Получить("ПарамЗаполнятьТранзакциюПриОтсутствииПартий")) и НЕ(ЗначениеЗаполнено(ВремПартия)) Тогда
				Продолжить;	
			КонецЕсли;
			НоваяСтрока = ДокВСД.Товары.Добавить();
			НоваяСтрока.Номенклатура = ТекСтрокаТовары.Номенклатура;
			НоваяСтрока.Продукция_Элемент = Продукция_Элемент;//ВСД.Получить_ВСД_Продукция_Элемент( НоваяСтрока.Номенклатура );
			НоваяСтрока.Партия = ВремПартия;//ВСД.ВыбратьПартию(НоваяСтрока.Продукция_Элемент, Отправитель_Площадка, Отправитель_ХозСубъект );
			НоваяСтрока.Артикул = НоваяСтрока.Продукция_Элемент.Артикул;
			НоваяСтрока.GTIN = НоваяСтрока.Продукция_Элемент.GTIN;
			НоваяСтрока.Количество = РассчитатьКоличествоДляВСД(ТекСтрокаТовары);	
			НоваяСтрока.ЕдиницаИзмерения = НоваяСтрока.Партия.ЕдиницаИзмерения;	
			
			ДокВСД.ТермическоеСостояние = МИН(НоваяСтрока.Продукция_Элемент.ТермическиеУсловияПеревозки,ДокВСД.ТермическоеСостояние) ;
		КонецЦикла;
...
Процедура ОтправитьСписокВСД() Экспорт
	...
	Для каждого СтрОтгрузка Из Отгрузки Цикл
		...
		Если ((_ВСД.Проведен) и (СокрЛП(_ВСД.Статус) = "COMPLETED")) или (СокрЛП(_ВСД.Статус) = "COMPLETED") или (_ВСД.ПометкаУдаления) Тогда
			Сообщить("Документ ["+_ВСД+"] статус=["+СокрЛП(_ВСД.Статус)+"] уже был обработан");
			продолжить;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(_ВСД.applicationID) Тогда 
...
Функция ЗагрузитьПараметры(_Организация="") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НазваниеРеквизитаКоличество) Тогда
		...
		Сообщить("Проверьте и сохраните параметр НазваниеРеквизитаКоличество");	
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ПарамКоэффициентПересчетаКоличества) Тогда
		ПарамКоэффициентПересчетаКоличества = "1";
		Сообщить("Проверьте и сохраните параметр КоэффициентПересчетаКоличества");	
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(НазваниеРеквизитаКоличествоМест) Тогда
		НазваниеРеквизитаКоличествоМест = "КоличествоМест";
		Сообщить("Проверьте и сохраните параметр НазваниеРеквизитаКоличествоМест");	
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ВСДЦель) Тогда
		Сообщить("Проверьте и сохраните параметр ВСДЦель");	
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ВСД_РезультатыИсследований) Тогда
		ВСД_РезультатыИсследований = Перечисления.ВСД_РезультатИсследования.VSEFULL;
		Сообщить("Проверьте и сохраните параметр ВСД_РезультатыИсследований");	
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ПарамКолонкаСортировкиПартииСписания) Тогда
		ПарамКолонкаСортировкиПартииСписания = "ДатаИзменения";
		Сообщить("Проверьте и сохраните параметры Распределение партий");	
	КонецЕсли;
