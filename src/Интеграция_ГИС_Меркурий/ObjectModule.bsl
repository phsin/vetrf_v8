...
Функция ПодключитьМодульПереопределения() Экспорт
	...
	Если СписокКонстант = Неопределено Тогда
		//Либо из хранилища читать, либо возьмем из объекта	
	КонецЕсли;
     //АдресФайла= СокрЛП(ПолноеИмяФайлаОбработки);//СписокКонстант.Получить("ПолноеИмяФайлаОбработки");
	 Если НЕ ЗначениеЗаполнено(КаталогИнтеграции) Тогда
         СообщитьОшибку("Отладка -> Параметры не заполнены!");
         Возврат Неопределено;
	 Иначе
		Попытка
		    ...
		    Если Не ФайлНаДиске.Существует() Тогда
        		СообщитьИнфо("Не существует модуля переопределения "+Путь+"Меркурий_Переопределения.epf");
...
//ЖД - оптимизировать - передавать в параметре, это запрос или ответ, - Сделано, удалить ненужные ф-ции
// достаточно будет 1 функции вместо 2х
// назвать например СохранитьИмяЗапросаОтветаВДокументе(док,ЗапросОтвет="ИмяФайлаЗапрос, ИмяФайлаОтвет")

Функция СохранитьИмяЗапросаОтветаВДокументе(док,ЭтоЗапрос) Экспорт
	Если НЕ ЗначениеЗаполнено(док) Тогда
		Возврат ложь;	
	КонецЕсли;
	Если ЭтоЗапрос Тогда
		ИмяРеквизитаДок = "ИмяФайлаЗапрос";	
	Иначе
		ИмяРеквизитаДок = "ИмяФайлаОтвет";	
	КонецЕсли;
	Попытка
		Если док.Метаданные().Реквизиты.Найти(ИмяРеквизитаДок)<>Неопределено Тогда
			ДокОб = док.ПолучитьОбъект();
			ДокОб[ИмяРеквизитаДок] = КомпонентаНаСервере.LogFilenameResponse;
			ДокОб.Записать();
			Возврат Истина;
		КонецЕсли;
	Исключение 
		Сообщить("Обновите SoapDLL в ближ время");
		СообщитьОшибку(ОписаниеОшибки());
		Сообщить("Текущая версия "+КомпонентаНаСервере.Версия);
		Возврат ложь;
	КонецПопытки; //ЖД Обновить dll
КонецФункции

Функция УдалитьСохранитьОтветВДокументе(док) Экспорт
	Попытка
...
Функция УдалитьСохранитьЗапросВДокументе(док) Экспорт
	Попытка
		Если ЗначениеЗаполнено(док) Тогда
			Если док.Метаданные().Реквизиты.Найти("ИмяФайлаЗапрос")<>Неопределено Тогда
				ДокОб = док.ПолучитьОбъект();
				ДокОб.ИмяФайлаЗапрос = КомпонентаНаСервере.LogFilenameResponse;
				ДокОб.Записать();
			КонецЕсли;
		КонецЕсли;
	Исключение 
...
Процедура ЗагрузитьПлощадкиСписком()  Экспорт
...
Функция УдалитьПолучитьДанныеДляСозданияВСДТранзакции(ДокОсн = Неопределено, Получатель_Хозсубъект = Неопределено, Получатель_Площадка = Неопределено) Экспорт
    Рез = Новый Структура;
...
Функция ПолучитьРезультат_ВСД2(applicationID, докСсылка, Пауза = Истина ) Экспорт
	...
	Если ЗначениеЗаполнено(докСсылка) или ЭтоТЗ Тогда 
		Если НЕ ЭтоТЗ Тогда
			СохранитьСтатусЗапроса(докСсылка);
			СохранитьИмяЗапросаОтветаВДокументе(докСсылка,ложь);
...
Функция ПолучитьСписокВСД2_ЗапросXML(ГУИД_Площадки, vetDocumentType, vetDocumentStatus, Смещение, ПоступилиС = "", ПоступилиПо ="",ОтХСГУИД = "", ОтПлощадкиГУИД="")
	...
	Если значениеЗаполнено(ПоступилиС) тогда
		...
		|            </vd:issueDateInterval>";
	КонецЕсли;
	Если значениеЗаполнено(ОтХСГУИД) и значениеЗаполнено(ОтПлощадкиГУИД) тогда
		Запрос = Запрос +"
		|            <vd:consignor>
		|               <dt:businessEntity>
...
Функция  ПолучитьСписокВСД(Предприятие, vetDocumentType	= "INCOMING", vetDocumentStatus = "CONFIRMED", Смещение,ПоступилиС = "", ПоступилиПо ="",ОтХС = "", ОтПлощадки="") Экспорт
	...
	ЗапросXML = ПолучитьСписокВСД2_ЗапросXML(СокрЛП(Предприятие.GUID), vetDocumentType, vetDocumentStatus, Смещение,ПоступилиС , ПоступилиПо,ОтХС, ОтПлощадки);
...
Функция ИнициализацияХС_ЗагрузитьВидыПродукции(guid, Владелец) Экспорт
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		Сообщить("Отсутствует список видов продукции для продукции с ГУИД "+guid);
		Сообщить(ОписаниеОшибки());
	КонецПопытки;
