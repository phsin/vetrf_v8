// http://www.kb99.pro версия 1.7.13 от 26/01/2019
...
Функция ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементовОбщМодуль(Парам,ТабСырья, тзАктуальныхПартий = "") Экспорт
	ПартииКСписанию = Новый ("ТаблицаЗначений");
	Обработка = ИнициализацияОбработки(Парам.Организация,ложь);   
	Если типЗнч(Обработка) = Тип("Строка") тогда
	...
	Иначе
		ПартииКСписанию = Обработка.ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(Парам,ТабСырья);
	КонецЕсли;
