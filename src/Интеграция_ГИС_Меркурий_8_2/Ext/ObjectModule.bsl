...
Функция ПолучитьПартии2_ЗапросXML(Предприятие , ПартииСмещение, НачПериода = "", КонПериода = "", Ненулевые = Истина)
	...
	Если (ЗначениеЗаполнено(НачПериода)) или (НеНулевые) Тогда
		Запрос = Запрос +"
		|             <merc:searchPattern>";
		Если НеНулевые Тогда
			Запрос = Запрос +"
...
Функция  ПолучитьПартии2_ОтветXML(Предприятие, appID, ПартииСмещение, УдалятьПартии=0, ПартияНачПериода = "", ПартияКонПериода = "", ПартияНеНулевые = Ложь)
	...
	Если Результат="COMPLETED" Тогда
		...
		Если (УдалятьПартии=1)  и (ОчищатьСправочникВСД_Партии = Истина) и ПартияНеНулевые Тогда
...
Функция  ПолучитьПартии2(Предприятие, ПартииСмещение=0, УдалятьПартии=1, ПартияНачПериода = "", ПартияКонПериода = "", ПартияНеНулевые = Истина) Экспорт
...
Функция УдалитьПроверитьСуществованиеКаталога(ИмяКаталога) Экспорт
	Если НЕ(ЗначениеЗаполнено( ИмяКаталога )) Тогда
		СообщитьОшибку("Имя каталога ["+ ИмяКаталога +"] указано неправильно");
		Возврат Ложь;
	КонецЕсли;	
    ...
    Иначе
		...
		СообщитьОшибку("каталога ["+ ИмяКаталога +"] не найден, создаем...");
...
Функция УдалитьПроверитьСуществованиеФайла(ИмяФайла) Экспорт
...
//		ТекущаяНастройкаОсновногоСклада = ОбщегоНазначенияБПВызовСервера.ПолучитьЗначениеПоУмолчанию("ОсновнойСклад", ИмяПользователяИБ);
	...
	//УДАЛИТЬ ниже  , все в  ВСД.ЗагрузитьПараметрыВОбработку
	...
	Если ВСД.ПроверитьСуществованиеКаталога(КаталогИнтеграции) Тогда			
		КаталогЛогов = КаталогИнтеграции + "logs\";
		ВСД.ПроверитьСуществованиеКаталога(КаталогЛогов);
	...
	Если НЕ(ЗначениеЗаполнено(ОтладкаЗапросовXML)) Тогда 
		ОтладкаЗапросовXML = Истина;
	КонецЕсли ;
	
	ТестовыйРежим = СписокКонстант.Получить("ТестовыйРежим");
	Если НЕ(ЗначениеЗаполнено(ТестовыйРежим)) Тогда 
		ТестовыйРежим = Истина;
	КонецЕсли ;
	
	ПолноеИмяФайлаОбработки = СписокКонстант.Получить("ПолноеИмяФайлаОбработки");
	Если НЕ ВСД.ПроверитьСуществованиеФайла(ПолноеИмяФайлаОбработки) Тогда			
		ПолноеИмяФайлаОбработки = КаталогИнтеграции + "Интеграция_ГИС_Меркурий.epf";
		ВСД.ПроверитьСуществованиеФайла( ПолноеИмяФайлаОбработки );
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(НазваниеРеквизитаКоличество) Тогда
		НазваниеРеквизитаКоличество = "Количество";
	КонецЕсли;	
	...
	ПарамРазрешитьВводНаОснованииБолееОдногоВСД = СписокКонстант.Получить("ПарамРазрешитьВводНаОснованииБолееОдногоВСД");
	ПарамРазрешитьВводНаОснованииБолееОдногоВСД = ?(ЗначениеЗаполнено(ПарамРазрешитьВводНаОснованииБолееОдногоВСД),ПарамРазрешитьВводНаОснованииБолееОдногоВСД,Ложь);
	
	флИспользоватьВнешнююОбработку = СписокКонстант.Получить("ИспользоватьВнешнююОбработку");
	флИспользоватьВнешнююОбработку = ?(ЗначениеЗаполнено(флИспользоватьВнешнююОбработку),флИспользоватьВнешнююОбработку,Ложь);
		
	ПолноеИмяФайлаПереопределения = СписокКонстант.Получить("ПолноеИмяФайлаПереопределения");
	флСохранятьНастройкиДляТекущегоПользователя = СписокКонстант.Получить("флСохранятьНастройкиДляТекущегоПользователя");
