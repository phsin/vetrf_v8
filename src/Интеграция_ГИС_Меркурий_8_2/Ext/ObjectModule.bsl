...
Функция ПодключитьМодульПереопределения() Экспорт
	...
	Если НЕ ЗначениеЗаполнено( ПолноеИмяФайлаПереопределения ) или (типЗнч(МодульПереопределения) = Тип("Булево"))  Тогда 
	...
	Если НЕ ЗначениеЗаполнено(КаталогИнтеграции) Тогда
         СообщитьОшибку("Ошибка -> не заполнен параметр [КаталогИнтеграции]!");
         Возврат Неопределено;
	Иначе
		Попытка
			//Путь=КаталогИнтеграции;
			//ФайлНаДиске = Новый Файл(Путь+"Меркурий_Переопределения.epf");
			ФайлНаДиске = Новый Файл( ПолноеИмяФайлаПереопределения );
		    Если Не ФайлНаДиске.Существует() Тогда
        		СообщитьИнфо("Отладка -> Не существует модуля переопределения "+ПолноеИмяФайлаПереопределения);
				МодульПереопределения = Ложь;
		    	Возврат Неопределено;
			КонецЕсли;
			МодульПереопределения = ВнешниеОбработки.Создать( ПолноеИмяФайлаПереопределения,ложь);
        	Возврат МодульПереопределения;
		Исключение
			СообщитьИнфо("Отладка -> не удалось подключить переопределения!");
...
//	Если Результат="SUCCESS" Тогда
	...
	Попытка
		...
		Если Число(xdto.Body.getBusinessEntityListResponse.businessEntityList.total)>0 Тогда	
			...
			иначе
			// end
			guid = xdto.Body.getBusinessEntityListResponse.businessEntityList.businessEntity.guid;
...
//	Результат = КомпонентаНаСервере.GetSubProductByProductList( guid );
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		Сообщить("Отсутствует список видов продукции для продукции с ГУИД "+guid+" "+КомпонентаНаСервере.LogFilename);
		Сообщить(ОписаниеОшибки());
	КонецПопытки;
...
Функция ЗагрузитьПараметры(_Организация="") Экспорт
	Если НЕ(ЗначениеЗаполнено(_Организация)) Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение КонецПопытки;	
// Примеры стандартного функционала типовых конфигураций
