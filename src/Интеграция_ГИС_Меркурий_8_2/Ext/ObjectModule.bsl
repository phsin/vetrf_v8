...
Функция ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(Парам,тзЭлементыкСписанию, тзАктуальныхПартий = "") Экспорт
...
// вызов переопределения
	...
	Если НЕ ЗначениеЗаполнено(тзАктуальныхПартий) Тогда
		Попытка
			тзАктуальныхПартий = ПолучитьАктуальныеПартии(тзЭлементыкСписанию.ВыгрузитьКолонку("Продукция_Элемент"), Парам.Отправитель_Площадка, Парам.Отправитель_ХозСубъект);
		Исключение
			Сообщить("Не переданы параметры для получения Актуальных партий - ДанныеЗаполнения или тзЭлементыОснования");
...
//	ПартииКСписанию = ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(Парам,ТабСырья);
	ДокВСД.ПартииСписания.Загрузить(ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(Парам,ТабСырья));
КонецПроцедуры
...
Процедура Создать_ВСД2_Производство(ДатаВыпуска = "") Экспорт
	...
	тзАктуальныхПартийСписанияВПроизводство = ПолучитьАктуальныеПартии(ТзНеобходимогоСырья,Отправитель_Площадка,Отправитель_ХозСубъект);
...
//		КонецЕсли;
		...
		ТабСырья = ВСД.СформироватьТЧСырьяДляСписанияВПроизводствоПоСписку(ДокВСД.Продукция.Выгрузить());
		Парам = Новый Структура("Отправитель_ХозСубъект, Отправитель_Площадка",ДокВСД.Производитель_ХозСубъект,ДокВСД.Производитель_Площадка);	
		ДокВСД.ПартииСписания.Загрузить(ПолучитьТЗПартииКСписаниюПоТЗПродукцияЭлементов(Парам,ТабСырья,тзАктуальныхПартийСписанияВПроизводство));			
