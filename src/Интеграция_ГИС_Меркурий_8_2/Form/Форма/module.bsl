...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(НазваниеРеквизитаКоличество) Тогда
		Сообщить("Не указан Параметр [НазваниеРеквизитаКоличество] !");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ПарамКоэффициентПересчетаКоличества)Тогда 
		Сообщить("Не указан Параметр [ПарамКоэффициентПересчетаКоличества] !");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(НазваниеРеквизитаКоличествоМест) Тогда
		Сообщить("Не указан Параметр [НазваниеРеквизитаКоличествоМест] !");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ВСДЦель) Тогда
		Сообщить("Не указан Параметр [ВСДЦель] !");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ВСД_РезультатыИсследований) Тогда
		Сообщить("Не указан Параметр [ВСД_РезультатыИсследований] !");		
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ПарамКолонкаСортировкиПартииСписания) Тогда
		Сообщить("Не указан Параметр [ПарамКолонкаСортировкиПартииСписания] !");		
	КонецЕсли;
...
Процедура СохранитьПараметры( _Организация )
	
	ВСД.СохранитьПараметры(ЭтаФорма);
	
	
	ИмяПользователя = ИмяПользователя();
	Сообщить("Настройки сохранены для "+ИмяПользователя+" - "+_Организация);
	...
	Если НЕ ЗначениеЗаполнено( КаталогИнтеграции) Тогда 
		КаталогИнтеграции = КаталогПрограммы();
	КонецЕсли;
	...
	СписокКонстант.Вставить("КаталогИнтеграции", КаталогИнтеграции);		
	СписокКонстант.Вставить("ПолноеИмяФайлаОбработки", ПолноеИмяФайлаОбработки);		
	СписокКонстант.Вставить("ПолноеИмяФайлаПереопределения", ПолноеИмяФайлаПереопределения);		
	...
	СписокКонстант.Вставить("ПарамРазрешитьВводНаОснованииБолееОдногоВСД",ПарамРазрешитьВводНаОснованииБолееОдногоВСД);

	СписокКонстант.Вставить("ИспользоватьВнешнююОбработку", флИспользоватьВнешнююОбработку);
...
Процедура кнЗагрузитьПартииНажатие(Элемент)
	...
	Если ВвестиДату(ВыбДата, "Выберите дату начала запроса",ЧастиДаты.Дата ) Тогда 
		ПолучитьПартии2( Отправитель_Площадка, ,, ВыбДата, ТекущаяДата() );
	КонецЕсли;
КонецПроцедуры
...
Процедура кнЗагрузитьПлощадкиТЧНажатие(Элемент)
	...
	спЗначений.Добавить("ПоХС","Найти по ХозСубъекту");
	спЗначений.Добавить("ПоНаименованию","Найти по Наименованию",истина, БиблиотекаКартинок.Найти);
	
	//ВыбранныйЭлемент = ВыбратьИзСписка(спЗначений, ЭлементыФОрмы.кнЗагрузитьПлощадкиТЧ);	
	ВыбЭлемент = спЗначений.ВыбратьЭлемент();
	Если ВыбЭлемент = Неопределено Тогда
		Возврат;
	ИначеЕсли ВыбЭлемент.Значение="ПоХС" Тогда
		ЗагрузитьПлощадкиСписком();	
		Автозаполнение(ЭтаФорма.ФПеремещения);
		
	ИначеЕсли ВыбЭлемент.Значение="ПоНаименованию" Тогда		
		...
		Форма.ВыбХозСубъект = ВыбХозСубъект;
		Форма.НазваниеПлощадки = ВыбХозСубъект.Наименование;
		Форма.Регион = СписокКонстант.Получить("Регион");
		Форма.Открыть();
		//СнятьПометки(
	КонецЕсли;
...
Процедура ПлощадкаНачалоВыбора(Элемент, СтандартнаяОбработка) Экспорт
	...
	ГУИДХСдляОтбора = ЭтаФорма.ТекущийЭлемент.ТекущиеДанные.ХозСубъект.GUID;
...
Процедура ТабличноеПоле1ПлощадкаПриИзменении(Элемент)
	...
	СоздатьНовуюПлощадку= Ложь;
	Если НЕ(Выбор_ВСД_Площадка = Справочники.ВСД_Площадка.ПустаяСсылка()) Тогда 
		Ответ = Вопрос("Создать новую привязку Контрагента к Площадке?", РежимДиалогаВопрос.ДаНет);
		Если Ответ = КодВозвратаДиалога.Нет Тогда 
			...
			КонецПопытки;
		Иначе
			СоздатьНовуюПлощадку= Истина;
		КонецЕсли;		
	КонецЕсли;		
	Если НЕ (Элемент.Значение = Справочники.ВСД_Площадка.ПустаяСсылка()) Тогда 
		Если СоздатьНовуюПлощадку Тогда 
			ТекЗначение = 
			СпрОб = Справочники.ВСД_Площадка.СоздатьЭлемент();
			СпрОб.GUID = Элемент.Значение.GUID;
			СпрОб.GuidХозСубъекта = Элемент.Значение.GuidХозСубъекта;
			СпрОб.UUID = Элемент.Значение.UUID;
			СпрОб.Адрес = Элемент.Значение.Адрес;
			СпрОб.Наименование = Элемент.Значение.Наименование;
...
Процедура ТабличноеПоле1ПриПолученииДанных(Элемент, ОформленияСтрок)
    Для Каждого ОформлениеСтроки Из ОформленияСтрок Цикл
		ОформлениеСтроки.Ячейки.колонкаНомерДок.ОтображатьТекст = Истина;
		ДокВСтроке = ОформлениеСтроки.ДанныеСтроки.Док;
		ОформлениеСтроки.Ячейки.колонкаНомерДок.Текст = ДокВСтроке.Номер;
		Если ДокВСтроке.Метаданные().Реквизиты.Найти("АдресДоставки") <> Неопределено Тогда
			ОформлениеСтроки.Ячейки.колонкаАдресДоставки.ОтображатьТекст = Истина;
			ОформлениеСтроки.Ячейки.колонкаАдресДоставки.Текст = ДокВСтроке.АдресДоставки;
		Иначе
			ОформлениеСтроки.Ячейки.колонкаАдресДоставки.Текст = "";
		КонецЕсли
    КонецЦикла;
КонецПроцедуры
...
Процедура кнЗагрузитьПлощадкиПоНаименованиюНажатие(Элемент)
	Форма = ПолучитьФорму("Форма_ЗагрузкаПлощадок");
	Форма.ВыбХозСубъект = ВыбХС;
	Форма.НазваниеПлощадки = ВыбХС.Наименование;
	Форма.Регион = СписокКонстант.Получить("Регион");
	Форма.Открыть();
...
Процедура кнПечатьНажатие(Элемент)
	...
	спЗначений = Новый СписокЗначений;
	спЗначений.Добавить("ВСД","ВСД сокращенная форма (с просмотром)");
	спЗначений.Добавить("ВСД_печать","ВСД сокращенная форма (сразу на печать)");
	спЗначений.Добавить("Реестр","Реестр ВСД",, );
	
	//ВыбранныйЭлемент = ВыбратьИзСписка(спЗначений, ЭлементыФОрмы.кнЗагрузитьПлощадкиТЧ);	
	ВыбЭлемент = спЗначений.ВыбратьЭлемент();
	Если ВыбЭлемент = Неопределено Тогда
		Возврат;
	ИначеЕсли ВыбЭлемент.Значение="ВСД" Тогда
		ПечатьСокрВСД(Ложь);
	ИначеЕсли ВыбЭлемент.Значение="ВСД_печать" Тогда
		ПечатьСокрВСД(Истина);
	ИначеЕсли ВыбЭлемент.Значение="Реестр" Тогда		
		ПечатьРеестра(Ложь);
	КонецЕсли;
...
Процедура ВСД_транзакцияНачалоВыбора(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ЗначениеОтбора = Новый Структура("Отправитель_ХозСубъект", Отправитель_ХозСубъект);
	ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора,Отбор", Истина, Истина,ЗначениеОтбора);	
	ОткрытьФорму("Документ.ВСД_транзакция.ФормаВыбора", ПараметрыПодбора, Элемент);	
...
Процедура ВСД_производствоНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ЗначениеОтбора = Новый Структура("Отправитель_ХозСубъект", Отправитель_ХозСубъект);
	ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора,Отбор", Истина, Истина,ЗначениеОтбора);	
	ОткрытьФорму("Документ.ВСД2_Производство.ФормаВыбора", ПараметрыПодбора, Элемент);	
...
Процедура Транзакция_Аннулируемый2НачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ЗначениеОтбора = Новый Структура("Отправитель_ХозСубъект", Отправитель_ХозСубъект);
	ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора,Отбор", Истина, Истина,ЗначениеОтбора);	
	ОткрытьФорму("Документ.ВСД2_транзакция.ФормаВыбора", ПараметрыПодбора, Элемент);	
КонецПроцедуры
...
Процедура ВСД_АннулируемыйНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	ЗначениеОтбора = Новый Структура("Отправитель_ХозСубъект", Отправитель_ХозСубъект);
	ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора,Отбор", Истина, Истина,ЗначениеОтбора);	
	ОткрытьФорму("Документ.ВСД.ФормаВыбора", ПараметрыПодбора, Элемент);	
КонецПроцедуры
...
Процедура ВыбПлощадкаНачалоВыбора(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ГУИДХСдляОтбора = ЭтаФорма.ВыбХС.GUID;
	ГУИДХСдляОтбора = ?(ЗначениеЗаполнено(ГУИДХСдляОтбора),ГУИДХСдляОтбора,"****");
	ФормаВыбора =  Справочники.ВСД_Площадка.ПолучитьФормуВыбора(, Элемент);
	Если ТипЗнч(ФормаВыбора) = Тип("УправляемаяФорма") Тогда
		ЗначениеОтбора = Новый Структура("GuidХозСубъекта", ГУИДХСдляОтбора);
...
Процедура кнВСД(Элемент)
	
	спЗначений = Новый СписокЗначений;
	спЗначений.Добавить("Отправить","Отправить ВСД");//,, БиблиотекаКартинок. ОтправитьЭлектронныеПисьма);
	спЗначений.Добавить("ПолучитьОтветы","Получить ответы");//,, БиблиотекаКартинок.ЗагрузитьИзменения);
	спЗначений.Добавить("Аннулировать","Аннулировать ВСД");//,, БиблиотекаКартинок.Остановить);
	спЗначений.Добавить("Регионализация","Получить условия перевозки");//,, БиблиотекаКартинок.ЗначокСклад16);
	
	//ВыбранныйЭлемент = ВыбратьИзСписка(спЗначений, ЭлементыФОрмы.кнЗагрузитьПлощадкиТЧ);	
	ВыбЭлемент = спЗначений.ВыбратьЭлемент();
	Если ВыбЭлемент = Неопределено Тогда
		Возврат;
	ИначеЕсли ВыбЭлемент.Значение="Отправить" Тогда
		кнОтправитьВСДНажатие(Элемент);
	ИначеЕсли ВыбЭлемент.Значение="ПолучитьОтветы" Тогда		
		кнПолучитьОтветВСД2Нажатие(Элемент);
	ИначеЕсли ВыбЭлемент.Значение="Аннулировать" Тогда		
		кнАннулироватьВыбранныеВСДНажатие(Элемент);
	ИначеЕсли ВыбЭлемент.Значение="Регионализация" Тогда		
		кнЗапросРегионализацияНажатие(Элемент);
	КонецЕсли;
	
КонецПроцедуры

Процедура кнПартииНажатие(Элемент)
	спЗначений = Новый СписокЗначений;
	спЗначений.Добавить("ЗагрузитьПартии","Загрузить Партии");//,,БиблиотекаКартинок.ЗагрузитьИзИнтернета16);
	спЗначений.Добавить("УстановитьСоответсвия","Установить соответствия");//,, БиблиотекаКартинок.Найти);
	спЗначений.Добавить("Гашение","Гашение входящих партий");//,, БиблиотекаКартинок.ПолучениеПочты);
	
	//ВыбранныйЭлемент = ВыбратьИзСписка(спЗначений, ЭлементыФОрмы.кнЗагрузитьПлощадкиТЧ);	
	ВыбЭлемент = спЗначений.ВыбратьЭлемент();
	Если ВыбЭлемент = Неопределено Тогда
		Возврат;
	ИначеЕсли ВыбЭлемент.Значение="ЗагрузитьПартии" Тогда
		кнЗагрузитьПартииНажатие(Элемент);
	ИначеЕсли ВыбЭлемент.Значение="УстановитьСоответсвия" Тогда		
		Форма = ПолучитьФорму("Форма_Соответствия");
		ОткрытьФорму(Форма);
	ИначеЕсли ВыбЭлемент.Значение="Гашение" Тогда		
		Форма = ПолучитьФорму("ФормаГашение");
		ОткрытьФорму(Форма);	
	КонецЕсли;
...
Процедура кнОбработкиНажатие(Элемент)
	спЗначений = Новый СписокЗначений;	
	спЗначений.Добавить("УстановитьСоответсвия","Установить соответствия");//,, БиблиотекаКартинок.Найти);
	спЗначений.Добавить("Гашение","Гашение входящих партий");//,, БиблиотекаКартинок.ПолучениеПочты);
	спЗначений.Добавить("Создать_Продукци_Элементы","Создать продукцию");
	...
	ИначеЕсли ВыбЭлемент.Значение="Гашение" Тогда		
		Форма = ПолучитьФорму("ФормаГашение");
		ОткрытьФорму(Форма);	
	ИначеЕсли ВыбЭлемент.Значение="Создать_Продукци_Элементы" Тогда
		//ОповещениеОВыборе  = Новый ОписаниеОповещения("ОткрытиеФормыГашения", ЭтаФорма, Новый Структура("ИмяКоманды", ""));
		ОткрытьФорму("Обработка.Интеграция_ГИС_Меркурий.Форма.ФормаСоздатьВСД_ЭлементыУФ", , Элемент );	
	КонецЕсли;
КонецПроцедуры
