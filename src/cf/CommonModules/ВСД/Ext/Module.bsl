...
Функция НайтиВСД_ИсходящийНаВозврат(ДокументОснование) Экспорт
// Удалить - ВСД2_исходящий не используется
     Запрос = Новый Запрос;
     Запрос.УстановитьПараметр("ДокументОснование" , ДокументОснование);

     Запрос.Текст = "ВЫБРАТЬ
                    |    ВСД2_исходящий.Ссылка
                    |ИЗ
                    |    Документ.ВСД2_исходящий КАК ВСД2_исходящий
                    |ГДЕ
                    |    (ВСД2_исходящий.ДокументОснование = &ДокументОснование)
					|    И (ВСД2_исходящий.ПометкаУдаления = ЛОЖЬ)";
...
Функция ИнициализацияОбработки( Организация="", Инициализировать = Истина) Экспорт
	...
	Если Организация = "" Тогда
	 ...
	 Если СписокПараметров.Получить("ИспользоватьВнешнююОбработку") Тогда 
		 ...
		 АдресФайла = Строка(СписокПараметров.Получить("КаталогИнтеграции"))+"Интеграция_ГИС_Меркурий.epf";
